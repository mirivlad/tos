...
events() {
    tr -d '\r' < "$1" | grep '^TOS\.' |
        sed -E 's/(fb_(format|width|height|pitch)|stack|runtime|available|begin|end|spin_begin|spin_end|ticks|first_tick|last_tick|admitted_frames|table_reserve_frames|table_reserve_free|pool_frames|root_frames|nucleus_space_frames)=[^ ]*/\1=<platform>/g'
}
