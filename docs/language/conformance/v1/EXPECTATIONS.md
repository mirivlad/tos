...
| C030 | `accept/import-envelope-chain.tos` plus `accept/imported-surface.tos` | Bootstrap | accepts a module declaring `imports: 1` that reaches exactly one module | ADR-0091: `docs/41` §6 bounds the module's **transitive** dependency count, and a declaration equal to it is exact rather than short. This vector is also the middle of R096's chain |
...
| A014 | `accept/contextual-integer-literal.tos` | Bootstrap | accepted | ADR-0087 §2a and docs/40 §3: an unsuffixed literal takes the type its position requires — under a `size`, `i64` and `u64` annotation, as an argument, and with no expectation at all — so none of them is a mismatch |
...
| R086 | `reject/value-type-binding.tos` | Bootstrap | `E1216_VALUE_TYPE_MISMATCH` with `context=binding`, `expected=bool`, `actual=i64` | ADR-0087: a binding's annotation constrains its initializer, and no more specific code owns a `bool`/`i64` disagreement |
| R087 | `reject/value-type-text-bytes.tos` | Bootstrap | `E1216_VALUE_TYPE_MISMATCH` with `context=binding`, `expected=string`, `actual=bytes` | ADR-0087: the residual code covers every exact-type position, not only the numeric ones |
| R088 | `reject/value-type-array-element.tos` | Bootstrap | `E1216_VALUE_TYPE_MISMATCH` with `context=binding` | ADR-0087: an aggregate's type comes from its components and its declared type, and here they disagree |
| R089 | `reject/integer-type-binding.tos` | Bootstrap | `E1210_INTEGER_TYPE_MISMATCH` | **The precedence vector** (ADR-0087 §2a). Two exact integer types disagree, so `E1210` owns it; reporting `E1216` here would be the residual code absorbing a specific one |
| R090 | `reject/call-arity-local.tos` | Bootstrap | `E1217_CALL_ARITY_MISMATCH` with `callee=add`, `expected=2`, `actual=1`, `context=local named` | ADR-0089: `docs/43` §4's exact ordered operand list, said in source. Determined before any argument-type code, because one argument to a two-parameter callee forms no complete positional correspondence |
| R091 | `reject/call-arity-value.tos` | Full | `E1217_CALL_ARITY_MISMATCH` with `callee=twice`, `expected=1`, `actual=2`, `context=callable value` | ADR-0089: a callable value's parameter list is exact in the way a declared function's is, and is read from the function type |
...
| R096 | `reject/import-envelope-exceeded.tos` | Bootstrap | `E1705_IMPORT_ENVELOPE_EXCEEDED` with `declared=1`, `actual=2` | ADR-0091: one `import` declaration, two module dependencies — the closure is this vector with `accept/import-envelope-chain.tos` and `accept/imported-surface.tos` behind it. `docs/41` §6 bounds the transitive count, so the module the import names brings its own dependency into this module's envelope. Reported at the `imports:` value token during module-set resolution, which is the first pass that can know the resolved graph, and only when that graph resolved — `E1604`, `E1605`, `E1606` and `E1609` own an unmeasurable one |
