...
| `E1217_CALL_ARITY_MISMATCH` | a source call supplies a different number of arguments than the resolved callee declares (ADR-0089). Fields: `callee`, `expected`, `actual`, `context`. `context` names the kind of callee — `local named`, `imported named`, `predeclared`, `callable value`. **Determined before any positional argument-type code**: with the wrong number of arguments there is no complete positional correspondence to type-check, so `E1210`, `E1211`, `E1215` and `E1216` describe pairs the source never formed. Once the counts agree their precedence is unchanged. A resolution finding still comes first: an unresolved callee declares no count and is `E1202`. `imported named` is a registered owner whose check waits on imported signature transport; `docs/43` §4's exact ordered operand list is what the code names, so the independent verifier's `V2011_CFG` refusal of a malformed artifact is unaffected |
...
| `E1705_IMPORT_ENVELOPE_EXCEEDED` | the module's exact resolved transitive dependency set contains more unique module dependencies than its declared `resource imports` value permits (ADR-0091). Fields: `declared`, `actual`, and **not** the dependency list, which reaches 255 identities at the ceiling. The count is of unique resolved module identities reachable from the module, direct and indirect together, the module itself excluded, two bindings of one dependency counted once, and a capability-interface import counted not at all — it names an interface contract rather than a module of this source set (`docs/42` §4). Taken over the resolved graph, never over source spelling. Reported at the value token of the module's `imports:` resource declaration. **Only over a graph that resolved**: `E1604`, `E1605`, `E1606` and `E1609` own an unmeasurable one, and no other finding suppresses it. Its stage is `resource` because that is the class of the error, while the pass that discovers it is module-set resolution — the first phase holding the graph the count is taken over (ADR-0091 §3a). **Not ADR-0090's edge ceiling**, which bounds unique direct edges in a whole closure; neither quantity bounds the other. The independent launch proves the same bound from authenticated artifacts as `V2022_RESOURCE` |
