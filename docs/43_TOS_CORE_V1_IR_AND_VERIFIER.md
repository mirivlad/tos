...
| region/DMA | typed grant, rights, checked range/alignment, transfer/share rule, no physical-address exposure. **DMA ordering** (ADR-0086, minor 4): for `DmaSync`, the artifact's minor admits the operation, the region operand dominates it, its exact type is `DmaRegion(_)` or `DmaRegionMut(_)` and every other type refuses, the direction is one value of the closed discriminator, the result is `unit`, and no ownership is consumed. It is proved from the artifact, never from a producer-supplied string, and is not representable as `Call`, `Capability`, `Read`, `Write`, `MmioRead` or `MmioWrite` |
