...
- **Stage 4A — половина построена и зелёная, вторая половина упёрлась во второй
  ...
  **Не достигнуто:** чтение configuration space из текста. Операции есть и
  ...
  теперь 6.
- **Stage 4B закрыт** (2026-09-04). ADR-0081 принят: модель доступа к региону
  решена один раз для обычной памяти, DMA и device-памяти, с явно раздельными
  видами. Реализовано: индексный доступ `r[i]` для четырёх режимов Region/
  ...
  ADR-0075/0076 **не** расширены на device-память.
