...
**E. ADR-0058 (Proposed) — объёмные аргументы вызова.** Один вопрос, три места:
сообщение, передающее capability (`IPC_V1` §3 и ADR-0057 дают четыре и два, а
свободных регистров четыре на всё вместе со счётчиками); наделение ребёнка при
`process_create`, которое ADR-0055 §A описывает как аргумент; и имя модуля,
которое сегодня — порядковый номер в списке, который никто не публиковал.
Варианты: **A** — слот сообщения становится областью объёмных аргументов вызова
(механизм уже есть и уже обоснован: адрес выбрало ядро, читает через свою
identity-карту, размер объявлен регистром); **B** — область, названная
region-capability, что буквально отвечает §3 и циклично на bootstrap; **C** —
накопление за несколько вызовов, что кладёт в ядро состояние между вызовами и
делает «доставлено целиком или никак» обязательством вместо свойства.
Рекомендация — **A**, с тремя оговорками в тексте решения: область принадлежит
**контексту исполнения**, а не процессу (сегодня контекст один, и сказать это
сейчас стоит предложение, а найти потом — порчу памяти, похожую на дефект
планировщика); её содержимое — аргументы и никогда не канал; каждая раскладка
фиксируется контрактом, владеющим операцией.

**E2. ADR-0059 (Proposed) — блокировка.** Форма согласована с владельцем: состояние
`Blocked` с точным пробуждением; **правило живости** как клапан (когда бежать
некому, а ждущие есть, ничто маршрутизированное в Stage 3 этого не изменит —
ядро снимает все блокировки с `E_CANCELLED` и говорит, кто на чём стоял);
маленький счётчик срабатываний подряд без доставки между ними как терминатор
ливлока; **терпение** — не ядру, а тому, кто запускал. Осталось записать
решением. Правило живости сформулировано как «нет Runnable **и ничто
маршрутизированное не может это изменить**»: сегодня вторая половина верна по
построению, а на Stage 4 появится устройственное прерывание — и формулировка без
неё стала бы тихо неверной ровно в тот момент, когда появится драйвер.

Оба ADR — `Proposed`, строка approval пустая. Ни один не реализуется до подписи.
Они независимы друг от друга: блокировке объёмные аргументы не нужны, передаче
capability блокировка не нужна.
