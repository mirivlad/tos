...
- **Типизированное значение (2026-09-14).** Устранена корневая причина двух
  предыдущих починок: у каждого понижаемого операнда и места теперь ровно один
  точный тип, а lowering, не сумевший его определить, прекращается явно вместо
  подстановки какого-нибудь валидного `TypeId`. Контекстные беcсуффиксные
  литералы разрешаются в тип своей позиции — чем регрессия `1cf4c74` исправлена
  вперёд, без ослабления верификатора. Выделен `E1216_VALUE_TYPE_MISMATCH`
  (ADR-0087); минор языка не потребовался. Две строки канонического исходника
  Stage 4 исправлены с санкции Архитектора: `region[...] as u64` — исходник сам
  нарушал `docs/40` §3. Аппаратные свидетельства не изменились. **Stage 4D-4
  остаётся заблокированным**; открытым остаётся частичная типизация чекера.
- **Граница вызова починена (2026-09-13).** Независимый верификатор теперь
  проверяет у вызова точный упорядоченный список операндов — арность, типы
  операндов и объявленный результат — как docs/43 §4 требовал всегда; раньше у
  `Op::CallValue` проверялось только что вызываемое имеет функциональный тип, а
  у локального вызова — только что индекс в пределах таблицы. Проверка вскрыла
  четыре места, где корректный канонический исходник лоуэрился с неверными
  типами, и все четыре починены. Дайджест пониженного модуля Stage 4D-2 из-за
  этого сдвинулся — исходник побайтно тот же, сдвинулся производный артефакт,
  чья таблица типов была неверной; аппаратная семантика не изменилась, и
  свидетельство Stage 4D-3 побайтно то же. **Stage 4D-4 остаётся
  заблокированным**, и следующий срез — не он: оракул типов в lowering'е
  неисправен шире починенных мест, и это блокирующий дефект типовой надёжности
  Core. Подробности и полный список нерешённого — в записи журнала за эту дату.
- **Языковая починка после 4D-3 (2026-09-12).** Два дефекта реализации TOS
  Core V1, найденные при постройке Stage 4D-3 и записанные в его свидетельстве,
  закрыты отдельным срезом: `borrow mut` теперь сохраняет заимствованное место
  через границу вызова (локального, межмодульного и вызова замыкания), а
  `array<T, N>` принимает ту `const_expression`, которую даёт принятая
  грамматика docs/39, включая именованную константу. Третий дефект внесён и
  закрыт внутри самого среза: обратная запись через callable-значение могла
  положить величину чужого типа в место вызывающего, и теперь несовпадение —
  ловушка, а не тихая запись. Неразличимость режимов в типе callable вынесена
  Архитектору: она в принятом контракте, а не в реализации. Аппаратное поведение не
  изменилось: Stage 4D-1/4D-2/4D-3 перезапущены без изменений, свидетельство
  4D-3 побайтно то же. Схема IR не менялась. Подробности — в записи журнала
  ниже за эту же дату.
- **Где проект находится сейчас (2026-09-12).** Stage 4C построен целиком
  (4C-0 liveness, 4C-1 IRQ authority, 4C-2 DMA authority, 4C-3 порядок MMIO ↔
  DMA) и Stage 4D доведён до 4D-3. Формального closure approval ни у 4C, ни у
  4D нет и здесь он не заявляется. **Журнал верификации ниже не вёлся для
  4C-2, 4C-3, 4D-1 и 4D-2** — записи заканчиваются на 4C-1b (2026-09-07), и это
  пробел в рабочем логе, а не пробел в свидетельстве: свидетельство этих
  срезов — `docs/evidence/STAGE4C2_CAPABILITY_REPRESENTATION.md`,
  `STAGE4C3_DMA_ORDERING.md`, `STAGE4D1_FIRST_VIRTQUEUE.md`,
  `STAGE4D2_FIRST_BLOCK_READ.md`, плюс принятые ADR-0084 и ADR-0086 и
  соответствующие гейты профиля `qemu`. Ранние записи не переписывались, чтобы
  не изображать, будто эти возможности существовали раньше, чем существовали.
- **Stage 4D-5 построен и зелёный** (2026-09-17). Канонический текстовый модуль
  **впервые меняет состояние настоящего устройства**: составляет 512 байт по
  правилу `(0x5C + 7*i) mod 256`, пишет их в сектор 5 одним `VIRTIO_BLK_T_OUT`
  с дескриптором нагрузки, доступным устройству **на чтение**, затирает буфер
  нагрузки нулями и читает тот же сектор обратно в **другой** отравленный буфер
  одним `VIRTIO_BLK_T_IN`, получая ровно эти байты. Байт статуса доказательством
  не служит: помеченная на запись нагрузка даёт `S_OK` и нетронутый диск.
  Ёмкость читается у устройства до первой публикации — обход настоящего списка
  capability до `VIRTIO_PCI_CAP_DEVICE_CFG` (первая подходящая — §4.1.4.1) и
  64-битный `capacity` по протоколу поколений §2.5.1 (32768 секторов за одну
  попытку); диск из 4 секторов даёт
  `-44` и ноль прерываний. Нуклеус не получил ничего. Свидетельство —
  `docs/evidence/STAGE4D5_FIRST_WRITE.md`. **Не доказано и не заявляется:**
  долговечность, персистентность на хосте, fsync, flush, файловая система.
- **Stage 4D-4 построен и зелёный** (2026-09-16). Канонический текстовый модуль
  держит **два настоящих `VIRTIO_BLK_T_IN` одновременно** в одной уже
  инициализированной split-очереди: обе цепочки строятся из пула шириной ровно в
  две цепочки, обе головы пишутся в соседние слоты available-кольца, и **один**
  store двигает `avail.idx` с 0 на 2 — после чего обе цепочки принадлежат
  устройству, а потребительский индекс ещё 0. Завершения сопоставляются по
  `used_elem.id` с незавершёнными головами, а не по слоту кольца, потому что
  `VIRTIO_F_IN_ORDER` не согласован и §5.2.6 говорит о блочных запросах «not
  necessarily in order»; оба порядка допустимы. У каждого запроса свой буфер,
  своя отрава и свой сектор. Один DMA-регион на 8 KiB, `capability_delta=1`.
  Нуклеус насчитал **одну** доставку MSI-X на **два** завершения — свойство
  батчинга ADR-0082 §10, впервые задействованное; это подтверждает путь дренажа
  у драйвера и **не** устанавливает параллельного исполнения на устройстве.
  Свидетельство —
  `docs/evidence/STAGE4D4_TWO_OUTSTANDING.md`. **Не доказано и не заявляется:**
  что устройство исполняло оба запроса в один физический момент, запись,
  мультиплексирование очередей, планирование, файловая система, фреймворк.
  **Исторический блокер Stage 4D-4 снят** — подробности в записи журнала.
- **Stage 4D-3 построен и зелёный** (2026-09-12). Канонический текстовый модуль
  проводит **две последовательные настоящие операции `VIRTIO_BLK_T_IN` через
  одну инициализированную split-очередь**: одна инициализация устройства, одно
  согласование фич, один `queue_enable`, один DMA-регион и один MSI-X источник
  на оба запроса. Пул дескрипторов — четыре записи таблицы, по три на запрос,
  то есть второй запрос арифметически невозможен без возврата дескрипторов
  первого; свободный список — FIFO, поэтому вторая цепочка это `3 -> 0 -> 1`, с
  другой головой и двумя переиспользованными дескрипторами. Индексы кольца
  больше не константы: `avail.ring[avail_idx % qsz]`, `used.ring[last_seen_used
  % qsz]`, оба счётчика переживают запрос, арифметика 16-битного переполнения
  проверяется через границу 65535 -> 0 до первого обращения к устройству.
  Образ диска профиля Stage 4 теперь засевается различимым содержимым: сектор
  `n` — 512 копий байта `0xC0 + n` для секторов 1..4, сектор 0 остаётся нулевым,
  так что свидетельство 4D-2 не тронуто. Нуклеус не получил ни одной новой
  операции, версии интерфейса, minor'а языка, полномочия или механизма.
  Свидетельство — `docs/evidence/STAGE4D3_QUEUE_REUSE.md`. **Не доказано и не
  заявляется:** несколько запросов одновременно, запись, мультиплексирование
  очередей, планирование, файловая система, драйверный фреймворк.
- **Stage 4C начат** (2026-09-05). Подэтап **4C-0 построен и зелёный**: правило
  liveness `SYSTEM_ABI_V1` §6 получило вторую половину — нуклеус вычисляет
  перепись заблокированных контекстов и их источников пробуждения вместо
  булева «что-то заблокировано». Это завершение реализации принятого правила, а
  не новое решение; поведение не изменилось, потому что все четыре
  существующие причины блокировки — peer, и это доказано аудитом, а не
  предположено. Свидетельство — `docs/evidence/STAGE4C_LIVENESS.md`, запись —
  ADR-0059 §«Realisation». Дальше по обязательной декомпозиции: 4C-1 (IRQ
  authority и routed delivery), 4C-2 (DMA authority), 4C-3 (порядок MMIO ↔ DMA).
  Stage 4D не начат.
- **Stage 4C-1b построен и зелёный** (2026-09-07). Канонический текстовый
  драйвер, держащий одну PCI-функцию, выводит из неё `platform.irq.Source`,
  блокируется в `irq_wait` и просыпается от **настоящего MSI-X прерывания
  настоящего устройства**. Перепись liveness впервые отвечает `routed=1`.
  Реализовано по ADR-0082: `pci_interrupt_claim` (операция 28), `irq_wait`
  (операция 29), права `interrupt` и `wait`, объект `platform.irq.Source`,
  списываемый аллокатор векторов, однобитная защёлка pending, один ожидающий,
...
- **ADR-0083 Accepted** (Vladimir Tomashevskiy, 2026-09-06). Stage 4C-1 обнажил
  ...
  интервалах с разным началом, поэтому инертное смещение раскладки, не
  исполняющее ничего, двигало отношение через границу conformance при
  неподвижном native. Метрика заменена на same-artifact paired: один
  измерительный образ, два режима во время исполнения, общая граница
  `TOS.TEST.PAIRED.START`, блокирующая линия `p95 <= 1,30`, удержанный baseline
  1,0076. Старое отношение сохранено как историческое и больше не является
  активной конформностью. Performance STOP снят.
...
### Состояние на 2026-09-26 — настоящее время, по категориям

**Раздел в настоящем времени, и единственный в «Текущей позиции», который им
...
Stage 1.5, Stage 2, Stage 3, Stage 4A, Stage 4B.

**Построено и зелено, не закрыто: всё остальное из списка deliverables Stage 4
`docs/16`.** PCI discovery, контракты прерываний/MMIO/DMA (4C-1…4C-3), текстовый
драйвер VirtIO block (4D-1…4D-5), сервис и принятый протокол `block.device.v1`,
persistent object/state storage (ADR-0099) и первая половина capsule-to-repository
handoff — связывание и проверка (ADR-0102). Crash/reset: смерть сервиса и успешник со
сбросом устройства (`block-lifecycle.sh`), случай D ADR-0093 как граница и
незавершённый `READ` (`block-fault.sh`). Враждебное устройство: отказ реального запроса
через QEMU `blkdebug`, ложь о кольце от тестового DMA-противника (`block-fault.sh`),
устройство, которое никогда не затихает (`dma-quarantine.sh`). Обязательства ADR-0084
§8, у которых не было свидетельства в рантайме (4, 9a, 9b, 11, 12, 13a), теперь
загейчены. Модель угроз сведена: X4.1–X4.12 и список того, чего Stage 4 не заявляет.

**Аудит готовности к закрытию** — `docs/evidence/STAGE4_CLOSURE_AUDIT.md`, 70
обязательств: 60 PASS, 1 PARTIAL, 3 MISSING, 6 NONCLAIM. Вывод свидетельства, не
смена статуса:

- **Stage 4C — READY TO CLOSE**;
- **Stage 4D — READY TO CLOSE**, с бюджетами `docs/35` §Stage 4, перенесёнными на
  закрытие Stage 4 (как 4B перенёс identity gate);
- **Stage 4 — BLOCKED** отчётом о performance contract
  (`docs/evidence/STAGE4_PERFORMANCE_REPORT.md`): один `region_allocate` и восемь
  передач процессора на завершённый READ против бюджетов 0 и 4 — оба структурны для
  принятого дизайна; бюджеты reference platform — P0, у них нет принятого метода
  измерения. Плюс запись решения по патентному обзору Stage 4 (`docs/24` шаг 8).

**Что НЕ доказано и не заявляется.** Аппаратная DMA-изоляция без IOMMU;
доступность против устройства, которое перестало отвечать; power-loss durability,
`FLUSH`, crash consistency, exactly-once запись; целостность хранимых байт против
локального атакующего с доступом к диску; переход к repository-backed `/system` и
системный commit id — это Stage 5.
...
### 2026-09-05 — Stage 4C-0: у правила liveness появилась вторая половина

Раунд узкий и намеренно бесповеденческий. `SYSTEM_ABI_V1` §6 и ADR-0059
(вариант D) формулируют правило целиком — «ничего не готово к исполнению, что-то
заблокировано, **и ничто маршрутизируемое не может это изменить**», — а нуклеус
реализовывал только первую половину и сам об этом писал в комментарии. ADR-0079
§12 нашёл этот дефект в Stage 4A и назвал его обязательным условием до первого
routed device interrupt. Это **завершение реализации, а не новое решение**: ни
одно принятое положение не изменено.

Проведён аудит всех причин блокировки. Их четыре, множество закрыто (§6 требует,
чтобы ждали всегда на handle, который процесс держит), и каждая классифицирована
по тому, что должно произойти, чтобы ожидание закончилось:

| Причина | Что её завершает | Источник пробуждения |
|---|---|---|
| `Message(endpoint)` | `endpoint_send` / `endpoint_call` другого контекста | peer |
| `Room(endpoint)` | `endpoint_receive` другого контекста | peer |
| `Reply` | `endpoint_reply` / `endpoint_reply_receive` | peer |
| `ChildOf(instance)` | завершение потомка — его выход, его сбой или `process_terminate` | peer |

Все четыре — peer, поэтому перепись возвращает ноль маршрутизируемых источников
на любой загрузке, которую эта система сегодня может произвести, и правило
срабатывает ровно там же, где срабатывало. Это и есть проверяемое свойство
раунда: классификация меняется, поведение — нет.

Реализовано: `Waiting::wake_source()` — исчерпывающий `match` без `_`, так что
новая причина блокировки не может быть добавлена без ответа компилятору;
`Liveness` из трёх состояний вместо булева `any_blocked()`;
`Liveness::AwaitingHardware` останавливает машину `sti; hlt` (не крутится и не
делает вывод), причём `sti` обязателен — контекст, завершившийся внутри
системного вызова, возвращается в планировщик с замаскированными прерываниями;
событие `TOS.RUN.LIVENESS blocked= routed= verdict=`, добавленное в
`RUNTIME_OBSERVABILITY_V1` §7.

`reason=no-runnable-context` на `TOS.RUN.BLOCK_CANCELLED` **не** переименован:
это объявленное значение принятого контракта, а перепись — утверждение о другом
предмете (о системе, а не об одном ожидании), поэтому у неё своё событие.

Гейт *QEMU blocking and the liveness rule* усилен: обе строки переписи
проверяются точно (`blocked=1 routed=0 verdict=stalled`, ровно две), и
проверяется, что `verdict=awaiting-hardware` не появляется на стадии, которая не
маршрутизирует ни одного device interrupt. `routed=0` рядом с `verdict=stalled`
— это наблюдаемая разница между нуклеусом, который вычислил вторую половину
правила, и нуклеусом, который её не спрашивал.

Названное ограничение, записанное сейчас, а не найденное потом: как только у
какого-то ожидания появится живой маршрутизируемый источник, peer-deadlock рядом
с ним перестанет диагностироваться. Это тот же класс, что уже названная
ADR-0059 частичная starvation, и ограничен он тем же — маршрутизируемый источник
это **полномочие**, а не окружающее условие: он умирает вместе с процессом,
вместе с assignment'ом устройства и вместе с любым отзывом со стороны launcher'а.

Свидетельство: `docs/evidence/STAGE4C_LIVENESS.md`. Проверено:
`./scripts/preflight.sh` — PASS 38/38; `./scripts/preflight.sh --profile qemu` —
PASS.

### 2026-09-06 — ADR-0083 принят: метрика Stage 1 мерила компоновщик

Stage 4C-1 упёрся в performance STOP: ownership-репарации ADR-0082 сдвинули
принятое отношение ADR-0026 с ~1,11 до 2,09–2,27 при бюджете 1,30 — в нагрузке,
которая доказуемо не исполняет ни одной новой строки. Правильный вопрос был не
«как это починить», а **«измеряет ли это отношение то, что заявляет»**. Не
измеряло.

Контролируемый эксперимент, а не рассуждение: **инертное смещение раскладки** —
не исполняющее ничего и не меняющее длину образа — двигает отношение с ~1,11 до
1,546, через границу conformance, при неподвижном native (0,999). Дописывание
инертных байтов, которое *не* смещает горячий путь, не воспроизводит эффект ни
на одном размере от 64 до 4096. Объясняющий механизм — граница гостевой
страницы 4 KiB внутри `Sha256::compress_block`, которую TCG translation block
пересекать не может, — записан, но нормативной зависимостью не является:
несостоятельность конструкции следует из самого эксперимента.

**Два независимых структурных дефекта старой метрики.** Первый: числитель
измерялся в production-нуклеусе, знаменатель — в **отдельно скомпонованном**
`test-crypto-baseline`-нуклеусе. Две компоновки — значит, чувствительность
эмулятора к раскладке не общая, а значит, в частном она не сокращается, хотя
именно ради сокращения общего частное и берут. Второй, найденный уже при
постройке ремонта: интервалы не начинались в одной точке — числитель нёс всю
фазу UEFI-загрузчика, около 1370 мс из ~2740, выполненную **другим бинарником**,
которого эта метрика не компонует и не варьирует.

**Ремонт — один артефакт, два режима, выбираемых во время исполнения** через
firmware-configuration QEMU. Раскладка, адреса кода, размещение статических
данных и среда трансляции TCG общие для числителя и знаменателя и сокращаются;
репортёр **отказывается** считать частное, если SHA-256 двух образов не равны.

Третий дефект был найден Project Architect'ом уже в первой редакции ремонта и
стоит отдельной записи: числитель мерил **один** проход валидации против
знаменателя, моделирующего **два**. Отношение 0,35 было арифметикой этого
рассогласования, а не утверждением о структурных издержках, и предложенный из
него порог `<= 0,40` закодировал бы ошибку в контракт. Исправлено: `FULL_EXACT`
— это теперь та же двухвалидаторная нагрузка, что и в native-эталоне, а
знаменатель **не сужен** (`101203397` байт, `2007` вызовов — принятая модель без
изменений). Центр отношения переехал с 0,35 на 1,00.

**Свойство разумности выполняется, и деревья снова неразличимы:**

| | p95 | median | native |
|---|---|---|---|
| pooled mean | 1,0076 | 0,9977 | 0,9988 |
| min–max | 0,9549–1,0746 | 0,9826–1,0109 | 0,9900–1,0146 |

Разница между деревьями — **0,0075** при pooled stdev 0,0401. То самое
возмущение раскладки, которое старая метрика превращала в 1,9×.

**Принято 2026-09-06**: одна блокирующая линия `p95 <= 1,30`, конформной
статистикой остаётся nearest-rank p95, median удерживается рядом как
диагностика. Предложенная в свидетельстве вторая абсолютная линия `<= 1,15`
**отклонена**: она смешивала абсолютные структурные издержки, которыми ведает
бюджет 1,30, с регрессией относительно удержанного baseline, которой ведает уже
существующая политика >15 % / >30 % — и применяется та политика к **baseline**,
а не к математической единице 1,0.

**Совпадение двух `1,30` — именно совпадение.** Старое число ограничивало
частное двух артефактов на двух несопоставимых интервалах, где оно не значило
ничего проверяемого; новое выведено из приведённого распределения и стоит на
21 % выше худшей из шести серий.

Переход гейта выполнен атомарно: `stage1-paired-conformance.sh` — активная
конформность Stage 1; `stage1-performance-conformance.sh` переименован в
`stage1-performance-historical.sh`, продолжает мерить все три серии, продолжает
писать частное и **больше на нём не падает** — и проверяет, что отчёт не несёт
порога, потому что отчёт с порогом заявлял бы конформность, которой этот скрипт
уже не выполняет. Удаление того, что упало, — не способ сделать стадию зелёной.

Названное ограничение, записанное там, где его нельзя не заметить: на текущей
фикстуре 1000 файлов / 16 MiB метрика **криптодоминирована**. Она обнаруживает
катастрофические издержки архитектуры валидации и **не разрешает** малый
структурный дрейф — в двух сериях из шести числитель оказался ниже знаменателя.
Фикстура, смещающая работу с хеширования, — отдельная позиция будущего
исследования, а не контрабанда внутрь этого ADR.

Проверено на `b95bcd2`: `./scripts/preflight.sh --full` — 87/89, паре гейтов
Stage 3 не хватает локально собранного наблюдателя ADR-0066 (в CI он строится);
`STAGE1-PAIRED-CONFORMANCE PASS: p95_ratio=0,968 <= 1,30`, median 0,998,
отклонение −3,9 % от baseline; `STAGE1-PERFORMANCE-HISTORICAL PASS:
qemu_p95_ratio=1,183 (recorded, not asserted)`.

### 2026-09-07 — Stage 4C-1b: драйвер ждёт своё устройство, и устройство его будит

Самая маленькая недостижимая операция ADR-0082 §1 — достигнута:

```text
TOS.RUN.IRQ_SOURCE process=0 bus=0 device=4 function=0 entry=0 transport=msix
TOS.RUN.LIVENESS   blocked=1 routed=1 verdict=awaiting-hardware
TOS.RUN.IRQ_DELIVERED source=0 entry=0 vector=48 deliveries=1 woke=1
TOS.RUN.COMPLETED  value=i64:7
```

Канонический процесс на TOS Core, держащий одну bus-способность, забирает одну
функцию, выводит из **этого assignment'а** одно прерывание этой функции,
сообщает устройству — через окно, которое само же и отобразило, — какой из его
MSI-X entries использовать, и блокируется. Перепись планировщика впервые в
истории системы отвечает `routed=1`: машина останавливается, а не отменяет
ожидание. Затем реальное изменение конфигурации реального `virtio-blk-pci`
поднимает реальное MSI-X сообщение; оно приходит на вектор, который нуклеус
записал в таблицу; обработчик находит entry 0 функции 00:04.0 и будит
единственный контекст, ожидающий на этом источнике.

**Число — не полномочие, и механизм в том, что его негде назвать.** Ни одна
операция ни одного контракта не принимает вектор, GSI, legacy IRQ, пару
address/data или BDF, и ни одно из этого не появляется ни в результате, ни в
описании объекта. Единственное число, которое принимает `pci_interrupt_claim`, —
индекс entry MSI-X-таблицы **той функции, которую способность вызывающего уже
называет**: тот же класс аргумента, что индекс BAR у операции 27. Поэтому два из
негативов §13 не отклоняются, а **невыразимы** — их нельзя написать, и checker
отвергает их раньше верификатора.

**Что узнал ring 0 и чего не узнал.** Раскладку entry MSI-X-таблицы, как выделить
вектор CPU, как подтвердить через local APIC и что источник — это entry N
функции. Не что такое очередь, не что entry 0 у этого устройства —
конфигурационный вектор, не что это блочное устройство. Это модуль сообщил
**устройству**, через свои же регистры. Гейт вырезает комментарии из исходников
ring 0 и ищет `virtio`, `virtqueue`, `device_status`, `driver_ok`,
`feature_select`, `blk` — не находит ничего.

**Порядок создания и разрушения — §5e и §5f, и каждый шаг заслужен.** Entry
маскируется до записи сообщения и читается обратно после, потому что
конфигурационное чтение не является барьером для отложенной MMIO-записи; источник
публикуется до снятия маски, чтобы доставленное сообщение никогда не нашло
обработчик без источника; гейты IDT на весь диапазон устройств существуют с
загрузки — именно это делает позднее сообщение на **списанном** векторе
приземляющимся в определённое место. Единственное исключение §5b используется
ровно один раз и ровно как написано.

**Вектор списывается, а не переиспользуется** (§5f). Аллокатор только считает
вверх; шестнадцать векторов, исчерпание — `E_LIMIT`. Свидетельство забирает одну
entry, освобождает её и забирает **ту же** entry той же функции снова: векторы
разные, чего переиспользующий аллокатор дать не может.

**Оба предиката enable двигаются для этого потомка, и только для него.**
`MmioRegion` включает декодирование памяти и не трогает bus mastering — это уже
доказано `virtio-mmio.sh`; источник включает оба, потому что его таблица в BAR, а
его сообщение — запись в память, которую инициирует устройство. Это и делает §5b
и §5d двумя правилами, а не двумя написаниями одного.

**Страница MSI-X-таблицы принадлежит ring 0 во всех адресных пространствах,
которые он исполняет** (§4). Иначе нельзя: таблица программируется из
пространства драйвера при захвате источника и из собственного пространства
нуклеуса при подметании источников умершего процесса. Зарегистрированные
страницы отображаются в существующие пространства и подхватываются
`paging::fill` для построенных позже; supervisor-only, uncacheable,
identity-mapped, ограничены `MAX_ASSIGNMENTS` и учтены в `table_reserve` — чтобы
граница не обнаруживалась процессом, который не смог запуститься.

**Триггер — внешнее событие устройства, а не подставленный ответ.** Изменение
конфигурации — это то, что внешний мир делает с блочным устройством, и это
единственное событие, которое это устройство может сообщить без очереди,
дескриптора и DMA (всё это — Stage 4C-2 и дальше). Стенд меняет размер диска
только после того, как **собственный планировщик гостя** объявил, что он
заблокирован, ничего не пишет в гостя и не даёт ему ни одного значения для
чтения.

Одно отклонение от §13 записано, а не обойдено: «source capability без `wait` не
может ждать» **невозможно предъявить** — у объекта ровно одно право, а
`CAPABILITY_V1` §4 отказывает в аттенюации до нуля прав. Диспетчер право всё
равно проверяет; свидетельство показывает, что ни одна программа не может
изготовить имя, которое эту проверку провалит.

Контракты: `PLATFORM_INTERFACE_V1` → **версия 2** (интерфейс
`platform.irq.Source`, операция `pci_interrupt_claim`, права `interrupt` и
`wait`); `SYSTEM_ABI_V1` — операции 28 и 29 и абзац §6 о первом ожидании, чей
источник пробуждения не контекст; `RUNTIME_OBSERVABILITY_V1` — четыре события;
`docs/34` — формулировка §5 ADR-0082 про отсутствие IOMMU, помещённая в S5, плюс
два пути эксплуатации (X4.1, X4.2).

Проверено на `6663fec`: `--profile docs` 12/12, `--profile source` 11/11,
`--profile provenance` 3/3, `--profile selftest` 14/14, `--profile qemu` 49/51 —
двум гейтам Stage 3 не хватает локально собранного наблюдателя ADR-0066, который
CI строит сам.

Stage 4C-2 не начат: ни DMA authority, ни DMA allocation, ни device-visible
addressing, ни IOMMU, ни порядок MMIO ↔ DMA.

### 2026-09-12 — Stage 4D-3: одна очередь переживает запрос и обслуживает второй

Stage 4D-2 доказал, что устройство выполнило DMA через очередь, построенную
каноническим текстом, — **один раз**, и каждый индекс кольца был в нём
константой: голова цепочки `const CHAIN_HEAD = 0`, слот available-кольца 0,
`avail.idx` записан литералом `1`, used-запись прочитана из слота 0. Аудит
перед реализацией зафиксировал это в пяти независимых местах и отдельно
зафиксировал, что универсальные помощники (`align_up`, `scale`, `put_le*`,
`read_le*`, `add_status`, `negotiate`, `usable_queue`, `window_offset`,
`multiplier_admitted`) односторонних допущений не несут.

Построено: `tests/vectors/virtio-block-reuse/init.tos` — **два последовательных
настоящих `VIRTIO_BLK_T_IN` через одну инициализированную split-очередь**. Одна
инициализация устройства, одно согласование фич, один `queue_enable`, один
`TOS.RUN.DMA_REGION`, один `TOS.RUN.PCI_ASSIGNED`, один MSI-X источник — на оба
запроса.

Пул дескрипторов: **четыре записи таблицы из 128, по три на запрос**. Второй
запрос арифметически невозможен без возврата дескрипторов первого — это
обеспечено размером пула, а не утверждением в комментарии. Свободный список —
циклический FIFO, а не стек, поэтому вторая цепочка это `3 -> 0 -> 1`: голова,
которой первый запрос не касался, и два дескриптора, которые он использовал.
Жизненный цикл явный — FREE → PREPARED → DEVICE → COMPLETED → FREE, состояние в
приватной памяти драйвера, а не в поле `next` таблицы дескрипторов; выдаётся
только FREE, публикуется только PREPARED, возвращается только DEVICE. Пути из
DEVICE обратно в цепочку нет.

Индексы больше не константы: `avail.ring[avail_idx % qsz]`,
`used.ring[last_seen_used % qsz]`, оба счётчика переживают запрос. Модулей два и
они разные — счётчики переполняются на 65536 (§2.7.13.3), кольца индексируются
по размеру очереди. Арифметика проверяется через границу `65535 -> 0` до первого
обращения к устройству (девять утверждений, отказ `-20`), потому что загрузка на
два запроса до 65536 не доходит, а строить загрузку на 65 536 запросов ради
одного переполнения — худший тест, чем проверка примитива на границе.

Образ диска профиля Stage 4 засевается различимым содержимым: сектор `n` — 512
копий байта `0xC0 + n` для секторов 1..4, **сектор 0 остаётся нулевым**, так что
свидетельство 4D-2 не тронуто. Оба запроса читают в **один и тот же** буфер,
отравленный **разным** сентинелом перед каждым: ни нетронутая память (0x00), ни
отрава (0xA5 / 0x5A), ни результат предыдущего запроса (0xC1) не могут
удовлетворить свидетельство второго.

`TOS.RUN.COMPLETED value=i64:565994011492351` — маска из 30 фактов полностью
(1073741823) плюс шесть измерений: `avail.idx` 2, потребитель 2, головы 0 и 3,
переиспользовано 2 из 3, очередь 128. Нуклеус со своей стороны записал **две**
доставки MSI-X на одном источнике, обе разбудившие ожидающего.

Негативы. Против судейства гейта: шесть подготовленных значений — правильная
форма, переинициализированная между запросами очередь, прогон без
переиспользования, вторая цепочка, совпавшая с первой, прогон с несовпавшими
данными и отказ модуля; первое обязано приниматься, остальные — отвергаться.
Против модуля, на настоящем устройстве: оба запроса читают сектор 1 → `-70`;
ожидаемый байт сдвинут на единицу → `-62`; слот available-кольца прибит к 0, как
в 4D-2 → второй запрос до устройства не доходит и загрузка не завершается; used
всегда читается из слота 0 → `-58`. Мутации откачены.

Нуклеус не получил ничего: ни ABI-операции, ни версии интерфейса, ни minor'а
языка, ни полномочия, ни механизма, ни знания об устройстве. Endowment —
`test-dma-driver`, без изменений. Хост засевает содержимое диска и читает
события; управление очередью, переработка дескрипторов, арифметика индексов и
обнаружение завершения — только в каноническом тексте.

Найдено попутно и **не** чинилось в этом срезе (заведено отдельно):
`borrow mut` параметр `Copy`-типа передаётся движком копией, и записи вызываемой
стороны теряются молча, без диагностики; а парсер требует литерал в длине
`array<T, N>` (`E1104_EXPECTED_LITERAL`), хотя `docs/40` §3 прямо говорит, что
именованная константа там допустима. Ни то, ни другое не блокирует срез:
состояние пула и счётчиков живёт в одной области видимости, а длины массивов
записаны литералами с объяснением почему.

Свидетельство — `docs/evidence/STAGE4D3_QUEUE_REUSE.md`.

**Не доказано и не заявляется:** несколько запросов одновременно, запись,
мультиплексирование очередей, косвенные дескрипторы, event index, packed
virtqueue, reset/resize очереди, планирование, кэш, файловая система, драйверный
фреймворк, клиентский блочный интерфейс. Формального closure approval у Stage 4C
и Stage 4D нет.

### 2026-09-12 — два дефекта TOS Core V1, найденные Stage 4D-3, и их починка

Срез языковой корректности, не аппаратный. Stage 4D-3 закрыт на
`cc17253`; этот раунд ничего в нём не переписывает и ничего нового про железо
не заявляет. Оба дефекта были найдены при постройке 4D-3, записаны в его
свидетельстве и отложены — здесь они закрыты.

**Дефект A — `borrow mut` не сохранял заимствованное место.**

`Op::Borrow` порождает *значение*, а значение здесь — копия прочитанного.
Операнд вызова `f(borrow mut x)` — это слот этой копии, и план обратной записи
движка целился именно в него: `writeback_plan` возвращал пары
`(слот параметра вызываемого, слот операнда вызывающего)`. Записи вызываемой
стороны попадали во временную копию и исчезали молча — ни ловушки, ни
диагностики.

Затронуто **не только** `Copy`-агрегаты, а всё, чьё runtime-значение несёт сами
данные: скаляры, массивы, записи, вложенные места (`p.a`, `p[i]`, включая
вычисляемый индекс). Объекты за хэндлом — регионы, MMIO, capability — выглядели
исправными только потому, что копия хэндла достаёт тот же внешний объект.

Затронутые формы вызова: локальный вызов; межмодульный вызов; вызов значения
функции (замыкание) — там вдобавок lowering принудительно ставил
`PassMode::Owned` всем объявленным параметрам замыкания, то есть `borrow mut`
в списке параметров замыкания молча означал не то, что написано. Цепочка
`defer` была исправна: ADR-0035 передаёт cleanup'у **сами привязки** области, а
не заимствования их, поэтому там операнд и был слотом места.

Починка — на слое представления, без изменения схемы IR. `Op::Borrow { place,
kind }` уже называет место точно; чего не хватало, так это переноса этой
идентичности через границу вызова. Кадр получил `borrows: BTreeMap<usize,
BorrowedPlace>`: при вычислении **мутабельного** заимствования туда кладётся
место с уже разрешёнными динамическими индексами и вид элемента для регионного
доступа. План обратной записи стал `(позиция, место)`, а сама запись идёт через
то же место — или через `System::access`, если корень места регион. Всё
владеемое и ограниченное: ни одной host-ссылки, ни одного обращения в модуль,
поэтому обратная запись переживает вытеснение вызывающего модуля ровно так же,
как остальной кадр (ADR-0071 §6). Разделяемое заимствование не записывает
ничего и записей не оставляет. Параметры замыкания теперь лоуэрятся со своим
объявленным режимом.

**Схема IR не менялась и не требовала изменения**: `Op::Borrow` уже несёт
`Place`, `Parameter` уже несёт `PassMode`, `tos-ir/v1` не тронут. Дайджест
модуля Stage 4D-2 до и после починки один и тот же —
`sha256:21101a2d...bcbcb3`.

**Дефект B — именованная константа в `array<T, N>`.**

docs/39 пишет продукцию прямо: `array_type = "array" "<" type ","
const_expression ">"`, `const_primary = integer | size | identifier | "("
const_expression ")"`. ADR-0052 строит на этом весь свой аргумент
(«V1 не даёт пользовательских дженериков, поэтому имя в этой позиции — это
именованная константа и ничто иное»), docs/40 §2 говорит то же со своей
стороны. Парсер же вызывал `expect_literal()` и отвечал
`E1104_EXPECTED_LITERAL` на имя.

Починено ровно до того, что допускает принятая грамматика: разобрана
`const_expression` (`const_sum` / `const_product` / `const_primary`), а lowering
вычисляет её во время компиляции — литералы `integer` и `size` с единицами
`B`/`KiB`/`MiB`/`GiB`, именованные константы модуля с подстановкой и защитой от
цикла, пять операторов с проверяемой арифметикой, скобки. Длина попадает в
артефакт **числом**: `array<T, COUNT>` и `array<T, 4>` при одном значении — один
`TypeDef::Array(element, 4)`, поэтому переименование константы не двигает ни
таблицу типов, ни дайджест модуля.

Токен, не начинающий ни одну из четырёх альтернатив, теперь
`E1107_UNEXPECTED_TOKEN` вместо `E1104`: у `array<i64, "four">` литерал как раз
есть, и он всё равно не `const_primary`. `E1104` остаётся тем, чем был, — для
пределов в `resource [...]`, где литерал действительно требуется. docs/44 §7 не
менялся, потому что описания обоих кодов уже верны для позиций, где они теперь
выдаются.

**Почему оба дефекта пережили набор тестов.** Для `borrow mut` end-to-end
проверка была ровно одна —
`residency_execution.rs::a_mutable_borrow_writes_back_into_a_caller_that_was_evicted`
— и она передаёт аргумент голым именем (`mid.touch(cell)`), которое лоуэрится в
слот самой привязки; обратная запись попадала куда надо по причине, не имеющей
отношения к заимствованию. Все прочие тесты `borrow` проверяли диагностику
(статическое принятие/отказ), а не видимую вызывающему мутацию. Для длины
массива контрактные гейты проверяют, что документы *говорят* нужное, и ни одна
фикстура языкового корпуса не писала именованную константу в позицию длины —
правило, записанное в трёх принятых документах и не скомпилированное ни разу.

Тесты добавлены на том слое, где дефекты и проскочили:
`tests/integration/tests/mutable_borrow.rs` (19 случаев) и
`tests/integration/tests/array_length_constants.rs` (19 случаев), плюс два
межмодульных случая в `residency_execution.rs`. На непочиненном дереве падают
14 из 19 и 18 из 19 соответственно; проходят там ровно те, что про уже
работавшее поведение — диагностику разделяемого заимствования, исключительность
и обычный `defer`.

**Найдено и НЕ чинилось (напряжение между принятыми документами).** docs/40 §2
описывает инициализатор константы как допускающий «`const_expression`, чей
`identifier` называет другую константу, **этого модуля или импортированного**»,
а продукция `const_primary` из docs/39 точечной формы не имеет вовсе — так что
`other.COUNT` не является `const_expression`. Вдобавок импортированные
константы не реализованы **нигде**: `leaf.COUNT` в обычном выражении даёт
`Gap { construct: "unbound place" }`. Это не дефект длины массива и не
разрешается парсером; по AGENTS §2 конфликт зафиксирован, а не улажен молча.
Негативный тест `a_dotted_name_is_not_a_const_expression` фиксирует то, что
принятая грамматика допускает сегодня.

**Дефект C — обратная запись через callable-значение могла записать значение
чужого типа.** Найден финальным аудитом перед коммитом и **внесён этим же
срезом**: до него `Op::CallValue` не делал обратной записи вовсе, поэтому
дефекта не было.

Тип callable-значения не несёт режима параметра — и это **контракт**, а не
реализация: docs/39 даёт `function_type = "fn" "(" type_list? ")" "->" type`, а
`tos-ir/v1::TypeDef::Function(Vec<TypeId>, TypeId)` отражает это точно. Owned,
`borrow` и `borrow mut` в типе callable неразличимы на всех слоях — источник,
checker, verifier. Вдобавок вызов через значение сегодня **вообще** не
проверяется против этого типа: ни арность, ни типы аргументов (измерено;
у именованного вызова типы аргументов проверяются, `E1215_ARGUMENT_TYPE_MISMATCH`,
а вот арность — нет, и недостающий параметр отлавливается только ловушкой
`RUNTIME_UNINITIALIZED_VALUE` во время исполнения).

Отсюда: замыкание с `borrow mut v: i32` достижимо через привязку типа
`fn (i64) -> unit`, и его обратная запись клала `i32` в место типа `i64`.
Измерено до починки: `main -> i64` возвращала `Int(I32, 8)` — граф значений
оставался с величиной, тип которой никто не объявлял, без ловушки и без
диагностики.

Починено там, где дыра и открылась: перед построением плана обратной записи для
вызова в пределах одного модуля движок требует, чтобы объявленный тип параметра
`borrow mut` совпадал с типом места у вызывающего (оба `TypeId` одной таблицы —
вызов через значение всегда входит в тело собственного модуля, потому что
`Op::Closure` называет локальную функцию). Несовпадение — `RUNTIME_TYPE_CONFUSION`,
а не тихая запись. Для именованных вызовов та же проверка лишь подтверждает уже
доказанное фронтендом.

**Не чинилось и вынесено Архитектору:** сама неразличимость режимов в типе
callable. Закрыть её — значит внести режимы в `function_type` и в
`tos-ir/v1::TypeDef::Function`, то есть изменение принятого языка и схемы IR
уровня ADR; в срез языковой корректности такое не протаскивают. То же про
непроверяемые арность и типы аргументов у вызова через значение: `function_type`
типы несёт, так что это уже реализуемый в checker'е пробел, но он шире этого
среза. Поведение зафиксировано тестами (`a_callable_value_carries_no_parameter_mode`,
`a_callable_value_keeps_its_body_s_modes_wherever_it_is_carried`,
`a_value_call_may_not_write_back_through_a_mismatched_type`), чтобы оно было
записанным свойством, а не тем, что кто-то потом обнаружит в поведении.

**Граница и время жизни `Frame::borrows`.** Таблица ключуется слотом, который
определяет инструкция заимствования, а слоты функции фиксируются при lowering'е
— поэтому заимствование в цикле переписывает одну запись сколько угодно раз, и
вся таблица ограничена своей активацией (`debug_assert!` рядом со вставкой это
сторожит). Устаревшая идентичность недостижима: прочитать запись можно только
передав ровно то значение заимствования операндом, а V1-заимствование не
переживает оператор, который его взял. Проверено программами: 2000 итераций
одного места; цикл по вычисляемому индексу даёт `1234`, то есть каждая итерация
писала в свой элемент; вызовы после закончившегося заимствования — и в форме
`borrow mut`, и в голой — попадают каждый в своё место.

### Остаётся нерешённым после этого среза

**1. Тип callable не кодирует `PassMode` — контракт языка и IR, а не баг движка.**

```text
source   fn(A, B) -> R
IR       TypeDef::Function(Vec<TypeId>, TypeId)
```

Ни то, ни другое не несёт режима параметра. Поэтому callable-значения, тела
которых различаются **только** тем, что параметр
owned / `borrow` / `borrow mut`, статически неразличимы — при том что их
семантика владения и обратной записи разная. Закрытие требует архитектурного
решения и, вероятно, версионированного изменения источника и IR. Здесь такое
решение неявно не принимается. Единственное, что сделано, — исполнительная
защита от тихой путаницы типов (дефект C выше); дальше движок эту дыру не
«решает».
**Stage 4D-4 заблокирован до решения по этому пункту.**

**2. Статическая валидация вызовов неполна — пробел реализации.**

Принятый контракт IR/verifier уже требует точных операндов вызова и их типов.
Реализация сегодня пропускает:

- `CallValue` с аргументом неверного типа;
- `CallValue` со слишком малым числом аргументов;
- `CallValue` со слишком большим числом аргументов;
- именованный/локальный вызов с неверной арностью.

Для случая неверного типа нормативная диагностика уже есть —
`E1215_ARGUMENT_TYPE_MISMATCH`. Для неверной арности код во фронтенде принятым
реестром (`docs/44` §7) внятно не определён, поэтому здесь он **не
выдумывается**. Verifier обязан со временем отвергать некорректную арность и
типы операндов вызова независимо от фронтенда; это работа следующего среза
архитектуры/корректности, а не этого.

Проверено: host-набор зелёный целиком; `--profile source` 12/12;
`--profile docs` 14/14; `--profile provenance` 3/3; `--profile selftest` 14/14;
`--profile qemu` 53 pass / 2 fail — те же два environment-only гейта ADR-0066.
Stage 4D-1, 4D-2 и 4D-3 перезапущены **без изменений** и проходят; свидетельство
4D-3 побайтно то же самое: `TOS.RUN.COMPLETED value=i64:565994011492351`,
`fuel=60032`, две доставки MSI-X, головы 0 и 3, переиспользовано 2 из 3.
Поведение железа от языковой починки не изменилось ни в одном бите.

### 2026-09-13 — граница вызова: точный список операндов, который контракт требовал всегда

Срез корректности вызовов. Stage 4D-4 не начат и остаётся **заблокированным**.

**Что требовал принятый контракт.** docs/43 §4: «A call names a declared
imported or local function signature and supplies an **exact ordered operand
list**», и про верификатор: «the verifier rechecks … **operand types,
call/effect signatures**». Реализация этого не делала: `Op::CallValue`
устанавливал лишь, что операнд-вызываемое имеет функциональный тип, а
`CallTarget::Local` — лишь что индекс функции в пределах таблицы. Ни арности, ни
типов операндов, ни объявленного результата. Это давало испорченный граф
значений **без всякого `borrow mut`**: слот, статически объявленный одним типом,
получал runtime-`Value` другого, и все последующие проверенные инструкции
читали его под ложным статическим допущением.

**Добавлено в верификатор** (именованный локальный вызов и вызов значения):
точное число операндов (`V2011_CFG` — код, которым этот файл уже отвечает на
«given a different number of operands than it declares»), тип каждого операнда
против объявленного параметра и объявленный результат против результата
вызываемого (`V2010_TYPE`). Режимы параметров **не** сравниваются: тип callable
не несёт `PassMode`, и эта стирающая особенность — отдельный вопрос контракта,
а не то, что здесь решается по умолчанию.

**Починки фронтенда, которые вскрыла эта проверка.** Все четыре — случаи, когда
*корректный* канонический исходник лоуэрился в IR с неверными типами:

- вызов `async fn` объявлял написанный `T` вместо `Task<T>` из сигнатуры
  вызываемого: слот результата говорил `i64`, а получал хэндл задачи;
- литерал массива из констант брал тип элемента из результата объемлющей
  функции: `[0u64, 0u64, 0u64]` внутри функции, возвращающей `i64`, давал
  `array<i64, 3>` и инициализировал место, объявленное `array<u64, 3>`;
- результат бинарного и унарного выражения, построенного из константного
  операнда, брал ту же подстановку — это тот самый случай `u64`-в-`i64`,
  из-за которого сдвинулся дайджест Stage 4D-2;
- замыкание или тело `spawn`, возвращающее захваченное имя, откатывалось к
  `unit`: `Task<Message>` лоуэрился как `Task<unit>`.

Корень всех четырёх один: `BodyBuilder::type_of` не может определить тип
операнда, называющего таблицу констант, и подставлял результат объемлющей
функции.

**Дайджест Stage 4D-2 сдвинулся, и это не регрессия.** Исходник побайтно тот же
(`sha256:40abe34d…5177b`); сдвинулся *производный* артефакт:

```text
до     sha256:21101a2d3ba05a1c6ac99a5f6347c2a920c49b9f9fed9b164a064b6745bcbcb3
после  sha256:c987d33d2849eee3842bfad5ee08285270bd255a7dee281e83d7785cd029c2a2
```

Старый дайджест называл таблицу типов, которая не говорила того, что
обозначает исходник. Сохранить его значило бы сохранить дефект. Аппаратная
семантика не изменилась ничем: свидетельство Stage 4D-3 побайтно то же —
`TOS.RUN.COMPLETED value=i64:565994011492351`, `fuel=60032`, две доставки MSI-X,
головы 0 и 3, переиспользовано 2 из 3. Подробности — в датированном приложении
к `docs/evidence/STAGE4D2_FIRST_BLOCK_READ.md`.

Тесты — `tests/integration/tests/call_boundary.rs`, 19 случаев: из исходника
(корректные вызовы, мало/много операндов локально и через значение, неверный тип
операнда, результат вызова значения против функционального типа, плюс три
сторожа для починок фронтенда) и **из подделанного IR** (у локального вызова и
вызова значения по очереди убран операнд, продублирован операнд, подменён тип
операнда и подменён тип результата). На непочиненном дереве падают 16 из 19;
если оставить верификатор и убрать только починки фронтенда, падают ровно три
сторожа.

### Остаётся нерешённым после этого среза

**1. Оракул типов в lowering'е неисправен шире починенных мест — блокирующий
дефект типовой надёжности Core, не вопрос Stage 4D.** `BodyBuilder::type_of` и
соседние пути пользовались запасным типом вместо точного типа каждого операнда.
Аудит доказал последствия сверх починенных: `Read` / `Move` / `Borrow` могут
переклеить месту тип, отличный от фактического типа проекции места (проверка
этого написана и **снята** с этого коммита: она отвергает Stage 4D-1/2/3 и два
модуля принятого корпуса, пока типизация фронтенда не починена шире); явные
аннотации `let` проверяются недостаточно — `let x: bool = 1i64;` принимается и
падает только в исполнении. Значит, некорректно пониженный или подделанный IR
по-прежнему может создать граф значений, расходящийся с объявленными типами.
Отдельные оставшиеся примеры в этом коммите **не** латались.

**2. Представление констант и контекстно типизируемых литералов не решено.**
`Op::Const` нельзя просто обязать совпадать с видом записи в таблице констант:
беcсуффиксные литералы типизируются контекстом (`E1210`: «an unsuffixed literal
takes the required type instead»). Проверка была написана и снята — она
отвергает `parallel-work.tos` и `bootstrap-parallel.tos`. Принятую семантику и
представление нужно проаудировать до изменения правила; здесь представление
неявно не выбирается.

**3. Сигнатуры импортированных вызовов недоступны.** `LoweringInterface` не
несёт упорядоченных типов параметров экспортов; `ResolutionSnapshot` не несёт
сигнатур вызываемых вовсе. Поэтому импортированный вызов не может проверить на
точную арность и типы ни фронтенд, ни независимый верификатор. Измерено:
`leaf.add(true, 2i32)`, `leaf.add(1i32)` и `leaf.add(1i32, 2i32, 3i32)`
принимаются сквозь весь путь. Это пробел представления/интерфейса, который
проектируется отдельно.

**4. Типизированные сигнатуры предопределённых операций верификатору
недоступны.** Верификатор не знает о них ничего — даже множества имён, — а
фронтенд пропускает некорректные предопределённые вызовы, которые падают или
усекаются только в исполнении (`to_u8(1u64, 2u64)` молча теряет лишний аргумент;
`to_u8()` и `to_u8(true)` дают ловушку). `E1215_ARGUMENT_TYPE_MISMATCH` по своей
формулировке уже покрывает «a resolved call **or predeclared operation**» для
несовпадения типа, а вот для неверной арности вызова принятого исходного кода
диагностики нет, и он здесь **не выдумывается**.

**5. Стирание `PassMode` у типа callable не решено.** Синтаксис функционального
типа и `TypeDef::Function` не менялись. Owned / shared-borrow / mutable-borrow
параметры остаются статически неразличимыми и требуют отдельного
архитектурного решения.

### Что дальше

Следующий срез — **не** Stage 4D-4 и **не** ADR про `PassMode`. Это аудит и
починка более фундаментального инварианта: у каждого пониженного и проверенного
операнда, значения и места ровно один точный тип, и никакой запасной или
контекстный сокращённый путь не может привести к расхождению графа
runtime-значений с проверенным графом типов. Тот срез должен решить правильный
оракул типов для констант, мест, аннотаций, элементов агрегатов и контекстных
литералов — до продолжения работ по ABI импортированных, предопределённых и
callable вызовов.

### 2026-09-14 — типизированное значение: у каждого операнда и места ровно один точный тип

Срез корректности Core. Stage 4D-4 не начат и остаётся **заблокированным**.

**Корневая причина, найденная аудитом.** Типов было два, и ни один не был
авторитетным от начала до конца. `Checker::check` возвращает только
диагностику: его срез типизации считает `Type` и **выбрасывает** его, и по
собственному признанию заголовка типизация «намеренно частична» — `Type::Unknown`
согласуется со всем. Затем `lower_module` обходит тот же AST второй раз и делает
**независимый, неполный** вывод типов, порождая `TypeId`. Не имея доступа к
доказанному чекером, он угадывал — и контракт его помощников был «если не знаю
тип, верну какой-нибудь валидный `TypeId`».

Принято решение Архитектора — вариант B: тип едет вместе с тем, что понижено.

**Что сделано.**

*Контекстные литералы.* Беcсуффиксный целый литерал теперь разрешается в тип,
которого требует его позиция, в момент интернирования константы: `take(4)` при
`fn take(v: i64)` даёт `Constant::Int(I64, 4)`, `let x: size = 4;` даёт
`Constant::Size(4)`, `let x = 4;` без ожидания остаётся `i32`. Диапазон
проверяется **после** того, как цель известна — иначе `200` нельзя назвать ни
`u8`, ни `i8`. Флага «этот литерал был беcсуффиксным» в IR нет и не нужно:
после контекстной типизации канонический производный IR просто содержит
разрешённую константу, и это тот факт, который читает верификатор. Схема
`tos-ir/v1` не менялась.

Этим **исправлена вперёд регрессия `1cf4c74`**: проверка операндов вызова
отвергала `take(4)` — корректный исходник. Верификатор не ослаблен: после
старого lowering'а `4` и `4i32` были одним артефактом, и различить контекстный
литерал от неверного явного было нечем. Различие восстановлено там, где оно
делается.

*Оракул типов.* `BodyBuilder::type_of` **удалён**, а не улучшен. `operand_type`
и `place_type` возвращают `Result` и не подставляют ничего: ни результат
объемлющей функции, ни `unit`, ни индекс 0, ни тип контейнера. Устранены все
найденные подстановки: захват, полезная нагрузка варианта, база поля, индекс
поля (`field_index` больше не отвечает «поле 0» на не объявленное имя), аргументы
конструктора типа, корень места при распространении ошибки, тип задачи у `join`,
приёмник и результат атомарной операции, результат вызова значения, константа
`Unit`. Результат тела замыкания и `spawn` различает два случая, которые раньше
были одним: тело без `return` даёт `unit` — это его тип; тело, чей результат
определить нельзя, **прекращает** lowering.

*Места.* `place_type` проецирует точно — привязка, поле записи, элемент кортежа,
массив по константному и по вычисляемому индексу, срез и все четыре семейства
регионов — а шаг, который не применяется, прекращает lowering. Раньше здесь было
`_ => current`: `a[i]` получал тип `a`, поле — тип своей записи.

*Предопределённые операции.* Их результат больше не `unit`: `to_<int>` даёт
`Result<D, ConversionError>`, `wrapping_*` — тип своего операнда. Типизация
конверсии как `unit` делала `match (to_u8(...))` разбором ничего — что и вскрыла
починка проекции мест.

*`E1216_VALUE_TYPE_MISMATCH`* (ADR-0087, Level 2) выделен как остаточная
диагностика точного типа и зарегистрирован в `docs/44` §7 с полями `context`,
`expected`, `actual`. Приоритет соблюдён: `let x: i64 = 1i32;` — это `E1210`,
контекстный литерал под числовой аннотацией — не расхождение вовсе, а
аннотация, не называющая ни одного типа, остаётся за `E1203`. Новый **минор
языка не требуется**: `E1608` управляет новыми *исходными формами*, а здесь не
добавлено ни одной, поэтому версии в заголовках канонических модулей не
менялись. Векторы соответствия: R086–R089 и A014.

*Верификатор* выводит, а не доверяет, для доступа к местам: `Read`, `Move`,
`Borrow` против спроецированного типа, `Write` — против типа своего значения,
плюс четыре негатива из подделанного IR. Одно честное ограничение: полезная
нагрузка варианта — это `Field(0)` в любой ветви, и какая это ветвь, знает
`match`, а не место; это семейство оставлено не выводимым, а не угаданным.

**Исправлены две строки канонического исходника Stage 4** — с санкции
Архитектора и после независимой демонстрации, что нарушает контракт сам
исходник:
...
`region: DmaRegion<mut u8>`, значит `region[...]` — это `u8`, а `docs/40` §3
делает восемь целых типов точными и даёт `as` как единственное расширение.
Дефект был латентным: движок сравнивает величины, а не виды, поэтому проверка
статуса всегда отвечала правильно — не потому, что программа была типокорректна.
Та же фикстура двадцатью семью строками ниже уже писала `region[data_offset] as
u64`, то есть конверсия была задумана и просто пропущена. Дайджесты и хеши — в
датированных приложениях к свидетельствам 4D-2 и 4D-3. **Аппаратная семантика не
изменилась ничем**: свидетельство 4D-3 побайтно то же, `565994011492351`, две
доставки MSI-X, головы 0 и 3, переиспользовано 2 из 3.

### Остаётся нерешённым

1. **Типизация чекера всё ещё частична.** `Type::Unknown` согласуется со всем в
   55 местах, `Type::Array` не несёт длины, номиналы — по имени. Этот срез не
   переписывал `typing.rs` в авторитетный типизированный AST и не обязан был;
   следствие в том, что часть расхождений ловится позже, чем могла бы.
2. **Полезная нагрузка варианта не выводима из места** — верификатор её не
   проверяет.
3. **Граница починки предопределённых операций, точно.** Локальный lowering
   теперь даёт покрытым предопределённым операциям их настоящие типы
   результата — `to_<int>` даёт `Result<D, ConversionError>`, `wrapping_*` даёт
   тип своего операнда. Это **не закрывает** ранее записанный пробел
   «предопределённые сигнатуры / верификатор»: независимый верификатор
   по-прежнему не располагает полным типизированным набором сигнатур
   предопределённых операций — даже множеством имён — и не может проверить ни
   их арность, ни типы операндов, ни объявленный результат. Это отдельный
   блокер и отдельный срез.
4. Сигнатуры импортированных вызовов, стирание `PassMode` у типа callable и
   диагностика неверной арности вызова — всё по-прежнему открыто и в этот срез
   не входило.

**Stage 4D-4 заблокирован.**

### 2026-09-15 — один авторитет на предопределённый вызов, и арность как диагностика

Срез контракта Core. Stage 4D-4 не начат и остаётся **заблокированным**.

**Что было вместо контракта.** `docs/39` §2 фиксирует замкнутое пространство из
двадцати двух предопределённых имён. Четыре компонента встречали такие вызовы, и
каждый знал свою часть: чекер типизировал результат `to_*` и **не смотрел ни на
один аргумент**; шлюз минора держал два рукописных списка имён; lowerer держал
третий; независимый верификатор не держал ничего — `CallTarget::Predeclared(_)
=> {}` был пустой веткой. Измерено на дереве `a25a784`: `to_u8(1u64, 2u64)`,
`to_u8()`, `to_u8(true)`, `wrapping_add(1u64)` и `wrapping_add(1u64, 2i32)`
принимались чекером, lowerer'ом и верификатором. Подделанный артефакт мог
назвать предопределённой операцией любую строку, дать ей любые операнды и
объявить любой результат.

Отдельная иллюстрация того, почему список на компонент — не контракт: обход
мест вызова в шлюзе ADR-0081 читал не тот аксессор и **не совпадал ни с одним
вызовом два минора подряд**. Это не замечали, потому что типовая половина того
же шлюза ловила всякий модуль, который мог держать `MmioRegion`. Всплыло, лишь
когда ADR-0086 добавил операцию без собственного типа.

**Что сделано.**

*Один авторитет — ADR-0088.* Новый крейт `tos-predeclared`: `no_std`, только
данные, без поведения. На каждую из двадцати двух операций — имя, минимальный
минор, имя фичи для `E1608`, точный упорядоченный список правил параметров
(его длина и есть точная арность), правило результата и форма в IR. Правила
**декларативны**: второй операнд `wrapping_add` — «тот же точный целый тип, что
и операнд 0», и одного `TypeId`, который мог бы это сказать, не существует.
Каждый потребитель вычисляет правило над собственным представлением типов и не
верит ответу другого.

*Форма IR — часть контракта.* `share`, восемь устройственных доступов и две
точки упорядочения DMA понижаются в собственные операции, потому что `docs/43`
§3 запрещает прятать доступ к разделяемой памяти за непрозрачным вызовом,
ADR-0081 §8 требует собственной операции для доступа к устройству, а ADR-0086 §4
— для точки упорядочения. Верификатор отвергает `Call`, называющий такую
операцию. Форма несёт также ширину и порядок байт доступа — это тот вопрос,
который верификатор реально задаёт артефакту, и второй таблицы ширин в
верификаторе теперь нет.

*Дайджест контракта, привязанный на этапе компиляции.* `docs/43` §5 разрешает
общую декларативную таблицу, **только если её content digest — вход обоих
компонентов**. Каждый потребитель называет принятые дайджесты по минорам в
собственном исходнике; build-скрипт каждого вычисляет дайджесты той таблицы,
которую эта сборка действительно линкует; `const`-утверждение в исходнике
сравнивает их. Правка таблицы под любым из компонентов ломает **его
компиляцию**, а не его тесты. Поля в `tos-ir/v1` не добавлено,
`VerifiedModuleRecord` не расширен. Проверено обеими сторонами: изменение одной
строки таблицы даёт `error[E0080]` и в `tos-core`, и в `tos-verifier`.

*Три слоя читают авторитет.* Чекер — имя, арность, правило параметра, с
сохранением кодов, которые реестр уже выделяет под вид позиции (`E1211` для
байтового смещения, `E1210` для двух точных целых типов, остаточный `E1215`).
Шлюз минора — один обход на всё пространство имён вместо двух рукописных.
Lowerer — результат и форма из той же таблицы; `predeclared_result` и его
перечисление удалены. Верификатор — шесть обязательств для
`CallTarget::Predeclared`, минор последним по правилу ADR-0085 §17.7.

*`E1217_CALL_ARITY_MISMATCH`* (ADR-0089, Level 2): исходный вызов подаёт не
столько аргументов, сколько объявляет разрешённый вызываемый. Поля `callee`,
`expected`, `actual`, `context`. Определяется **до** любой позиционной
диагностики типа аргумента: при неверном числе аргументов полного позиционного
соответствия нет, и третий аргумент двухпараметрической функции не соответствует
ничему. Реализован для локальных именованных вызовов, вызовов значения и
предопределённых операций; **импортированные именованные вызовы зарегистрированы
как владелец, но не проверяются** — сигнатур импортированных функций на фазе
проверки не существует, и делать вид, что проверка есть, было бы хуже, чем её
отсутствие. Новый минор языка не требуется: исходных форм не добавлено. Векторы
соответствия R090–R094.

*Правило носителя MMIO — ADR-0081 §7a (поправка).* §7 фиксировал форму доступа
и тип смещения и **молчал о типе значения**. Молчание — не отсутствие правила
надолго: фронтенд не проверял там ничего, а смещение или значение, записанные
беcсуффиксным литералом, брали `i32` — умолчание `docs/40` §3, потому что ни
одна позиция не называла им тип. Поправка называет: всякое чтение MMIO в V1
возвращает точный `u64`, всякая запись принимает точный `u64`, смещение
остаётся точным `size`, а ширина и порядок байт транзакции решаются операцией и
её видимой верификатору формой IR, но никогда — целым типом значения-носителя.
Минор не двигается: MMIO — уже фича 1.2. Измеренное следствие — починка:

```text
mmio_write_le_u32(window, 4, 7)   было   Int(I32, 4), Int(I32, 7)
                                  стало  Size(4),     Int(U64, 7)
```

*Закрыт пробел верификатора по MMIO.* ADR-0081 §8 с самого начала утверждал,
что верификатор самостоятельно доказывает вид региона, мутабельную форму для
записи и версию языка. Веток `Op::MmioRead` и `Op::MmioWrite` в верификаторе
**не было вовсе**, и это дефект реализации против принятого контракта, а не
новое решение. Теперь доказываются: ширина и порядок байт называют принятую
операцию; регион — точно `MmioRegion`/`MmioRegionMut`, для записи только
мутабельная форма; смещение — точный `size`; записываемое значение — точный
`u64`; результат чтения — точный `u64`, записи — `unit`; минор — последним. На
каждый пункт есть негатив из подделанного IR, портящий ровно один факт модуля,
который верифицировался мгновением раньше.

**Движение производных дайджестов, санкционированное Архитектором.** Поскольку
позиция предопределённого аргумента теперь называет свой тип, в таблицу типов
модуля попадают `size`/`u64`, которых там могло не быть. Изменились ровно два
артефакта, и ни в одном не изменился поток инструкций:

```text
tests/vectors/virtio-msix-wait/init.tos   исходник побайтно тот же
  дайджест модуля  было   sha256:4ecb8375bc3255d60a0c76a919d3b56f70be5251f60177cf5d0d3786037c1e82
                   стало  sha256:e2fb96859105209ac35d411cdc340a07e069cf19eb506ea36108982142dd2d7c
```

Причина — только в том, что ранее неявный контракт типов предопределённых
операций стал явным и попал в производную таблицу типов; этот исходник объявляет
девять сущностей типа `size`, а таблица типа не называла. Ни один документ
свидетельства этот дайджест не фиксирует и ни один гейт его не сравнивает, так
что датированное приложение никуда не требуется. Второй артефакт — фикстура
совместимости 1.2 в `artifact_compatibility.rs`, перезакреплена на месте с
комментарием о причине. **Векторы Stage 4D-2 и 4D-3 побайтно те же**, дайджест
в дайджест, поэтому свидетельства, которые их фиксируют, не затронуты. Устаревший
производный дайджест **не сохранялся** ценой подавления требуемых типов.

### Остаётся нерешённым после этого среза

1. **Сигнатуры импортированных вызовов.** Ни `LoweringInterface`, ни
   `VerificationSurface`, ни объявление в бандле, ни `ResolutionSnapshot` не
   переносят типы параметров, поэтому ни фронтенд, ни верификатор не могут
   проверить вызов чужой функции. Три архитектурных вопроса — момент выдачи
   исходной диагностики, аутентификация снимка и сохранение через границу
   бандла — остаются открытыми и в этот срез не входили.
2. **`E1217` для импортированных вызовов** зарегистрирован, но не проверяется по
   той же причине.
3. **Стирание `PassMode`** у типа callable и у импортированных сигнатур — без
   изменений.
4. **Типизация чекера всё ещё частична** (`Type::Unknown` согласуется со всем), и
   полезная нагрузка варианта по-прежнему не выводима из места.

**Stage 4D-4 заблокирован.**

### 2026-09-15 — импортированный вызов проверяется по артефакту, который этот запуск уже верифицировал

Срез контракта Core. Stage 4D-4 не начат и остаётся **заблокированным**.

**Что принималось вместо проверки.** Импортированный вызов допускался силой
**декларации**. `ResolutionSnapshot` говорит, во что, по словам производителя,
разрешилось замыкание; верификатор задавал ему единственный вопрос — объявляет
ли названный импортом модуль такое имя экспорта — и не спрашивал у артефакта
зависимости ничего. Снимок никогда не сравнивался ни с одним образом, поэтому
враждебная декларация, называющая экспорт, которого у настоящего модуля нет,
принималась; и подделанный вызывающий, идеально согласованный с враждебной
декларацией относительно сигнатуры, которой настоящий модуль не объявляет,
принимался тоже. Типы пересекали границу непроверенными в обе стороны.

**Что принято (Candidate D).** Правило перезагрузки ADR-0071 §5, применённое во
время запуска:

```text
зависимость верифицирована ранее в этом же запуске
  -> непрозрачное свидетельство с её artifact digest
  -> те же байты получены из существующего ClosureSource
  -> хешируются здесь и сравниваются до чтения хотя бы одного экспорта
  -> префикс экспортов восстанавливается единственным декодером образов
  -> импортированные вызовы этого вызывающего проверяются по нему
  -> декодированный префикс отбрасывается
```

**Ничего не переживает границу модуля.** Ни в `VerifiedModuleRecord`, ни в
`VerifiedClosureManifest`, ни в `ResolutionSnapshot`, ни в декларации бандла не
попадает типизированная поверхность экспорта: из хода в ход переходят
шестьдесят четыре байта идентичности, которые не могут назвать ни одного
экспорта. Схема `tos-ir/v1` не менялась, формат `TOSBUNDLE` не менялся.

**И свидетельство нельзя предъявить.** У `VerifiedArtifactEvidence` приватные
поля, нет публичного конструктора и ровно один источник — успешная верификация,
выполненная этим же запуском. Провайдер, способный заявить «эта зависимость
верифицирована, вот её сигнатура», был бы тем же неаутентифицированным снимком
под лучшим именем.

**Порядок замыкания стал нормативным (ADR-0071 §1a, ADR-0073 §4).** Точное
разрешённое замыкание передаётся в порядке «зависимости первыми»: каждый модуль
после каждого модуля, который он импортирует. Сборщик уже выдавал этот порядок,
поэтому ни один канонический артефакт не сдвинулся. При верификации модуля на
позиции `N` каждый названный его импортами модуль должен быть уже успешно
верифицирован в этом же запуске; прямая зависимость вперёд отвергается
существующим `V2012_IMPORT`, и цикл падает по тому же правилу без второго
алгоритма.

**`Module::exports` — каноническая публичная проекция `Module::functions`**
(`docs/43` §2). Верификатор доказывает это сам: восстанавливает ожидаемую
таблицу из функций артефакта и сравнивает с полным равенством `Signature`,
включая `PassMode`, `is_async` и эффекты. До этого среза он проверял только
сортировку, а `Closure::export_of` во время исполнения разрешает имя по
`functions` — то есть аутентифицированная таблица экспортов могла описывать
функцию, в которую исполнение не входит. Расхождение — `V2004_TABLE_ORDER`.

**Исходные диагностики импортированных вызовов.** Проверка выполняется
непосредственно перед `lower_module_in_set` для каждого вызывающего, в тот
момент, когда его исходник уже перечитан для собственного хода понижения, а
`LoweringInterface` его зависимостей живы, — это первый момент, когда точная
сигнатура вообще существует. Диагностики настоящие, а не `Gap`: неверная
арность — `E1217` с `context=imported named`, неверный тип позиции — обычный
приоритет, заканчивающийся `E1215`, отсутствующий член разрешённого модуля —
`E1202_UNKNOWN_VALUE_NAME`. Понижающий `Gap` «импортированный модуль не
экспортирует это имя» удалён. Семантическая стадия диагностики остаётся
`Stage::Type`; внешний отказ конвейера честно сообщает `PipelineStage::Lower`,
потому что это первая фаза, в которой существует аутентифицированный интерфейс
зависимости.

**Потолок рёбер замыкания — ADR-0090.** Стоимость дисциплины «ничего не
удерживать» — одно аутентифицированное переоткрытие на отношение
вызывающий→зависимость, измеренное примерно в `7,1 мс` на модуль предельного
размера. Ничем не ограниченным оставалось само число таких отношений: под уже
принятыми потолками (256 модулей, глубина графа 64) плотнейшее допустимое
замыкание достигает **32 256** рёбер, то есть **≈229 с** работы. Измерено, а не
оценено: 1 920 рёбер на 64 модулях стоили `18 287 мс` против базовой линии
`4 662 мс` без импортов.

`docs/44` §2 получает строку `resolved closure direct dependency edges 1024`.
Ребро — это одна **уникальная** пара `(идентичность вызывающего, идентичность
разрешённой зависимости)`, а не объявление импорта: два связывания одного модуля
— одно отношение и одно ребро. Обоснование потолка измерено в этом же дереве:
182 модуля / 49 прямых рёбер во всём репозитории, крупнейшее реальное замыкание
— 3 модуля / 4 ребра, наибольшее число прямых импортов одного модуля — 4. 1024
— это **256× крупнейшего реального замыкания** и ограничивает переоткрытия
примерно 7,3 с. Выделен `E1609_IMPORT_EDGE_LIMIT` (stage `type`, поля `limit`,
`actual`). Запуск проверяет тот же потолок **независимо**, одним `usize`,
живущим только на время запуска, и отвергает существующим `V2001_LIMIT` там,
где потолок перейдён, а не после работы, которой требовал остаток замыкания.

**Точный тест границы нашёл дефект реализации, и исправлена реализация.**
Требование Архитектора — точные границы со стороны запуска, где число рёбер
считается по артефактам, а не берётся из названия фикстуры, — показало, что
§2a ADR-0090 была **заявлена, но не реализована**. Основание считать два
связывания одной зависимости одним ребром звучит так: «запуск аутентифицирует
эту зависимость один раз и проверяет по единственной восстановленной
поверхности все места вызова этого вызывающего». Цикл переоткрытия в
верификаторе при этом шёл по `module.imports` **по объявлениям** и
аутентифицировал одну и ту же зависимость заново на каждое связывание. Квота
считала отношения, а работа оплачивалась по объявлениям, поэтому одно ребро
бюджета покупало до 255 аутентифицированных переоткрытий: 256 модулей, каждый
связывающий одну зависимость 255 раз, — это 255 рёбер, заведомо внутри потолка,
и 65 025 переоткрытий. Амплификация, ради закрытия которой ADR-0090 и
принималась, закрыта не была.

Исправлена реализация, а не решение: цикл открывает каждую **различную**
зависимость один раз на вызывающего и проверяет по этой единственной
поверхности все места вызова, достигающие любого её связывания. Ни одно решение
о допуске не изменилось — проверяются те же места вызова по тем же экспортам, —
а рёбра и переоткрытия стали одним и тем же числом, и именно это делает потолок
границей работы. Доказательство — журнал переоткрытий: запуску передаётся
записывающий `ClosureSource`, и два связывания одной зависимости дают ровно
три чтения образов (два модуля и одно переоткрытие), а замыкание ровно из 1025
рёбер отвергается `V2001_LIMIT` после 1024 оплаченных переоткрытий и ни одного
за ребро, которое переходит потолок.

**Отвергнутые варианты, с измерениями.** Удерживаемый «фронт» верифицированных
сигнатур нарушает правило времени жизни ADR-0071 (после верификации модуля
переживает только запись фиксированной формы) и при широком fan-in удерживал бы
255 поверхностей сразу. Ограниченный кэш поверхностей измерен и опровергнут:
шаблон доступа — скан, а не рабочее множество, поэтому LRU на шестнадцать
поверхностей убирает `0,5 %` разборов в плотном случае и **ни одного** в цепочке
или широком fan-in.

**Коррекция предыдущего аудита.** Заявленного дефекта типизации импортированного
`async` не существует: `Signature::result` экспорта уже является результатом
места вызова (`async fn -> T` экспортируется как `Task<T>`), поэтому `is_async`
переносить не нужно и специального правила на месте проверки нет.

### Остаётся нерешённым после этого среза

1. **`resource imports` проверяется не так, как определён.** `docs/41` §6
   определяет поле как «максимум транзитивных зависимостей модуля», а
   верификатор сравнивает с ним `module.imports.len()` — число **прямых**
   объявлений. Это отдельный дефект корректности: поле не переопределено, не
   использовано как потолок рёбер и не «починено попутно». Следующая
   архитектурная задача после этого среза — точечный аудит того, где и
   проверяется ли вообще транзитивная граница.
2. **`PassMode` у импортированных и callable-сигнатур** по-прежнему не
   переносится и не сравнивается на вызове; доказано только то, что
   аутентифицированная таблица экспортов описывает ту функцию, в которую войдёт
   исполнение.
3. **Типизация чекера всё ещё частична**, и полезная нагрузка варианта
   по-прежнему не выводима из места.

**Stage 4D-4 заблокирован.**

### 2026-09-15 — `resource imports` считает модули, и теперь его кто-то считает

Срез исправления контракта Core. Stage 4D-4 не начат и остаётся
**заблокированным**.

**Контракт был верен, а проверки не было.** `docs/41` §6 с принятия V1 говорит
одно — `imports: integer, // maximum transitive module dependencies`, — а
единственная проверка на всём production-пути сравнивала это с
`module.imports.len()`. Это разные величины, и расхождение идёт в обе стороны.

Замер до исправления, сквозь `execute_set`:

```text
A импортирует B; B импортирует C; A объявляет imports: 1   => COMPLETED
A импортирует B дважды;           A объявляет imports: 1   => V2001_LIMIT
```

Первое — **fail-open**: модуль достигает двух модулей, объявляет место для
одного, и замыкание исполнилось. Форма не ограничена глубиной: под принятыми
потолками модуль с `imports: 1` может стоять над **255** транзитивными
зависимостями, то есть реально действующая граница была короче объявленной
в 255 раз. Второе — **ложный отказ**: одна зависимость, объявлена одна, отказ; и
модуль становился допустимым от объявления `imports: 2`, что и доказывает, какая
величина на самом деле требовалась. ADR-0090 §2a незадолго до этого решил, что
два связывания одного модуля — одно отношение, а эта проверка отвергала их как
два.

**Ключ не переосмыслен.** ADR-0071 дважды прочитал его строго — в тот момент,
когда отказывался на него опираться, — и ADR-0090 §2b повторил то же. Три
принятых решения уже зависят от строгого чтения, поэтому исправляется
реализация.

**Считается множество модулей** (ADR-0091 §2): уникальные разрешённые
идентичности модулей, достижимые из этого модуля, прямые и косвенные вместе, сам
модуль исключён, два связывания одной зависимости — одна зависимость, импорт
capability-интерфейса не считается вовсе. По **разрешённому графу**, а не по
написанию в исходнике.

**Две проверки, доказывающие разное.** Верификатор одного модуля не располагает
аутентифицированным графом и потому не может доказать транзитивное свойство —
а поле в артефакте, которое бы это «исправило», сделало бы авторитетным
утверждение производителя, то есть открыло бы ту самую дыру, которую закрыл
предыдущий срез. Доказуемо локально только то, что **уникальные прямые**
зависимости помещаются в конверт: они подмножество транзитивных, значит
превышение прямыми есть превышение транзитивными. Необходимое условие, никогда
не достаточное; код перевыставлен с `V2001_LIMIT` на `V2022_RESOURCE`, потому
что это модуль, нарушающий собственный объявленный конверт, а не потолок
реализации. Достаточное условие доказывает запуск: по замыканию «зависимости
первыми» он объединяет фиксированные 256-битные множества, и `ResolutionSnapshot`
в этом решении не участвует вовсе.

**Отдельный найденный дефект, исправленный первым.** `launch()` читал
`source.count()` и сразу выделял таблицу записей по этому числу — 464 байта на
модуль — до какой-либо проверки. Замер до исправления: замыкания из 256, 257 и
**400** модулей принимались одинаково. `docs/44` §2 публикует
`module dependency closure 256 modules`, и на production-пути запуска этого не
проверял никто. Теперь это первое утверждение запуска, до любого выделения
памяти, пропорционального замыканию, с существующим `V2001_LIMIT` и субъектом
`module dependency closure`; действующая граница — `min(limits.modules, 256)`.
Потолок, проверенный после выделения, которое он ограничивает, — не потолок,
поэтому отрицательный тест на 257 доказан записывающим `ClosureSource`: **ноль**
прочитанных образов.

**Память и стоимость.** `[u64; 4]` на позицию замыкания — тридцать два байта
независимо от того, что модуль объявляет. На потолке в 256 модулей это
`8 192 B` сырых и `8 216 B` с учётом контейнера; постоянного состояния после
запуска — **ноль**. Изолированная стоимость объединений и popcount'ов: от
`1 027 нс` (цепочка 64) до `5 385 нс` (256 модулей, 1024 ребра). На уровне
запуска разница не разрешается над разбросом прогонов: он составляет
`±150…1 100 мкс`, а в двух формах измеренная «разница» отрицательна.

**Одна несоответствующая синтетическая фикстура исправлена.** Генератор
шестимодульной цепочки в `module_sets.rs` объявлял плоское `imports: 4`, тогда
как последний модуль достигает пяти. Фикстура была неверна по контракту,
который уже существовал; исправлена фикстура, а не язык.

**Канонические исходники не сдвинулись.** Из 182 `.tos` в дереве импорты
объявляют восемь модулей, у каждого ровно одна транзитивная зависимость и
объявлено не меньше. Исправление меняет проверки, а не lowering, поэтому ни один
артефакт и ни один дайджест Stage 4 сдвинуться не может.

### Остаётся нерешённым после этого среза

1. **Потолок замыкания в 256 модулей не проверяется на стороне исходника и
   сборки.** ADR-0091 §5 чинит независимый запуск — тот, который обязан держать
   против провайдера. Должен ли фронтенд отказывать набору из 257 модулей
   собственным диагностическим кодом — отдельное решение, не принятое здесь.
2. **`PassMode` у импортированных и callable-сигнатур** по-прежнему не
   переносится и не сравнивается на вызове.
3. **Типизация чекера всё ещё частична**, и полезная нагрузка варианта
   по-прежнему не выводима из места.

### 2026-09-16 — Stage 4D-4: два запроса в очереди одновременно

Аппаратный срез. **Исторический блокер Stage 4D-4 снят** — основание ниже.

**Что доказывал 4D-3 и чего не доказывал.** Его цикл — submit, wait, consume,
reclaim, submit, и он говорит это вслух. Пул из четырёх дескрипторов при трёх на
запрос делал второй запрос невозможным без возврата первого, то есть ни в один
момент устройство не держало больше одной цепочки этого драйвера.

**Что доказано здесь.** Глубина очереди больше единицы. Обе цепочки строятся из
пула шириной ровно в две цепочки, обе головы пишутся в соседние слоты
available-кольца, и затем **один** store двигает `avail.idx` на два:

```text
avail.ring[0] = head A ; avail.ring[1] = head B
dma_publish -> avail.idx 0 -> 2 (один store) -> dma_publish -> notify
```

**Два store по `+1` были бы другой программой даже без чего-либо между ними.**
§2.7.13.3 говорит, что после обновления `avail.idx` устройство «MAY access the
descriptor chains ... immediately», поэтому после первого `+1` оно вправе
завершить запрос A до того, как второй store состоится, и такой прогон доказал
бы ровно то, что уже доказал 4D-3. Один store с `i` на `i + 2` — это форма
самого §2.7.13.2 («the driver MAY add many descriptor chains before it updates
idx»), разрешённая шагом 3 §2.7.13.

**Порядок завершения не предполагается.** `VIRTIO_F_IN_ORDER` не согласован,
§2.6 говорит, что устройство «is not generally required to use buffers in the
same order», а §5.2.6 — что блочные запросы используются «not necessarily in
order». Идентичность — это `used_elem.id`, сопоставляемый с двумя
незавершёнными головами; слот кольца не значит ничего. Оба порядка проходят;
наблюдённый — измерение, а не утверждение.

**Два независимых свидетеля.** У каждого запроса свой заголовок, свой
512-байтный буфер и свой байт статуса: запросы висят одновременно, поэтому
разделять буфер они не могут в принципе. A читает сектор 1 в буфер, отравленный
`0xA5`, и обязан найти 512 копий `0xC1`; B — сектор 2, отрава `0x5A`, `0xC2`.

**Один регион на 8 KiB.** Два блока запросов не помещаются в 4 KiB: кольцо
кончается на 3342, блокам нужно 1075 байт, остаётся 754. Субстрат очереди и блок
запроса A стоят побайтно там же, где их поставил 4D-3; блок B дописан следом по
тем же правилам выравнивания. Один `dma_region_allocate`, `contiguous=1`,
`capability_delta=1`, девять устройственных адресов из девяти ограниченных
смещений.

**`resource allocation` не менялся, и это проверено, а не предположено.**
`docs/41` §6 делает `allocation` максимумом живых выделяемых байт модуля, и
движок списывает его только там, где конструируется значение; DMA-регион
финансируется capability `system.memory.Authority` через `charge_grant`
нуклеуса. Измерено: боот держит регион на 8192 байта и сообщает
`allocation=576/4096`.

**Что именно доказывает одновременность.** Не головы цепочек и не число
прерываний, а конъюнкция фактов, установленных непосредственно **перед**
единственной публикацией: все шесть дескрипторов `PREPARED` и попарно различны,
свободных в пуле ноль, потребительский индекс ещё равен предпубликационному, оба
слота available-кольца уже содержат головы, перед публикацией ровно один
`dma_publish`, ровно один store в исходнике двигает `avail.idx` на два, после
него все шесть принадлежат устройству, и до публикации нет ни `irq_wait`, ни
`dma_consume`, ни возврата цепочки. Несущий здесь — **нулевой счёт свободных**:
драйвер, обслуживающий запросы по очереди, до этой строки с нулём не доходит.

**Соотношение голов ничего о конкурентности не доказывает, и это записано.**
При FIFO на шесть слотов драйвер, который выделил A, вернул её и затем выделил
B, получил бы для B те же 3, 4, 5 — три дескриптора A встают *за* тремя, что
ещё впереди фронта. Гейт проверяет это соотношение как детерминизм аллокатора и
говорит об этом прямо.

**Свидетельство.** `TOS.RUN.COMPLETED value=i64:144688603790835711` — полная
маска из 30 бит, `avail.idx` 0 → 2, потребитель 0 → 2, головы 0 и 3, порядок
A→B, `fuel=84779`. Пять одинаковых прогонов. Нуклеус насчитал **одну** доставку
MSI-X, модуль сделал **один** возврат из `irq_wait`, и один `dma_consume` сделал
видимыми **две** used-записи: одно пробуждение открыло две завершённые
used-записи, то есть свойство батчинга ADR-0082 §10 было задействовано впервые в
проекте. **Это подтверждает путь «дренировать до пустоты» у драйвера и не
устанавливает параллельного исполнения на устройстве.** Нормативное утверждение
о конкурентности остаётся на границе владения драйвер/устройство, созданной
единственной публикацией `avail.idx += 2`.

**Форма публикации доказывается текстом, а не прогоном, и это сказано прямо.**
Никакое наблюдение драйвера над самим собой не отличает один store `+2` от двух
store `+1`: оба приходят к тому же счётчику. Рантайм-биты доказывают
предусловие — обе цепочки построены и не пересекаются, пул пуст, все шесть
дескрипторов `PREPARED`, потребитель ещё 0, и сразу после публикации все шесть
принадлежат устройству, — а то, что store написан один раз, гейт проверяет по
тексту вектора. Бит назван `publication_added_two`, а не «одна публикация»,
потому что доказывает он именно это.

**Негативы.** Восемь значений против судейства гейта, включая обязательное
принятие обратного порядка B→A. На настоящем устройстве — четыре мутации: чтение
used-записи всегда из слота 0 даёт `-88` (вторая запись называет уже
завершённый запрос), возврат чужой цепочки даёт `-89`, перестановка ожидаемого
содержимого секторов даёт `-62`, и возврат цепочки A в пул перед единственной
публикацией даёт `-81`. Две мутации отвергаются **до загрузки** статической
проверкой формы: два store по `+1` и точное восстановление дисциплины 4D-3
(publish A → wait/consume/reclaim → publish B), которой нужен второй store.

**Мутация с сериализацией нашла реальный промах в расстановке проверки.**
Нулевой счёт свободных сначала проверялся сразу после обеих аллокаций — место
естественное и неверное: дескрипторы можно вернуть в пул между аллокацией и
публикацией. Такая мутация ловилась на четыре шага позже финальной проверкой
целостности пула (`-64`), а не тем инвариантом, который обязан стеречь этот
момент. Проверка перенесена к двум другим, непосредственно перед публикацией, и
та же мутация теперь отвергается `-81`. Число инструкций не изменилось: то же
значение бота и тот же `fuel=84779`.

**Нуклеус не получил ничего.** Ни операции, ни версии интерфейса, ни типа
полномочия, ни minor'а языка, ни изменения схемы `tos-ir/v1` или `TOSBUNDLE`.
Векторы 4D-1/4D-2/4D-3 не тронуты. Свидетельство —
`docs/evidence/STAGE4D4_TWO_OUTSTANDING.md`.

**Не доказано и не заявляется:** что устройство исполняло оба запроса в один
физический момент; запись; вторая очередь; планирование; блочный сервис;
файловая система; драйверный фреймворк.

### Исторический блокер Stage 4D-4 снят

Запись от 2026-09-12 гласила: «**Stage 4D-4 заблокирован до решения по этому
пункту**» — по пункту стирания `PassMode` у типа callable. Блокер снят, и ни
один из перечисленных ниже пунктов не является блокером Stage 4D-4; все они
остаются реальным backlog'ом.

1. **Частичная типизация чекера / `Type::Unknown`** — реальный дефект, но
   Stage 4D-4 не вводит ни одной новой конструкции: вектор пользуется тем же
   набором, что и зелёный 4D-3.
2. **Вывод типа полезной нагрузки варианта** — то же самое.
3. **`PassMode` у типа callable** — стирается в
   `TypeDef::Function(Vec<TypeId>, TypeId)`. Достижимо только через значение
   функционального типа, а единственный производитель такого значения —
   выражение-замыкание. Вектор Stage 4D-4 не содержит ни замыкания, ни
   аннотации функционального типа, и **гейт проверяет это по тексту вектора**.
   **Коррекция аудита:** аудит утверждал, что все векторы Stage 4 —
   `profile bootstrap`, и потому замыкание невозможно по профилю. Это неверно:
   все векторы Stage 4, включая этот, — `profile full`, где замыкание
   разрешено. Вывод держится на более узком основании — в этом векторе
   замыкания нет, — и будущий Full-драйвер, который его напишет, дефект
   **достигнет**.
4. **Сравнение `PassMode` на месте импортированного вызова** — отдельно
   отметим, что `Parameter.mode` **уже переносится и аутентифицируется** в
   сигнатуре экспорта (`an_export_with_a_changed_pass_mode_is_refused`);
   открытым остаётся именно сравнение в точке вызова. Прежняя формулировка
   «не переносится» устарела и исправлена здесь. Stage 4D-4 — один модуль без
   единого импорта модуля, так что путь не достигается.

### Остаётся нерешённым после этого среза

1. **Потолок замыкания в 256 модулей не проверяется на стороне исходника и
   сборки** (ADR-0091 §5 починил независимый запуск).
2. **`PassMode` у типа callable и сравнение на месте импортированного вызова** —
   см. выше.
3. **Типизация чекера всё ещё частична**, и полезная нагрузка варианта
   по-прежнему не выводима из места.
4. **Ветка B-первым не подтверждена железом.** QEMU на эталонном профиле
   завершал A первым во всех прогонах, поэтому обратный порядок покрыт
   конструкцией кода и негативом судейства, но не настоящим устройством.

### 2026-09-17 — Stage 4D-5: первая настоящая запись на блочное устройство

Аппаратный срез. До него каждый боот Stage 4D **читал**: устройство писало в
память, выданную драйвером, а свидетелем был сентинел, который оно обязано было
заменить. Ни один байт на диске от действий гостя не менялся.

**Что доказано.** Модуль составляет 512 байт, пишет их в сектор 5 одним
`VIRTIO_BLK_T_OUT`, **затирает буфер, из которого писал**, и читает тот же
сектор обратно в **другой** отравленный буфер одним `VIRTIO_BLK_T_IN` — и
получает ровно эти байты.

**Байт статуса — не доказательство, и это измерено.** §2.7.5.1 говорит, что
устройство «SHOULD NOT read a device-writable buffer», поэтому дескриптор
полезной нагрузки, ошибочно помеченный на запись, — это нагрузка, которую
устройству велено не читать: диск сохранит старое содержимое, а статус скажет,
что всё в порядке. На настоящем устройстве, со снятой собственной защитой
модуля, такой прогон дал `-59`: QEMU сообщил `used.len` 513 вместо 1. Именно
поэтому утверждение этого среза требует обратного чтения.

**Ёмкость читается у устройства до того, как что-либо опубликовано.** §5.2.6.1:
«A driver MUST NOT submit a request which would cause a read or write beyond
capacity». Это первый вектор Stage 4, который выводит границу из устройства, а
не из известного диска эталонного профиля: обход настоящего списка
capability до `VIRTIO_PCI_CAP_DEVICE_CFG`, проверка формы, BAR, выравнивания
смещения и длины — и только затем чтение 64-битного `capacity` по протоколу
§2.5.1. Берётся **первая** подходящая структура, потому что это правило самого
драйвера — §4.1.4.1: «The driver SHOULD use the first instance of each virtio
structure type they can support», — а хорошим это правило делает сторона
устройства, §4.1.4.2: «If the device presents multiple structures of the same
type, it SHOULD order them from optimal (first) to least-optimal (last)». Две
клаузы, две стороны; §4.1.4.3 — это раскладка common configuration и этого
правила не содержит. **Требование того же BAR — ограничение вектора, а не
VirtIO**: capability несёт свой `bar`, и спецификация нигде не требует, чтобы
device-specific configuration делила BAR с common configuration. Эталонная
конечная точка Stage 4 кладёт выбранную структуру в уже отображённый BAR
VirtIO; вектор проверяет этот факт профиля и отвергает другой BAR (`-45`),
вместо того чтобы заводить второе отображение ради одного среза свидетельства, потому что «Drivers MUST NOT assume reads from fields greater than 32
bits wide are atomic». Повтор ограничен константой самого драйвера; не
устоявшееся поколение — отказ, а не цикл. Наблюдено: **32768 секторов за одну
попытку**.

**Негатив по ёмкости — тот же капсул на диске из 4 секторов**, собранном
поразово опциональным флагом харнесса (эталонный профиль не тронут): `-44` и
**ноль** прерываний по счёту нуклеуса. Ноль доставок и есть доказательство, что
до устройства ничего не дошло: нет уведомления — нет завершения — нет MSI-X, а
эту строку модуль не может ни подделать, ни подавить.

**Два независимых буфера и затирание между ними.** Экстенты проверяются
непересекающимися до постройки запросов, а после завершения записи и возврата её
цепочки буфер нагрузки перезаписывается нулями с проверкой. С этого момента
шаблон не существует нигде в памяти машины — ответ обратного чтения может
прийти только от устройства. Ожидаемый байт **пересчитывается из индекса** с
обеих сторон, а не сравнивается с сохранённой копией.

**Неоднородная нагрузка.** `(0x5C + 7*i) mod 256`; `gcd(7, 256) = 1`, поэтому
шаблон проходит все 256 значений и повторяется только через 256 — 512 копий
одного байта не отличили бы запись сектора от первого байта, записанного 512
раз. На образе после прогона: 256 различных значений в секторе 5.

**Свидетельство.** `TOS.RUN.COMPLETED value=i64:36081574113968127` — полная
маска из 29 бит, ёмкость 32768, одна попытка поколения, очередь 128,
`fuel=105432`, две доставки MSI-X. Три одинаковых прогона. Хост-сверка: образ
после прогона побайтно равен эталонному образу с переписанным сектором 5.

**Хост-проверка ловит то, чего гость увидеть не может, и это измерено.** С
сектором, изменённым на 9 в векторе, гость сообщил `36081574113968127` —
полное правильное значение, побайтно как у проходного прогона, потому что писал
и читал сектор 9 согласованно. Отверг только хост.

**Два негатива статические, и причина измерена.** Удаление `dma_consume`
оставляет боот с побайтно тем же правильным результатом: ADR-0086 §11 понижает
операцию до барьера компилятора и исполнения в ring 3, а x86 под QEMU делает
записи устройства видимыми и так. Аппаратная гонка была бы недетерминированной и
негодной как свидетельство, поэтому гейт проверяет исходник: `dma_consume`
обязан предшествовать каждому чтению used-кольца и статуса. Так же — и
неизменность опубликованной нагрузки.

**О долговечности сказано ровно то, что говорит контракт.** Устройство
предлагает `FLUSH`(9) и `CONFIG_WCE`(11) и **не** предлагает `RO`(5); драйвер
не согласовывает ни первое, ни второе, поэтому §5.2.6.2 относит завершённую
запись к **случаю 1** — «stable». Это то, что говорит контракт. **Stage 4D-5 не
измеряет реализацию долговечности на стороне хоста и ничего о ней не
утверждает**: бэкенд настроен `cache=writeback` (измерено через QMP), а
содержимое образа после выхода QEMU не доказывает фиксацию в момент завершения.
Для этого нужен отдельный срез со своим дизайном свидетельства.

**Нуклеус не получил ничего.** Ни операции, ни версии интерфейса, ни типа
полномочия, ни minor'а языка, ни изменения схемы, ни ADR. Конфигурация,
специфичная для устройства, достаётся теми же обобщёнными чтениями PCI
capability и тем же отображённым окном BAR, что дал Stage 4B. Профиль Stage 4 не
изменён: харнесс получил один опциональный флаг, которым пользуется только
негатив этого гейта. Векторы 4D-1/2/3/4 не тронуты. Свидетельство —
`docs/evidence/STAGE4D5_FIRST_WRITE.md`.

**Не доказано и не заявляется:** долговечность при сбое, персистентность на
хосте, семантика fsync, переживание завершения QEMU, реализация flush, блочный
сервис, файловая система, кэш, планировщик или драйверный фреймворк.

### 2026-09-17 — подготовка к закрытию 4C/4D: два пробела в свидетельстве закрыты

Срез подготовки к закрытию. Продукционного механизма не менялось ничего.

**Аудит готовности к закрытию Stage 4C/4D нашёл два пробела свидетельства** —
не поведения. Оба теперь покрыты механически.

**1. Разбор устройства при гибели процесса не утверждался ни одним гейтом.**
Каждый боот 4D заканчивается тем, что процесс умирает, держа функцию, окно,
источник прерывания и DMA-регион; нуклеус при этом печатает
`DMA_QUARANTINED … kept=1`, дренаж назначения, `DMA_RECLAIMED … drained=1
refunded=1`, `IRQ_RELEASED … vector_retired=1` и `PROCESS_RECLAIMED …
dma_quarantined=0` — и **ни один гейт этого не проверял**. Обязательство
ADR-0084 о release/quarantine/drain было закрыто гейтом только для пути явного
освобождения. Теперь гейт Stage 4D-5 требует эти строки **в порядке жизненного
цикла** и привязывает их к этому боту: число кадров вычисляется из региона,
который гейт уже утверждает (8192/4096 = 2), а число доставок — из двух запросов
этого бота. Проверено девятью мутациями журнала: удаление строки карантина,
`kept=0`, отсутствие дренажа, `drained=0`, оставшиеся в карантине кадры,
невыведенный вектор, чужой регион, чужой источник и перестановка reclaim перед
карантином — каждая отвергается с именем шага.

**Путь явного освобождения не переписан.** `dma-region.sh` по-прежнему
доказывает своё — модуль зовёт `capability_release` и обнаруживает оба
устаревших пути закрытыми. Это разные обязательства, и ни один тест не
доказывает второй: явное освобождение — решение модуля, а здесь проверяется то,
что делает нуклеус, когда не решил никто.

**2. Страж словаря ring 0 устарел до Stage 4D.** Список токенов
(`virtio|virtqueue|device_status|driver_ok|feature_select|blk`) был заморожен
раньше, чем в дереве появились кольца, индексы, типы запросов и секторы, и **ни
один гейт 4D его не запускал**. Общий расширенный список теперь в
`stage4-profile.sh`, его гоняет гейт Stage 4D-5, и охват — три дерева, которые
боот действительно исполняет: ring 0, ring-3 бинарь рантайма и движок, который
он линкует. Зафиксировано до изменения: дерево **чисто** по расширенному списку;
после изменения та же проверка идёт в составе профиля и проходит. Проверено
инъекцией: `const AVAIL_IDX_PROBE` ловится, а комментарий со словом `virtqueue`
— нет. Родовые слова (`queue`, `descriptor`, `capacity`, `status`) в список
намеренно не входят: страж против семантики устройства, а не против английских
существительных. Единственное узкое исключение — `EXPRESS_DEVICE_STATUS`
(ADR-0084 §5b), и оно задокументировано на месте.

**3. Свидетельство Stage 4C-1 появилось в надлежащей форме.**
`docs/evidence/STAGE4C1_ROUTED_INTERRUPT.md` — **пост-реализационное**
свидетельство поведения, зелёного с 2026-09-07: полномочие
(`platform.irq.Source` выводится из живого назначения PCI, а не из окружающего
контроллера), доставка настоящего MSI-X, перепись liveness с `routed=1`,
однобитная защёлка и смысл пробуждения, время жизни (один ожидающий,
освобождение, выведение вектора, связь с назначением) и негативы, которые
**реально исполняются сегодня**. `STAGE4C1_REVIEW_FINDINGS.md` остаётся тем, чем
был, — пред-реализационным отчётом от 2026-09-05, когда ADR-0082 был ещё
*Proposed*, а 4C-1b не начат; новый документ говорит об этом различии прямо.
Гейты перезапущены на этом дереве, а не пересказаны по старым логам.

**Stage 4C и Stage 4D готовы к закрытию и ожидают формального рассмотрения
Project Architect.** Здесь **не** заявляется, что они закрыты, и **не**
заявляется identity-гейт Stage 4 целиком: постоянное хранилище, передача
capsule → репозиторий, крэш/рестарт на стороне сервиса и отчёт о
производительности Stage 4 остаются вне этого закрытия — последний потому, что
бюджеты `docs/35` говорят о копии между памятью **клиента** и памятью
устройства и о передачах на запрос, а клиента ещё нет.

### 2026-09-21 — решения по срезу Stage 4: вариант A, и два ADR в Proposed

Реализации нет. Записано, что решено, и подготовлено то, что решать.

**Project Architect выбрал Branch A для `persistent data`.** Stage 4 identity
gate доказывает: запись реально дошла до устройства; последующее чтение видит
изменение; данные остаются доступны после остановки и перезапуска block
service. Power-loss durability, семантика `VIRTIO_BLK_F_FLUSH`/barrier и
доказательство сохранности после некорректного завершения QEMU **в Stage 4 не
входят** и выносятся в отдельный будущий durability contract/gate. Первые два
пункта уже доказаны гейтом 4D-5; новым является только третий, и он —
следствие критерия restart, внутри одного бота. Ни работы с устройством, ни
нового контракта ветка A не требует.

**ADR-0092 «Who may reset a PCI function, and what "reset" means» — Proposed.**
Фиксирует достижимость PCIe FLR через нынешний `config_write`: бит 15 Device
Control (Initiate Function Level Reset) не входит в множество, которое
резервирует `write_is_permitted`, а комментарий там прямо оставляет остаток
регистра драйверу как «ordinary business». Разбирает конфликт с ADR-0081 §14:
FLR возвращает BAR'ы к значениям по умолчанию, тогда как ADR-0081 §13 меряет
их однократно при claim, а §14 держит assignment живым ради того, чтобы
измеренное оставалось тем, что функция декодирует. **ADR-0082 §5a закрыл
прямой путь к этому нарушению и не закрыл этот**, и на то, что он считал reset
своим делом, указывает его же Type 1: Secondary Bus Reset там зарезервирован.
Разделены три вещи, которые зовут одним словом: VirtIO-сброс преемником (T1,
не требует ничего — гейт `check-device-status-additive` уже отдельно разрешает
запись `0u64` как «explicit reset»), FLR собственной функции (T2), сброс
третьей стороной после смерти или зависания драйвера (T3, право отсутствует).
Варианты R1a/R1b/R2/R3/R4 без выбора. Экспериментально FLR на устройстве не
проверялся — по прямому указанию, и §3 написан под этот стандарт: анализ
достижимости по коду, не продемонстрированный дефект.

**ADR-0093 «Publication and lifetime of `block.device.v1`» — Proposed.**
Восемь вопросов о времени жизни публикации, четыре варианта P1…P4 без выбора.
Попутно найдено, что **ADR-0051 в своём списке evidence требует, чтобы
отклонённый сервис «не появлялся в interface registry»** — реестра, который ни
одно принятое решение не определяет; и что номер `OBJECT_INTERFACE = 4` занят
в `tos-launch` и в `ObjectKind` фронтенда, а в `nucleus/src/capability.rs`
такого вида объекта нет. Главное в ADR — вопрос 8: случаи A (клиент в
`endpoint_call`, сервис умер), B (capability на мёртвый endpoint, вызова ещё не
было) и C (вызов после рестарта со старой capability) различимы по-разному в
разных вариантах — C **не различим** при P1, — а случай D (запрос принят и не
отвечен) остаётся неоднозначным **при любом варианте**, потому что никакой
механизм capability не отличает «устройство не получило» от «сделало, а ответ
умер вместе с процессом». Единственный рычаг на D — идемпотентность `write`, и
это свойство формы интерфейса, а не его публикации; ADR это называет и не
решает. Поверхность интерфейса зафиксирована узкой: `read`, `write`,
`capacity`.

Оба ADR не внесены в `docs/SPECIFICATION_SOURCES.txt`: гейт требует только
Accepted, и `ADR-0044` — уже существующий Proposed — там тоже не значится.

**Требуют утверждения Project Architect** — список в конце этой записи и в
§14 архитектурной записки; сверх него ничего не предлагается.

### 2026-09-21 — ADR-0092 и ADR-0093 приняты: R1a и P3

Приняты Project Architect 2026-09-21, **до реализации**. Реализации нет и в
evidence не записано ничего: evidence появляется только вместе с прошедшим
гейтом.

**ADR-0092 — вариант R1a.** Initiate Function Level Reset, бит 15 регистра
Device Control PCI Express Capability, входит в множество, защищённое от
CPL-3 `pci_config_write`. Запрет — по существующему правилу «would change», по
биту: чтение не затронуто, обратная запись того же значения проходит, обычный
read-modify-write драйвера по max payload size / extended tags / max read
request не отвергается. Нового ABI operation и нового права нет; строка `reset`
в таблице ADR-0079 §10 по-прежнему «no right allocated», но теперь по решению,
а не по умолчанию. FLR собственной функции держателю `config_write` не
разрешён. Восстановление умершего block driver'а на Stage 4 идёт через
преемника и обычный VirtIO-сброс `DEVICE_STATUS=0` с повторной инициализацией.

**T3 отложен, а не объявлен ненужным, и ADR это говорит прямо.** Сброс функции
третьей стороной после смерти или зависания драйвера — то, о чём написан шаг 2
`docs/11` §Crashes-and-restart, и то, что нужно именно зависшему, а не мёртвому
драйверу. Он уходит к bus/management service вместе с политикой сброса общих
устройств и любым доменом сброса шире одной функции. Принятое решение — это
сознательное закрытие найденной достижимости FLR, и ничего больше.

Контрактная часть внесена тем же изменением: строка в таблице зарезервированных
полей `SYSTEM_ABI_V1` §5, упоминание в описании операции 26, и абзац о том, что
ADR-0082 §5a зарезервировал регистры размещения, но не операцию, которая
возвращает их к значениям по умолчанию.

**ADR-0093 — вариант P3.** Реестр — обычный текстовый сервис. Нуклеус не
получает глобального пространства имён интерфейсов, naming authority не
сливается с supervisor authority (разделение ADR-0051 §3), публикация и lookup
— обычные IPC/capability-операции, правила остаются инспектируемым каноническим
текстом. Клиент получает endpoint capability по lookup и после рестарта делает
новый lookup: **повторный lookup — единственный механизм восстановления на
Stage 4**, старая capability не чинится и никто не переэндаумит работающего
клиента. Именно это делает случай C доказуемым.

Масштаб зафиксирован узко: один интерфейс `block.device.v1`, один publisher,
один lookup. Ни enumeration API, ни multi-device registry, ни metadata
framework, ни discovery protocol, ни согласование версий интерфейса.

Bootstrap записан как требование, а не спроектирован: name service стартует
раньше block service, оба сервиса и клиент получают endpoint capability на него
из своих sealed launch plans, и **реестр не переживает смерть самого name
service** — записи восстанавливаются перезапускаемыми сервисами, а клиент со
старой capability на name service оказывается в случае B относительно него.
Переживание реестром собственного рестарта потребовало бы durable state, а это
`docs/09` и инженерный выход, не Stage 4.

**Случай D остаётся намеренно неоднозначным.** Ни идемпотентности, ни журнала
запросов, ни transaction id, ни exactly-once на Stage 4 не добавляется ради его
устранения. Граница формулируется прямо: если старый block service принял write
request и умер до доставки reply, клиент не получает гарантии, позволяющей
отличить «write не выполнен» от «write выполнен, reply потерян». Это не провал
Stage 4 и не незакрытый дефект, а сознательная граница нынешнего
`block.device.v1`, той же формы, что и non-claims 4D-5 о durability. Идемпотентная
запись остаётся возможным отдельным interface-level решением позже.

**Открытые решения на 2026-09-21, полным перебором строк `- Status:`:**
`ADR-0044` (versioned module digest, Proposed с 2026-08-12, ждёт Project
Architect и к Stage 4 отношения не имеет) — и всё. ADR-0092 и ADR-0093 приняты,
остальные 91 — Accepted. Отдельно от ADR: формальное рассмотрение закрытия
Stage 4C/4D.

### 2026-09-21 — срез 1 остановлен на входе: канонический текст не умеет обслуживать вызов

Реализации нет. Срез «name service + publication + lookup» был начат и
**остановлен до первой строки продукционного кода**, потому что разведка
существующих примитивов нашла, что требуемого в TOS Core нет. Это находка, а не
неудача попытки: ни один файл не написан «на пробу».

**Три вещи, которых нет, и все три нужны P3.**

**1. Модуль не может получить reply capability.** `IPC_V1` §4 говорит, где она
появляется: «It occupies the **last** slot of the message's transfer table».
Таблица переноса живёт в argument region по смещению `MESSAGE_CAPABILITIES`
(ADR-0058), и **в TOS Core нет операции, которая её читает**. Интерфейс
`system.ipc.Reply` объявлен, у него две операции — `endpoint_reply` и
`endpoint_reply_receive` — и **ни одна операция всей схемы не производит
значение этого типа**: `endpoint_receive` объявлен с результатом `i64`.

Единственный способ, которым модуль мог бы завести `system.ipc.Reply`, — это
`import capability`, и именно так показывает `SYSTEM_INTERFACE_V1` §«Capability
parameters». Для per-call reply это несогласуемо: reply capability именует один
конкретный вызов и одноразова, а endowment выдаётся при запуске, до того как
вызов случился. Так что это не «продюсера забыли добавить», а объявленная
поверхность, форма которой предполагает то, чем reply не является.

**Следствие: канонический текстовый сервис сегодня не может ответить на
`endpoint_call` вообще.** Проверено перебором: IPC из TOS Core используют
четыре вектора, и все четыре только *дренируют* журнал `endpoint_receive`.
`deputy.sh` и `request-reply.sh` — те самые гейты, где request/reply и передача
capability действительно происходят, — **не используют ни одного `.tos`
вектора**: это рантаймовые стадии на Rust.

**2. Модуль не может положить capability в сообщение.** Публикация по P3 — это
передача name service своего endpoint. `endpoint_send`, `endpoint_call` и
`endpoint_send_text` принимают только длину или строку.

**3. Модуль не может взять capability из сообщения.** Lookup по P3 обязан
вернуть клиенту endpoint capability. Симметрично пункту 2.

**Обходных путей, совместимых с заданными границами, нет.** Вернуть из lookup
не capability, а «да, опубликовано», и выдать клиенту endpoint при запуске —
это ровно тот скрытый direct-endowment, который пункт 3 задания запрещает, и
тогда P3 не доказан. Сделать name service создателем клиента и выдать endpoint
через launch plan — это уже P4 по существу, и вдобавок не переживает случай B,
где клиент должен пережить сервис. Написать участников рантаймовыми стадиями на
Rust — это отказ от «canonical textual» и от самого предмета Stage 4.

**Что это по масштабу.** Недостающее — поверхность языка, а не ядро: нуклеус
переносит capability в сообщениях уже сегодня, и `deputy.sh` это доказывает.
Нужны строки схемы в `interfaces.rs`, строки `PERFORMED` в рантайме и,
вероятно, новый `Produced`. Прецедент формы есть и задокументирован: две строки
схемы над одним ABI-селектором, как `endpoint_send_text` над селектором 1, —
так что новых ABI-операций может не понадобиться вовсе. Прецедент масштаба —
ADR-0086, который добавил две исходные операции и поднял TOS Core до 1.4.
Диапазон поддерживаемых миноров сейчас 1.0…1.4.

**Решение не принимается здесь.** Это добавление поверхности TOS Core, то есть
уровень 2–3, и пункт 8 задания разрешает его только при недостаточности
существующих примитивов. Недостаточность установлена; форма — вопрос
Архитектора.

### 2026-09-21 — ADR-0094 подготовлен: недостающая поверхность для передачи capability из текста

Кода нет, evidence не тронут, ни один вариант не выбран. ADR-0094 — Proposed.

**Разбор ядра дал результат сильнее ожидаемого: транспорта не не хватает, его
некому вызвать.** Нуклеус переносит capability в сообщениях полностью и
сегодня. `resolve_transfers` читает таблицу отправителя по
`MESSAGE_CAPABILITIES`, отвергает регионы через `is_delegable()` и передаёт
**ровно те права, что держит отправитель** (`rights_of`); операция 3 создаёт
`Object::Reply { caller, generation }` и кладёт его в **последний** слот;
`retain_in_transit` берёт владение до того, как отправитель что-либо теряет;
`hand_over` выдаёт получателю его собственные имена и обнуляет незаполненные
слоты. Время жизни reply уже точное: `reply_generation` растёт ровно в двух
местах — когда ожидание `Waiting::Reply` заканчивается **любым** способом и
когда процесс окончен. `deputy.sh` и `request-reply.sh` это доказывают, и ни
один из них не использует `.tos` вектор.

**Отсутствует ровно текстовая поверхность, три пробела.** Модуль не может
записать таблицу переноса; не может её прочитать; и ни одна операция схемы не
производит значение `system.ipc.Reply`.

**Третий пробел глубже, чем «забыли продюсера», и это записано отдельно.**
Единственный способ завести `Reply` — `import capability`, и именно так его
показывает `SYSTEM_INTERFACE_V1`. Но endowment выдаётся при запуске, а reply
именует один конкретный вызов одного конкретного вызывающего и создаётся в
момент вызова. Модуль, наделённый им при старте, держал бы право ответить на
не случившийся вызов. Объявленная поверхность предполагает то, чем reply не
является.

**Новый ABI-селектор транспорту не нужен — вопрос в другом.** Все байты уже
пишутся и читаются нуклеусом по смещениям ADR-0058. Вопрос не «может ли данное
переехать», а **«кому позволено сказать, чем переехавшее является»**: строка
приёма, объявляющая `Result<system.ipc.Endpoint, i64>`, — это статическое
утверждение о хэндле, вид объекта которого выбрал **отправитель**. Подделка
исключена (хэндл — индекс в таблице, которую процесс не адресует,
`CAPABILITY_V1` §7), и любое использование падает закрыто, но верификатор
доказал бы тип, который рантайм установить не может. Отсюда варианты: A —
только схема и рантайм над существующими селекторами; B — новые ABI-операции с
проверкой вида в нуклеусе; C — гибрид по линии «своё проверять не надо,
чужое надо»; D — существующий механизм, **не найден**, оставлен в записи ради
протокола отказа.

Сверх выбора буквы ADR просит решить четыре вещи: можно ли пересылать `Reply`
из текста (`launch_plan_endow` его запрещает, отправка сообщения — нет); какие
виды объектов вообще выносить в текстовую поверхность (Stage 4 нужен ровно
один — endpoint); является ли объявленный интерфейс строки приёма утверждением
или проверкой; и заменяет ли строка reply `endpoint_receive` или встаёт рядом.

Случай D из ADR-0093 не трогается: никакой идемпотентности, журнала запросов,
transaction id и exactly-once этот ADR не вводит.

### 2026-09-21 — capability перешла между двумя текстовыми процессами

Первый канонический текстовый модуль в истории этой системы, **обслуживший
вызов**, и первая capability, пришедшая к тексту через сообщение, а не через
endowment лаунчера. Гейт `capability-transfer.sh`, значение 127.

**Ядро не менялось.** Разбор под ADR-0094 оказался верным до конца: транспорт
был на месте целиком — `resolve_transfers` читает таблицу отправителя,
делегирует ровно права отправителя, операция 3 создаёт `Reply` и кладёт в
последний слот, `hand_over` выдаёт получателю его собственные имена.
Добавлены **две строки схемы** и словарь моста, чтобы их исполнить:
`endpoint_receive_call` над селектором 2, производящий запись
`system.ipc.ReceivedCall { reply, carried }`, и `endpoint_call_carrying` над
селектором 3, кладущий делегируемую capability в слот 0. Новых ABI-операций
нет. `Placed` различает «в регистр» и «в слот таблицы переноса»; `Produced`
получил `ReceivedCall`, читающий оба слота.

**Доказательство — асимметрия, а не успех.** Сервер шлёт на собственный inbox
и получает `-1`, потому что `send` там у него нет; тут же шлёт через
capability, пришедшую в вызове, и получает `0`; и затем принимает это
сообщение на `channel`, где он получатель, — значит пришедшая ручка именовала
именно этот endpoint и несла полномочие, которого у сервера иначе не было. Одна
операция, один процесс, мгновение между ними.

Второй раунд — тот же вызов без ничего: слот нулевой, та же отправка отвергнута
`-1`. Объявленный тип принятой capability — **утверждение, которое нуклеус
проверяет при использовании**, и оно падает закрыто. Это измеренный ответ на
вопрос A против C из ADR-0094, а не рассуждение.

**Расстановку ролей выбрало правило одного получателя.** `IPC_V1` §2 разрешает
одного держателя `receive` на endpoint, поэтому сервером стал boot-процесс, а
клиентом — ребёнок. Первая редакция делала наоборот и была отвергнута
нуклеусом с `PROCESS_REFUSED reason=endowment-endpoint-received` — правило
сработало раньше, чем я о нём вспомнил.

**Ещё два наблюдения по ходу.** `launch_plan_endow` пересекает запрошенные
права с правами создателя, поэтому право, которого нет у родителя, — это право,
которого не получит ребёнок: `inbox` пришлось дать `call` именно ради передачи.
И `process_wait_child` в конце отменялся `E_CANCELLED`: перепись находила оба
контекста заблокированными и срабатывало правило живости. Сбор окончания убран
— ни один факт этого бота от него не зависел, а второй вопрос в одном боте
здесь не нужен.

Клиент не получает ничего, кроме `call` на inbox и `send` на channel: ни PCI,
ни окна, ни источника прерывания, ни DMA-региона, ни memory authority — и это
утверждается гейтом по журналу, а не предполагается.

**ADR-0094 остаётся Proposed** как исследовательская запись. Реализация
подтвердила его разбор и не потребовала новой архитектурной семантики;
закрывать его как resolved by existing semantics / minimal textual extension —
решение Архитектора, и оно не принимается здесь.

Устройства в этом срезе нет: ни VirtIO, ни DMA, ни MMIO, ни прерываний.
Следующий шаг — name service P3 на этом же механизме.

### 2026-09-21 — ADR-0093 P3 исполнен: клиент дошёл до сервиса, которого ему не давали

Гейт `name-service.sh`. Четыре канонических текстовых модуля, четыре процесса,
и единственное, что клиенту дали про сервис, — способ спросить.

```text
publisher --call, несущий свой endpoint--> registry
client    --call, несущий свой inbox----->  registry
registry  --send, несущий endpoint------->  inbox клиента
client    --call--------------------------> publisher, трижды
```

**Ответ не может нести capability, и это выяснилось при реализации, а не при
разборе.** `ipc::hand` копирует байты из argument region отвечающего в область
ждущего и таблицу переноса не трогает вовсе. Поэтому lookup отдаёт не ответ, а
канал: спрашивающий передаёт свой inbox, реестр посылает туда
зарегистрированную capability и только потом отвечает. Это обычная
capability-дисциплина, ядро не менялось, и P3 от этого не пострадал.

**Два endpoint'а и есть полномочие публиковать.** Кто дотягивается до
`publish`, тот регистрирует; кто может только `lookup` — нет, и не потому, что
модуль сверяется со списком, а потому что имени у него нет. `CAPABILITY_V1` §6
без единой добавки. Вторую половину доказывает сам клиент: он несёт **свой**
inbox в `lookup`, и это не регистрируется — второй lookup снова выдаёт endpoint
публикатора, и публикатор обслуживает этот вызов тоже.

**Случай B в том же боте.** Реестр обслуживает регистрацию и два lookup'а и
заканчивается. Клиент затем зовёт сервис через уже имеющуюся capability — и она
работает, потому что capability именует объект, а не сервис, который её выдал;
а следующий lookup не работает: на том endpoint'е больше никто не принимает,
и правило живости завершает ожидание `E_CANCELLED`. Существующая семантика IPC,
а не изобретённый случай.

**Ещё раз понадобился `capability_release`, и по той же причине.** `IPC_V1` §2
разрешает одного держателя `receive`, поэтому лаунчер не может отдать ребёнку
`receive`, который сам держит: `grant` отвергает создание целиком. Наделить план
и отпустить своё имя — вот как отдают роль получателя, и план держит
собственную ссылку.

Добавлено: строки схемы `endpoint_send_carrying` и `capability_release` на
`system.ipc.Endpoint`. **Новых ABI-операций нет, ядро не менялось.**

**Найден и закрыт дефект, внесённый мной прошлым коммитом:** гейт
`qemu_capability_transfer` был объявлен в инвентаре, а функция не определена —
правка, добавлявшая обе половины, прервалась между ними. `--list` печатает
такой гейт, поэтому счётчики профилей сходились и parity-гейт был зелёным;
упал бы только запуск, а профиль `qemu` — `full-only`. Теперь
`check-gate-parity.py` требует, чтобы каждая объявленная функция гейта была
определена в скрипте; проверено мутацией.

**ADR-0094 закрыт** как *resolved by existing semantics / minimal textual
extension*: вариант из его §6 не выбирался, потому что реализация показала, что
выбирать было нечего. Открытым в нём **остаётся** вопрос §3: `is_delegable()`
допускает пересылку `Reply`, а `launch_plan_endow` её запрещает; ничего из
построенного reply не пересылает, поэтому решение об этом не принято и не
выдаётся за принятое.

Устройства по-прежнему нет: ни VirtIO, ни DMA, ни MMIO, ни прерываний.

### 2026-09-21 — путь Stage 4 пройден целиком: клиент без единой части машины прочитал настоящий сектор

Гейт `block-service.sh`.

```text
отдельный текстовый клиент -> IPC -> текстовый block service -> DMA
    -> VirtIO -> IRQ -> ответ
```

Четыре канонических текстовых модуля, четыре процесса. Правая половина пути —
Stage 4D-2, без изменений в том, что она доказывает: один настоящий
`VIRTIO_BLK_T_IN` через одну настоящую split-virtqueue, и свидетельство —
сентинель, а не код успеха. Левая половина новая: запрос приходит из процесса,
который не держит ничего от машины и **не имел имени сервиса**, пока реестр
ADR-0093 P3 его не выдал.

**Число, которое переходит границу, — устройства.** Длина запроса — номер
сектора, длина ответа — сколько из 512 байт сектора нулевые, с потолком 256
(`IPC_V1` §3). Сервис заполняет буфер 0xA5 до запроса, так что прогон без DMA
ответил бы 0. Ни одного из этих чисел в клиенте нет, и проверить их иначе он не
может — устройства он не достаёт.

Отчёты: лаунчер 127, реестр 3, **клиент 15**, сервис 140738027323391 —
младшие двадцать бит которого равны 1048575, всем двадцати фактам 4D-2, а
`used.len` = 513.

**Граница capability утверждается, а не предполагается.** `platform.pci.Bus` и
`system.memory.Authority` запрошены ровно дважды каждая — лаунчером, который
передаёт, и сервисом, который пользуется; `platform.pci.FunctionConfig`,
`platform.irq.Source` и `platform.dma.Region` не запрошены никем, потому что
возникают только как результаты операций внутри сервиса.

**ADR-0077 §2 вмешался в устройство среза, и это стоит записать.** План
запуска несёт не больше четырёх capability, а блочному сервису нужно пять:
реестр, два имени своего endpoint'а, шина и полномочие тратить. Пятое поэтому
не едет в плане — лаунчер **посылает** реестр сервису сообщением, уже после
того, как процесс существует, и первый приём сервиса и есть получение реестра.
Capability, приходящая сообщением, а не эндаументом, — ровно тот механизм,
ради которого был предыдущий срез; здесь он понадобился там, где упёрся предел.

Три вещи, найденные машиной, а не рассуждением: права шины и права endpoint'а
не делят ни одного бита (`RIGHT_CLAIM` это `1 << 11`, а не 1), и число не из той
семьи пересекается в ноль; `size` не конвертируется в целое ни в одну сторону
(`docs/40` §3), поэтому счётчик, которому предстоит уехать числом, с самого
начала `u64`; и три объявления `endow_for_launch` над тремя интерфейсами в одном
модуле — это норма, а не коллизия имён.

Добавлено: поле `length` в `system.ipc.ReceivedCall` и строка
`endpoint_call_for` над селектором 3, возвращающая длину ответа вместо того,
чтобы её отбрасывать. Нуклеус не менялся, новых ABI-операций нет.

**Не заявляется:** больше одного запроса через очередь (это 4D-3), запись
(4D-5), durability, переход payload-региона через IPC, рестарт и что-либо о
производительности.

### 2026-09-23 — корректирующий раунд по внешнему аудиту: длина сообщения, полномочие публикации, заявления

**Внешний аудит трёх последних коммитов (`6c17b10`, `b276fde`, `89f7599`)
подтвердился по всем пяти пунктам.** Фундамент не сломан; сломаны были протокол
блочного сервиса, формулировки и машинная проверяемость открытого. Ниже — что
исправлено, что заблокировано и что не заявляется.

**1. Длина сообщения перестала быть транспортом для чужого числа.** Срез
2026-09-21 посылал номер сектора как `length` запроса и число нулевых байт как
`length` ответа. Это недопустимо как протокол: ядро копирует ровно столько байт,
сколько объявлено, `IPC_V1` §3 ограничивает это 256 байтами, и сообщение, чей
объявленный размер не равен его размеру, не является сообщением этого контракта.
Побочный симптом — счёт приходилось срезать до 256, то есть ответ «целый нулевой
сектор» был числом 256, а не 512.

Исправлено средствами уже существующего контракта, без новой ABI-операции и без
изменений ядра: число едет там, где §3 держит payload. Добавлены `Slot::Word`
(восемь little-endian байт по `MESSAGE_PAYLOAD`, длина — восемь, и регистр длины
модулю недоступен), `Produced::Answer`, запись `system.ipc.Answer { length,
word }`, поле `word` у `system.ipc.ReceivedCall`, строки `endpoint_call_word`
(селектор 3) и `endpoint_reply_word` (селектор 4). Строка `endpoint_call_for`,
выдававшая длину ответа как ответ, **убрана** — она и была той affordance.

Клиент теперь сначала проверяет, что длина ответа равна восьми, и только потом
читает payload; сервис так же проверяет длину запроса и отказывает до того, как
тронет устройство. Отчёт клиента — 31 вместо 15, и в нём `answer.word == 512`:
число, которое **нельзя** было провезти длиной, потому что 512 вне границы §3.
Это и есть регрессия, которая кусается.

**2. Полномочие публикации — STOP, решение не принималось.** ADR-0093 §3a.1–
§3a.2, `CAPABILITY_V1` §6 и ADR-0051 §2 требуют, чтобы право публикации было
capability, **чей номинальный тип — публикуемый интерфейс**. Реализовано другое:
публикатор держит обычный `system.ipc.Endpoint` под привязкой `publish`, имя
интерфейса в протоколе не едет вовсе, и `block.device.v1` в дереве не называет
никто. Реализация — подозреваемая, ADR — нет.

Сделать принятое сейчас нельзя без решения Архитектора, и это показано, а не
заявлено: ни одна принятая схема не объявляет пути публикации, а
`SYSTEM_INTERFACE_V1` §2 делает такой путь **новым Tier 2 контрактом**; какой
object kind стоит за такой capability — не решено (`OBJECT_INTERFACE = 4`
зарезервирован §2 и оставлен пустым P3); а текстовый реестр не может проверить
род того, что ему передали сообщением (ADR-0094 §11). Поэтому:

- обязательный негативный гейт — «сервис без publication capability не может
  зарегистрировать `block.device.v1`» — записан **BLOCKED**, а не зелёным: у
  него сейчас нет субъекта, `block.device.v1` не является именем, которое
  что-либо в системе может зарегистрировать или отказаться зарегистрировать, и
  единственная форма отказа, доступная сегодня, — отсутствие generic `publish`,
  которую аудит справедливо не считает доказательством;
- все формулировки, утверждавшие, что два endpoint'а **и есть** `CAPABILITY_V1`
  §6, отозваны — в `nameservice.tos` (оба фикстура), в `name-service.sh`, в
  выводе гейтов и здесь. Запись 2026-09-21 не переписана; она верна на свою
  дату, а вот её утверждение «`CAPABILITY_V1` §6 без единой добавки» неверно и
  настоящей записью отзывается;
- разрыв, четыре варианта и то, что каждый затрагивает, — в
  `docs/research/PUBLICATION_AUTHORITY_CONFLICT.md`, который ничего не
  принимает; ADR-0093 получил датированный §11, а вопрос — машинно-проверяемый
  идентификатор ADR-0093-Q1.

**3. Заявления среза приведены к тому, что гейт правда доказывает.** Клиент не
читал сектор: 512 байт блочных данных через IPC не проходили и не проходят.
Проходит одно число, которое сервис вычислил после настоящего DMA/VirtIO/IRQ
чтения. Граница `docs/research/STAGE4_DATA_PATH_BOUNDARY.md` §1 — память клиента
через IPC в DMA-память сервиса — **не достигнута**, и перенос региона из
канонического текста не существует (ADR-0094 §10). Заголовок коммита
2026-09-21 «путь Stage 4 пройден целиком» и строка гейта «client reads a real
sector» были завышением; гейт теперь говорит, что клиент, не держащий ничего от
машины, получил число, происходящее от устройства, и отдельно перечисляет, чего
это не доказывает. Ценность среза сохранена полностью.

**4. Открытое больше нельзя потерять машинно.** ADR-0094 одновременно говорил
«No option was chosen» и «the outcome is A» — противоречие, и аудит прочитал его
верно. Формулировка приведена к одной: исход — вариант A §6.1, полученный
реализацией, а не взвешиванием, и строка статуса это и говорит; §11b — датированная
поправка. Отдельно: §3 ADR-0094 оставил открытым, что `is_delegable()` допускает
`Reply`, а `launch_plan_endow` его запрещает, — а `check-open-decisions.sh`
считал ADR-0094 полностью закрытым. Причина в том, что вопрос переживает
решение: ADR закрылся в день, когда был поднят. Теперь ADR несёт строки
`- Open question: ADR-NNNN-QN — …`, гейт читает их **независимо от статуса ADR**
и сверяет с фенсом ```open-questions ниже. Проверено удалением строки из фенса —
гейт краснеет.

**5. README и текущий статус приведены в соответствие с деревом.** README
называл фронтиром 4D-3 и утверждал «нет блочного сервиса, нет пути записи»,
тогда как в дереве уже 4D-4, 4D-5 и срез сервис/клиент. Исправлено без
завышения; раздел «Текущая позиция» этого файла получил отдельный
present-tense блок: что построено, что доказано гейтами, что не доказано, что
формально закрыто, что только built/green и какой следующий честный фронтир.

**Архитектурных изменений нет.** Ядро не менялось, ABI-операций не добавлено,
новых механизмов IPC нет, `OBJECT_INTERFACE` по-прежнему зарезервирован и пуст,
Stage 4C/4D и Stage 4 не закрываются.

Зелёное: `block-service` (127/3/**31**/140738027323391), `name-service`
(127/7/15/255), `preflight --profile docs`, `--profile selftest`,
`--profile source`, SPDX, `cargo fmt`, `cargo test`, gate parity.

### Открытые вопросы — машинно-проверяемый список

**Второе и последнее место в этом файле, которое говорит в настоящем времени.**
Вопрос, который ADR поднял и не закрыл, не движется вместе со статусом ADR —
ADR-0094 закрылся в день, когда был поднят, и оставил §3 открытым. Поэтому
список открытых ADR отдельно от списка открытых вопросов, и
`scripts/check-open-decisions.sh` сверяет оба: строки `- Open question:` в
`docs/adr/` против фенса ниже, **независимо от строк `- Status:`**.

```open-questions
ADR-0094-Q1
```

- `ADR-0093-Q1` — **закрыт 2026-09-23 решением ADR-0095.** Вопрос закрыт не
  реализацией, а сужением контракта: `CAPABILITY_V1` §6 теперь требует
  publication authority как выделенный канал, а не как capability с номинальным
  типом публикуемого интерфейса. Расхождение, которое вопрос описывал, перестало
  существовать. Историческая запись — ADR-0093 §11a.
- `ADR-0094-Q1` — `is_delegable()` допускает `Reply`, `launch_plan_endow`
  запрещает; можно ли пересылать право ответить из канонического текста, не
  решено. До решения ни одна текстовая поверхность не помещает `Reply` в таблицу
  переноса сообщения.

### 2026-09-23 — hardening: два новых enforcement gate'а были неверны на состоянии, которого сегодня не бывает

**Внешний review `main` нашёл два дефекта в гейтах, добавленных корректирующим
раундом. Оба подтвердились.** Ни один не проявлялся на текущем дереве — и это
ровно та причина, по которой оба требовали self-test, а не повторного чтения.

**1. `stage4_data_path_claims` снимал ограничение по неверному условию.**
Комментарий гейта утверждал два независимых доказательства одного факта, чтобы
одно не могло устареть отдельно от другого, а реализация писала
`schema != 0 OR bridge != 0`: одной декларации без реализации — или реализации
без принятой строки — хватало, чтобы снять bound. Кроме того детектор считал
`platform.dma.Region` и `Placed::Transfer`, а ни то, ни другое не является
payload-транспортом границы:

- `DmaRegion<T>` и `DmaRegion<mut T>` по таблице ADR-0037 не shareable и не
  transferable ни в одном режиме — это то, что заполняет **сервис**, а не то,
  что посылает клиент (`IPC_V1` §5, research-заметка §1);
- `Region<mut T>` вообще нельзя послать (`IPC_V1` §5);
- `Placed::Transfer` — это `MESSAGE_CAPABILITIES` с собственным счётчиком и
  границей 4 (ADR-0058, `IPC_V1` §6), а регионы едут в **отдельной** области
  `MESSAGE_REGIONS` со своим счётчиком и границей 2. Переход capability возможен
  с ADR-0094 и payload'ом не является;
- а все сегодняшние обращения к `MESSAGE_REGIONS` в runtime image стоят под
  `#[cfg(feature = "test-…")]` — это Rust-нагрузки, которые ADR-0094 §0 прямо
  отказывается считать заменой каноническому тексту.

Условий теперь три, и они **И**, а не ИЛИ: принятая строка схемы с обычным
immutable `Region<...>` у IPC-интерфейса; обращение к `MESSAGE_REGIONS` из
всегда компилируемого кода (типизированный мост не под cfg, нагрузки — под ним,
поэтому детектор не угадывает имя будущего `Slot`/`Placed`); и наличие гейта
`region_ipc_payload`, объявленного **и** определённого в инвентаре. Третье — тот
самый маркер, который будущая реализация обязана переключить сознательно и
вместе со своим conformance-тестом, потому что в этом проекте свидетельство
следует за зелёным гейтом, а не наоборот. Будущий textual region surface здесь
не проектируется; пока его нет, гейт fail-closed.

**2. `check-open-decisions.sh` не умел состояния «ноль».** Оба списка читались
через «извлечённый текст непуст», поэтому корректное будущее состояние — ни
одного открытого ADR и ни одного открытого вопроса — было невыразимо: гейт
покраснел бы в день, когда закрылось последнее решение, и очевидным выходом
оказалось бы его ослабить. Теперь три состояния — три состояния: фенса нет —
FAIL; фенс есть и пуст — допустимо, если authoritative set тоже пуст; фенс
непуст — должен совпадать точно. Счётчики в сообщении об успехе считаются
`grep -c .`, а не `wc -l`, который для пустой строки даёт единицу.

**Третий экземпляр того же дефекта нашёл сам self-test, а не чтение.**
`grep -h '^- Open question: '` возвращает 1, когда не нашёл ничего, и под
`set -e` это завершало гейт целиком — то же нулевое состояние на слой ниже.
В дереве с двумя вопросами это не проявлялось никогда.

Заодно исправлен устаревший комментарий в `check-open-decisions.sh`: он всё ещё
говорил, что ADR-0094 закончился без выбора option из §6, тогда как §11b теперь
явно фиксирует исход как вариант A §6.1.

Добавлены два self-test'а в профиль `selftest`, по образцу
`check-gate-parity.sh`: миниатюрные репозитории, полная таблица истины и
дискриминатор, который не читает сообщение проверяемого гейта. Для
claims-гейта — десять отказов и два принятия; для open-decisions — семь отказов
и два принятия. Оба проверены мутациями самих гейтов: возврат к ИЛИ, возврат
подсчёта `DmaRegion`, приём cfg-нагрузки, возврат к «непустой фенс», снятие
`|| true` и снятие проверки, что вопрос называет существующий ADR — каждая
краснеет в своём кейсе.

**Ничего не начато:** region transfer, ADR-0093-Q1, publication authority, новый
механизм IPC, новая ABI-операция, закрытие этапа. Это только hardening.

Зелёное: `preflight --profile docs` (16), `--profile selftest` (16),
`--profile source` (13), `--profile provenance` (3), gate parity (108).

### 2026-09-23 — ADR-0095: полномочие публикации — выделенный канал, а не номинальный тип

**Направление Архитектора, и это сужение принятого контракта, а не реализация
фичи.** `CAPABILITY_V1` §6 требовал, чтобы номинальный тип publication authority
был публикуемым интерфейсом. Исследование (`0a320a4`) показало, что это требует
проверки, которой в системе нет ни на одном слое: `launch_plan_endow` пишет
object, rights и binding, launch record — то же, а startup-проверка сравнивает
**род объекта**. Поэтому §6 приведён к тому правилу, которое архитектура
действительно обеспечивает.

**Принято (ADR-0095).** Право публикации — capability, именующая **выделенный
publication object**, чья идентичность фиксирует, что через неё можно
опубликовать. Владение такой capability даёт право публиковать через этот канал;
отсутствие — нет. Ни объявление в исходнике, ни строка в payload полномочия не
дают. Для Stage-4 реестра P3 этот объект — Endpoint: реестр держит `receive`,
авторизованный публикатор — `call`. Имя интерфейса в протоколе не едет, потому
что этот endpoint представляет ровно один класс публикации.

**Явно не утверждается**, что номинальный интерфейс сохраняется ядром или планом
запуска. Явно не обобщается на второй публикуемый интерфейс: объект, чья
идентичность фиксировала бы два интерфейса, не фиксировала бы ни одного
(ADR-0095 §6).

**Реализация не менялась — и это и есть форма такого решения.** Дерево уже строит
ровно эту топологию. Не хватало двух вещей: чтобы контракт это говорил, и
негативного свидетельства.

**Негативное свидетельство: `publication-authority.sh`.** Два бота одной капсулы,
отличающиеся ровно одним эндаументом. Отказ: претендент запрашивает publication
endpoint под той же привязкой, под которой его запрашивает авторизованный
публикатор, ничего на этот запрос не отвечает, и он отвергнут **до первой
инструкции** —
`TOS.RUN.REFUSED stage=execute reason=capability-denied binding=publish interface=system.ipc.Endpoint`,
а реестр, который держит `receive` на этом endpoint'е, сообщает, что его ожидание
завершилось ни с чем (правило живости, `E_CANCELLED`), — то есть ничего не было
опубликовано, и это утверждение, а не отсутствие. Разрешение: тот же модуль, тот
же исходник, та же привязка, тот же реестр плюс `call` на этом endpoint'е — он
стартует, публикует, реестр принимает, отказов в боте нет.

**Мутация встроена в гейт, а не описана в комментарии** (ADR-0095 §5.3): гейт
выполняет оба бота, поэтому негатив, который выжил бы после выдачи capability,
краснеет сам. Проверено ещё и снаружи: выдача `call` в базовом фикстуре — «не
было отказа вовсе».

**ADR-0093-Q1 закрыт** (ADR-0093 §11a): расхождение, которое он описывал,
перестало существовать. Research-заметка помечена как история и не переписана.

**ADR-0096 (Proposed) — то, что осталось от исследования и Stage 4 не блокирует:**
должен ли номинальный интерфейс сохраняться при делегировании и эндаументе, когда
два интерфейса делят один род объекта. Сегодня вопрос недостижим — соответствие
интерфейс → род объекта **инъективно** по всем десяти принятым интерфейсам; если
бы такая пара была, обе стороны retyping'а прошли бы; Stage 4 такой пары не
создаёт; и закрытие Stage 4 от ответа не зависит.

**Не введено:** `block.device.V1Publisher`, Q1-A, Q1-A2, идентичность интерфейса
в записях плана запуска, изменения `SYSTEM_ABI_V1` операции 22,
`OBJECT_INTERFACE`, реестр в ядре, namespace имён интерфейсов, маршрутизация по
имени в payload, поддержка второго публикуемого интерфейса.

Зелёное: `publication-authority` (7 / 2 / отказ; затем 7 / 1 / 64),
`name-service` (127/7/15/255), `block-service` (127/3/31/140738027323391),
`preflight --profile docs` (16), `--profile selftest` (16),
`--profile provenance` (3), gate parity (109).

### 2026-09-23 — срез данных Stage 4D остановлен: текстовой поверхности региона нет, и добавить её нельзя без решения

**Архитектор назначил следующей целью реальный путь данных** — 512 байт сектора
переходят границу клиент/сервис через принятый region-механизм — исходя из того,
что он «уже предусмотрен `IPC_V1`, `MESSAGE_REGIONS`,
`STAGE4_DATA_PATH_BOUNDARY.md` и существующей region-машинерией».

**Про транспорт это верно, про текстовую поверхность — нет.** Транспорт готов со
Stage 3: `qemu_region_transport` переносит регион между процессами сегодня.
Отсутствует то, чем канонический текстовый модуль его называет, и добавить это
нельзя без решения уровня ADR-0085.

**Доказательство, а не мнение (ADR-0097 §2).** Две лазейки действительно есть:
результат region-порождающей операции представлением §4.3 не ограничен (у DMA
это `Result<DmaRegion<mut u8>, i64>`), а произведённая capability может ехать
**значением** параметра — так `launch_plan_seal` принимает sealed plan. Но одна
операция от capability-позиции уйти не может: `IPC_V1` §5 запрещает посылать
`Region<mut T>` вовсе, поэтому регион обязан стать неизменяемым через операцию 18
или 7, а обе берут регион **своей** capability в `rdi`. По §4.1 её объявленный тип
параметра — путь интерфейса, а по §4.3 аргумент в этой позиции должен принадлежать
семейству, допускающему `TypeDef::Region(_)`. В закрытом перечислении два члена, и
ни один им не является: `AsInterface` — это ровно `Capability(path)`, который
индексировать нельзя, а `DmaRegionFamily` по правилу кардинальности 1 не может
принадлежать второму интерфейсу.

Значит нужен **новый член закрытого перечисления**, а `SYSTEM_INTERFACE_V1` §4.3
говорит прямо: «adding a member is a decision of ADR-0085's weight». ADR-0085 —
Level 3, утверждён Архитектором, TOS Core 1.3. И по его же §13 это берёт минор
языка: позиция capability, которую соответствующий контракту фронтенд и верификатор
до поправки отвергают, становится валидной.

**Поэтому написан ADR-0097 (Proposed), а не срез.** Минимум, чтобы утверждение
было в одном шаге от реализации: один член представления, интерфейс
`system.memory.Region` (object kind `Region` — он у ядра уже есть; не
startup-importable по производному правилу §4.3), четыре строки схемы над
операциями 17, 18 и 1, и словарь моста для области `MESSAGE_REGIONS`. **Ядро и ABI
не меняются, операций не добавляется.** Варианты R-A / R-B / R-C и рекомендация
R-A — как рекомендация.

**R-C записан, чтобы его отклонение было на бумаге**: 512 байт двумя inline-
сообщениями по 256 дали бы зелёный гейт и неверное утверждение — `IPC_V1` §5
делает payload регионом, который ядро отображает и не копирует через себя, а этот
вариант копирует его через ядро дважды на каждые 256 байт. Ровно тот класс
дефекта, который три предыдущих раунда убирали.

Ничего не реализовано, ни один принятый контракт не правлен, схема, ABI, ядро,
верификатор и launch-машинерия не тронуты.

### 2026-09-23 — ADR-0097 принят и реализован: все 512 байт сектора переходят IPC регионом

**Принят как R-A с тремя уточнениями** (§8): семейство называется `RegionFamily`;
противоречие «any element type» против «u8 only» разрешено; строка 4 §3 решена в
пользу **отдельной** receive-поверхности, а не поля в `system.ipc.ReceivedCall`.
Level 3, TOS Core **1.5**.

**§8b.** `RegionFamily` — это языковое *семейство представления*, поэтому его
членство — над `TypeDef::Region(_)` и `TypeDef::RegionMut(_)` при любом типе
элемента: именно это решает, какие значения могут занимать capability-позицию.
Каждая принятая *строка* называет `u8` и только его, и ни одна не авторизована для
другого типа элемента. Никакой schema polymorphism.

**§8c.** `ReceivedCall` — это запись, которую читают поверхности передачи
capability и публикации, и её четыре поля, сопоставляемые по позиции, входят в то,
что доказывают уже зелёные боты. Пятое поле изменило бы запись у всех них ради
формы сообщения, которой нет ни у одного. Поэтому `endpoint_receive_region` —
своя строка над селектором 2, отдаёт один регион и ничего больше, и **fail
closed**: пустой слот региона или непригодный extent — это `E_NO_CAPABILITY`, а не
регион нулевой длины.

**Реализовано:** `RegionFamily` в таблице фронтенда и в независимой таблице
верификатора; интерфейс `system.memory.Region` (object kind `Region`, не
startup-importable); `region_allocate` над 17 с семью grant-фактами `docs/42` §2;
`region_freeze` над 18; `endpoint_send_region` над 1 через `MESSAGE_REGIONS` и свой
регистр счёта; `endpoint_receive_region` над 2; `endpoint_call_word_carrying` над
3 — обе размещённости уже существовали; `Placed::Region` и три результата в мосте,
с регистрацией отображения при выделении и при получении, перефайлингом
read-only при freeze и **снятием при передаче**: линейный случай `IPC_V1` §5
требует, чтобы отображение хозяина ушло вместе с хэндлом, иначе отправитель
продолжал бы читать память, которую отдал. **Семантика production-ядра и ABI не
менялись, нового object kind нет.**

**Формулировка «ядро не менялось» была бы неверна буквально, и это стоит
предложения.** Этому conformance-боту нужен свой launcher-constant — Cargo-фича
`test-region-transfer-text` и эндаумент, который она строит, — как у каждого
QEMU-гейта в этом дереве. Это evidence plumbing: production-артефакт ядра
собирается без этих фич, каждый гейт проверяет, что его хэш не изменился, пока
собирается изолированный, а `check-feature-builds.sh` типизирует каждую фичу.
Не менялось то, **что ядро делает**: ни `syscall.rs`, ни `capability.rs`, ни
`ipc.rs`, ни `region.rs`, ни `plan.rs` не ведут себя иначе, номера операций не
добавлены и не переопределены, `OBJECT_INTERFACE` остаётся зарезервированным и
пустым. ADR-0097 §10.

**`region-transfer-text.sh`** — conformance-гейт ADR-0097: отправитель выделяет
регион, пишет `i * 7 + 11 mod 256`, замораживает, посылает; читатель держит одно
принимающее имя и **никакого полномочия памяти** — он не мог сделать регион — и
индексирует каждый байт по адресу, который выбрало ядро и который он не узнаёт.
Отчёты 63 и 512. Три негатива, каждый своим ботом: `Region<mut u8>` отвергается
**транспортом** с `E_NO_CAPABILITY` и регион не приходит; обычный регион
отвергается на позиции `platform.dma.Region`; модуль 1.4, называющий
`system.memory.Region`, отвергается против минора **5**, а не 3.

**`block-data-path.sh` — граница §1 достигнута.** Клиент просит сектор и передаёт
канал одним сообщением; сервис выполняет настоящее чтение VirtIO/DMA, копирует
байты один раз из device-visible памяти в обычный регион — та самая копия, которую
ADR-0037 делает вынужденной, — замораживает и посылает через `MESSAGE_REGIONS`;
клиент индексирует все 512 байт. Сектор **1**, а не 0: сектор 0 заполнен нулями, а
ноль — это то, как выглядит нетронутая память. В буфере сервиса эти байты были
0xA5, пока их не записало устройство, и оба модуля вычисляют ожидаемый байт из
запрошенного сектора, а не берут из таблицы. Отчёты: лаунчер 127, реестр 3,
**клиент 31**, сервис 140738027323391 — все двадцать device-side фактов 4D-2 на
месте.

**Хост не трогает данные.** Гейт читает два числа; он не читает регион, не копирует
ничего через текстовую границу и не считает контрольную сумму вместо клиента.
Единственное, что он кладёт на диск, — содержимое диска.

**Три дефекта нашла работа, а не разбор.** У фронтенда было второе зеркало
производной представления — `interface_of_representation`, — которого план не
называл. Feature-gate читал `if minor < CAPABILITY_REPRESENTATION_MINOR`: это было
верно при одном семействе и молча пропускало весь обход, когда второму
потребовался более высокий минор — модуль 1.4 принимался, потому что 4 не меньше
3. Сравнение теперь на каждую находку. И детектор схемы в
`check-stage4-data-path-claims.sh` искал value-параметр — форму, которую §2b
показал возможной, а §3 не выбрал, — и поэтому читал ноль против схемы, которая
несёт нужное.

**Claim-bound снялся сам, по своим трём условиям:** одна строка схемы, пять
обращений к `MESSAGE_REGIONS` из всегда компилируемого кода и гейт
`region_ipc_payload`.

**`check-interface-schema.sh` ужесточён до per-operation:** каждый grant origin
несёт свою таблицу `| Fact | <op> |`, а строка, возвращающая регион без
порождения гранта, говорит об этом строкой, которую гейт читает —
`region_freeze` переиздаёт, `endpoint_receive_region` доставляет.

Мутации: возврат к ИЛИ; подсчёт DMA-регионов; угадывание имени семейства вместо
чтения; заявление, что `region_allocate` не порождает грант; удаление факта из его
таблицы; порча одного байта из 512 сервисом (клиент отчитывается 15, гейт
краснеет); запрос сектора 0 (проверка сервиса краснеет) — каждая красит свой
кейс.

Зелёное: `block-data-path`, `region-transfer-text`, `capability-transfer`,
`name-service`, `block-service`, `publication-authority`;
`preflight --profile docs` (16), `--profile selftest` (16), `--profile source`
(13), `--profile provenance` (3), gate parity (111), `cargo test`.

**Stage 4C, Stage 4D и Stage 4 не закрыты.** Осталось: случай C ADR-0093,
crash/reset lifecycle, durability сверх 4D-5, отчёт о производительности,
актуализация документации, closure review.

### 2026-09-23 — сервис умер, успешник перезапустил путь к устройству, данные сохранились

**Случай C ADR-0093 и T1-восстановление ADR-0092 R1a, одним ботом**
(`block-lifecycle.sh`). Четыре канонических текстовых модуля, **пять процессов на
четырёх слотах**, один сектор. Новых ADR нет, новых фреймворков нет.

```text
registry, экземпляр A и клиент запускаются
клиент --lookup--> endpoint A
клиент --регион и вызов--> A --VIRTIO_BLK_T_OUT--> устройство
A заканчивается, всё ещё держа функцию, окно, источник и DMA-регион
супервизор: wait_child(A) -> **отзыв публикации A** -> и только потом B
B: DEVICE_STATUS <- 0, обычная инициализация VirtIO заново, очередь/DMA/IRQ/окно,
   повторная публикация
клиент --свежий lookup--> endpoint B (это не endpoint A)
клиент --регион и вызов--> B --VIRTIO_BLK_T_IN--> устройство
клиент: **все 512 байт — образец, который записал A**
клиент --вызов по endpoint A, пока B ждёт--> отменён, не обслужен
```

Отчёты: супервизор 63, реестр 31, **клиент 127**, писатель
140737489928191 (19 device-side фактов, `used.len` = 1), читатель
144255926103179263 (все 20, `used.len` = 513, плюс бит «второе ожидание
отменено»). Generation назначения 1 и 2 — не одна.

**Отзыв публикации — отдельный путь и отдельное доказательство.** Реестр держит
три авторитета на трёх endpoint'ах: `publish`, `lookup` и `withdraw`. Последний
принадлежит только супервизору — это уведомление, которое ADR-0093 §3b допускает
«от того, у кого есть» надзорное отношение, и отдельный канал, а не второе
значение на канале публикации. Отзыв — **вызов**, и ответ реестра приходит уже
после `capability_release` той capability, которую он держал для A: то есть на
момент, когда супервизор вправе создать успешника, у реестра нет ни одного имени
ни для какого сервисного endpoint'а. Состояние «A уже отсутствует, B ещё не
опубликован» доказано порядком: BEGIN A -> PROCESS_RECLAIMED ->
`capability_release status=0` -> BEGIN B, проверено одним проходом по журналу.

**Устаревшее имя против живого B — это эксперимент, а не уборка.** Клиент держит
имя, которое выдал первый lookup; B в этот момент **ждёт** запроса; вызов не
обслуживается. Правило живости отменяет оба ожидания, и отменённое ожидание B —
это и есть доказательство, что ничего не пришло: если бы capability
«отремонтировали», B бы ответил. ADR-0093 §3a.5.

**Уборка после A — путь смерти процесса, не явного release.** Модуль ничего не
отдаёт: живое DMA-подкрепление уходит в карантин, а не в пул; назначение
дренируется (memory decoding и bus mastering выключены); только потом run
признаётся безопасным, возвращается и заряд возмещается; вектор источника
прерываний снят; процесс переработан без карантинных кадров. Порядок — это и есть
свидетельство.

**T1-сброс прочитан как ответ устройства.** `PROVED_RESET` ставит только модуль,
который записал 0 и **прочитал `device_status` обратно нулём** (§4.1.4.3.2) прежде
чем тронуть что-либо ещё, — и бит проверяется у каждого экземпляра отдельно, а не
выводится из того, что позднее что-то удалось прочитать.

**Сектор 8 и собственный образец.** Сектора 1–4 эталонного образа засеяны, сектор
0 нулевой — свидетельство в любом из них мог бы удовлетворить сам образ. Сектор 8
не тронут, образец `(i * 37 + 91) % 251 + 1` не равен нулю нигде, и клиент считает
все 512 байт **в каноническом тексте**: гейт читает два числа и не читает регион.

**`MAX_ENDPOINTS` поднят с 4 до 6 вместе с consumer и тестом.** Шесть объектов
нужны одновременно и каждый несущий: три авторитета реестра, по одному endpoint'у
на экземпляр (они обязаны быть **разными** объектами, иначе успешник и был бы
отремонтированной capability предшественника) и inbox клиента. Это размер таблицы,
а не число контракта: три числа `IPC_V1` §3 ограничивают *сообщение* и не тронуты,
а правило §7 — что очередь не растёт, — статически зарезервированная таблица
большего размера не нарушает. Семантика и ABI не менялись.

**Одна вещь, которую нашла работа, а не план.** Runtime image после возврата из
`main` упражняет **первую** endowed capability процесса, и для endpoint'а с
`receive` это блокирующий приём. Экземпляр обязан **закончиться**, когда его работа
сделана — `wait_child` супервизора ждёт именно этого, — а процесс, застрявший в
собственном эпилоге, census живости считает заблокированным и бот вставал вместо
перезапуска. Поэтому в плане сервиса первым идёт полномочие памяти: его упражнение
ни на чём не блокируется. Порядок записей плана стал значимым фактом, и это
записано там, где он задаётся.

Мутации: вызов устаревшего имени, направленный вместо этого на живого успешника
(гейт краснеет); один неверный байт из 512 (клиент отчитывается не 127); успешник
не ждёт второго запроса (гейт: «не сообщил, что второе ожидание отменено, значит
устаревший вызов мог до него дойти»).

**Не заявляется:** durability при потере питания, `VIRTIO_BLK_F_FLUSH`, fsync,
durability при неаккуратном завершении хоста, crash-consistent семантика ФС
(ADR-0092 выносит всё это за Stage 4), exactly-once, случай D ADR-0093, падение в
полёте, third-party reset T3, больше одного сектора, больше одного клиента.
**Stage 4C, Stage 4D и Stage 4 не закрыты.**

Зелёное: `block-lifecycle`, `block-data-path`, `region-transfer-text`,
`block-service`, `name-service`, `publication-authority`, `capability-transfer`;
`preflight --profile docs` (16), `--profile selftest` (16), `--profile source`
(13), `--profile provenance` (3), gate parity (112).

### 2026-09-23 — ADR-0098 и ADR-0099 предложены: протокол `block.device.v1` и первое персистентное хранилище

**Раунд только проектирования. Ничего не реализовано, оба ADR — `Proposed`.**

Исследование границы (`docs/research/STAGE4_PERSISTENT_STATE_BOUNDARY.md`)
принято как основа, и Project Architect вынес решения по всем открытым
альтернативам; §0b заметки фиксирует, какие из них закрыты и где решены.

**ADR-0098** отвечает на §9 ADR-0093: форма провода `read`, `write`, `capacity`.
Все три операции в v1 — `capacity` не отложена, потому что ограниченный клиент
обязан убедиться, что его раскладка влезает, **до** первой записи, а не узнавать
границу устройства, намеренно посылая запрос, который должны отвергнуть.
Двухсообщенная запись фикстуры (`send(region)`, затем `call(word)`) **не**
повышается в протокол: она корректна только при единственном
последовательном клиенте. Вместо неё — атомарный вызов, несущий слово и регион в
одном сообщении, поверх того, что операция 3 уже умеет: `call` передаёт
`frame.r8` как счётчик регионов, `send_transaction` их линейно переносит,
`write_regions` сообщает их получателю. Новых операций ABI, механизмов нуклеуса,
видов capability и объектов, границ IPC — нет; есть две additive строки схемы и
одна запись.

**ADR-0099** — substrate first: приватное нативное хранилище объектов, `/state`
не реализуется и не переопределяется. `u64` id в домене `1..64`,
детерминированное размещение, версионированный заголовок в секторе 0 с битовой
картой занятости, ровно 512 байт полезной нагрузки на объект и **никаких**
заголовков в объектах. Присутствие определяет только битовая карта — поэтому
засеянное или устаревшее содержимое сектора не является авторитетным. Регионы
пересылаются линейно, без второй копии; запись заголовка — отдельный блочный
запрос.

**Найдено при драфтинге и зафиксировано как расхождение:** `docs/21` относит
«changes persistent formats» к Level 3, и ADR-0017 применил этот же тест к себе
словами «Explicitly **not** Level 3: no capsule byte changes». ADR-0099
**создаёт** первую персистентную раскладку. Project Architect направил Level 2;
§0a черновика излагает аргумент за Level 3 и оставляет классификацию за ним.

Предложенные контракты лежат в `docs/proposed/`, а не в `source/interfaces/`:
правило допуска ADR-0020 и гейт `check-interface-contract-authority.sh` требуют
от всего в том дереве принятого статуса и записи в манифесте, так что черновик,
положенный туда, был бы авторитетом, который документ присвоил себе сам.

### 2026-09-24 — исправление черновиков ADR-0098/ADR-0099: семь правок, ни одной реализации

**Раунд только проектирования.** Оба ADR остаются `Proposed`.

**ADR-0099 — Level 3.** Прежнее направление на Level 2 отозвано: `docs/21`
относит изменение персистентных форматов к Level 3, а создание первого
нормативного формата состояния не менее архитектурно, чем его изменение. Язык
разногласия из черновика убран. ADR-0098 остаётся Level 2.

**Идентичность endpoint-объекта — не версия протокола.** Прежнее обоснование было
неверным: выделенный объект ADR-0095 §3 фиксирует класс публикации и полномочие, а
endpoint, опубликованный затем как сервисный, — другой объект. Свидетельство
`block-lifecycle.sh` уже это доказывает: два разных сервисных endpoint'а у двух
поколений, и оба реализуют один `block.device.v1`. Решение «в запросе нет поля
версии» сохранено, но по правильной причине — контракт сервиса сконфигурирован
топологией, а runtime-согласования версий в v1 нет.

**Обе необязательные позиции — честно необязательные.** `carried` стал
`Option<system.ipc.Endpoint>`: соответствующие `WRITE` и `CAPACITY` не несут
endpoint для ответа, и отсутствие должно быть отсутствием, а не нулевым
дескриптором. Проверка, которую требовало направление, **пройдена**: новых членов
семейства представления не нужно (перечисление §4.3 закрыто и не меняется),
языкового minor не нужно (`RegionFamily` допущен с TOS Core 1.5), а
`Option<system.ipc.Endpoint>` разрешается текущим фронтендом без изменений.
Зафиксированы два обязательства реализации: одна ветка для `Region<u8>` в
`schema_field_type` и перенос представленческого minor'а с полей schema-записи на
модуль, который её называет.

**Порядок READ/GET перевёрнут: сначала ответ, потом регион.** Прежний порядок
допускал осиротевший регион в очереди endpoint'а, который переживает процесс:
объект endpoint живёт всю загрузку, и смерть держателя не делает его новым.
Коды `BLK_ANSWER`/`ST_ANSWER` отозваны — отказ после отправленного успеха
невозможен; вместо них проверяемое до чтения отсутствие endpoint'а для ответа.

**Форматирование отделено от запуска сервиса.** Ребёнок не знает своего
restart generation — его утверждает супервизор, — поэтому сервис не отличает
первый запуск от рестарта с нечитаемым заголовком и не должен автоформатировать.
Заголовок пишет отдельный короткоживущий текстовый инициализатор; обычный сервис
только открывает. Границы пересчитаны заранее: семь экземпляров процессов, пик 4
слота, ровно 4 плана (клиентский — общий для писателя и читателя), 4 endpoint'а
из 6. `MAX_PLANS` не поднимается.

**Размер объекта отделён от протяжённости региона** (512 байт против «не меньше
кадра»), и `STATE_STORE_V1` больше не «владеет устройством»: он резервирует ровно
65 секторов и никогда не читает и не пишет сектор ≥ 65, а остальным не назначает
ни смысла, ни владельца.

### 2026-09-24 — ADR-0098 и ADR-0099 приняты; контракты промотированы в `source/interfaces/`

**Документация и принятие. Ни один протокол не реализован.**

Три последние правки перед принятием:

- **инициализатор отказывается форматировать любой ненулевой заголовок.** Он
  читает сектор 0 и пишет только если все 512 байт нулевые. Уже валидный
  заголовок, валидный но чужой схемы, неизвестного формата, повреждённый или
  просто мусорный — **не разрешение на инициализацию**. Проверка намеренно строже
  обычной валидации при открытии: открытие спрашивает «пригоден ли этот store»,
  форматирование — «точно ли здесь ничего нет». Секторы 1..64 перед
  форматированием не осматриваются: харнесс часть из них намеренно засеивает, а
  битовая карта занятости авторитетна только после того, как store существует.
  Отдельное негативное обязательство: запуск инициализатора против валидного
  заголовка → отказ, заголовок и occupancy не изменились; мутация, убирающая
  проверку нуля, обязана его покраснить;
- **область действия `schema_identifier` ограничена.** `u64`, выбранный
  владельцем этого store, стабильный для одной линии схемы между совместимыми
  ревизиями источника, осмысленный только вместе с этим 65-секторным экстентом:
  не глобально уникальный, не content id, не id модуля, не id пути `/state`.
  Реестра и механизма глобального выделения нет и не создаётся;
- **переформулирован перебор про получателя и освобождение слота в §13a.**
  Нуклеус запрещает создать преемника из общего плана, **пока предшественник
  жив** и держит `receive`; но `process::retire` вызывает `capability::clear`,
  поэтому полномочие приёма исчезает уже при retire, до сбора tombstone через
  `wait_child`. Порядок «A собран до создания B» остаётся обязательным — но как
  политика супервизора и свидетельство гейта, а не как гарантия нуклеуса. Три
  вещи теперь различаются: исключительность получателя (нуклеус, пока жив),
  переиспользование слота (`MAX_PROCESSES`) и точная последовательность
  (канонический супервизор плюс журнал).

**ADR-0098 принят** (Level 2) и **ADR-0099 принят** (Level 3, архитектурное
решение с одобрением Project Architect). Контракты переехали из `docs/proposed/`
в `source/interfaces/device/BLOCK_DEVICE_V1.md` и
`source/interfaces/state/STATE_STORE_V1.md`, получили статус
`Accepted Tier 2 interface contract`, добавлены в
`docs/SPECIFICATION_SOURCES.txt` — четыре условия допуска ADR-0020 выполнены, и
гейт `check-interface-contract-authority` видит 12 принятых контрактов. В
ADR-0093 добавлена датированная строка: §9 отвечен ADR-0098, остальное в §9 не
двигается.

**Ни один из двух протоколов не реализован.** Оба контракта говорят это в своей
шапке: обязательства по conformance evidence открыты, фикстуры
`block-data-path` и `block-lifecycle` продолжают нести собственную кодировку до
отдельного среза, и принятый контракт не закрывает Stage 4.

### 2026-09-24 — ADR-0098 реализован: `block.device.v1` говорят обе стороны каноническим текстом

**Зелёный гейт `block-protocol.sh`.** Три модуля, три процесса, два endpoint'а.

**Аддитивная поверхность.** Две строки на `system.ipc.Endpoint` —
`endpoint_call_word_region` над операцией 3 и `endpoint_receive_call_region` над
операцией 2 — и запись `system.ipc.ReceivedCallRegion` в `SYSTEM_INTERFACE_V1`,
таблице фронтенда и мосте runtime-образа. Новых операций ABI, механизмов
нуклеуса, видов capability и объектов, границ IPC, движения версии языка — нет:
операция 3 всегда передавала `frame.r8` как счётчик регионов, не хватало только
строки схемы, которой канонический текст мог бы это назвать.

**Закрыты две щели, которые ADR-0098 §2a записал как обязательства.**
`schema_field_type` теперь разрешает `Region<u8>`, и **schema-запись переносит
minor своих полей на модуль, который её называет** — иначе модуль 1.4 получил бы
`Region<u8>`, ни разу его не написав. Фронтенд отказывает в исходнике, верификатор
**независимо** отказывает в артефакте, по достижимому графу типов поля записи.
Первая редакция правила верификатора была неверна ровно в том, о чём
предупреждает ADR-0085: `Region` и `DmaRegion` — типы TOS Core V1 и встречаются в
таблицах модулей гораздо старше любого representation minor, так что запрет их
присутствия сломал шесть существующих тестов. Гейтится не наличие типа, а
обладание значением: заполнение capability-позиции (ADR-0085) и хранение внутри
записи (это решение).

**Сервис и клиент.** `word = sector * 4 + opcode`; READ, WRITE и CAPACITY;
отказы `BLK_OPCODE`, `BLK_RANGE`, `BLK_MALFORMED`, `BLK_NO_REGION`,
`BLK_NO_ANSWER`, каждый — **ответ**, и каждый со своим битом в отчёте сервиса.
`CAPACITY` — число самого устройства, прочитанное по протоколу генерации §2.5.1 и
зажатое по §6c; второй прогон против намеренно маленького устройства показывает
другое число из того же скомпилированного модуля.

**Две обязательные мутации проверены, обе краснеют.** Клиент отправляет
**приманку** — регион других байт, без вызова за ним — прямо перед атомарной
записью; сервис, берущий полезную нагрузку из ранее пришедшего региона, пишет
приманку, и 512-байтовая сверка падает (`i64:-14`). И сервис, отправляющий регион
**до** ответа, роняет журнальную проверку порядка: «the region was sent before the
reply». Порядок нормативен потому, что обратный оставляет осиротевший регион в
очереди endpoint'а, который живёт всю загрузку.

**Найдено при отладке и стоит записи.** Клиент обязан делегировать **send-only**
имя своего inbox: делегирование несёт права отправителя (`IPC_V1` §6), а §2
допускает одного держателя `receive` — так что имя с `receive` заставило бы сервис
стать вторым получателем, принимающий receive отказал бы сообщению целиком, и
запрос остался бы в очереди, пока оба процесса ждут. Два имени одного объекта,
`inbox` и `reply_to`. И одна очередь, обслуживающая несколько запросов, обязана
двигать `avail.idx`/`used.idx` вместе со счётчиком запросов: дескрипторы
переиспользуются, счётчики — нет.

**Ничего из ADR-0099 не реализовано.** `STATE_STORE_V1` принят и не построен.

**Один бит намеренно не выставлен.** `BLK_DEVICE` реализован — устройство,
сообщившее об ошибке, получает ответ с этим кодом, а не разрушение сервиса, — но
референсный endpoint QEMU отвечает `VIRTIO_BLK_S_OK` на любой корректный запрос
внутри своей ёмкости, и фальшивое устройство для этого не строится. Поэтому бит 31
**обязан отсутствовать** в отчёте зелёной загрузки, и гейт это проверяет: тот же
класс свидетельства, который `virtio-queue.sh` фиксирует для негатива MSI-X,
попытанного и отозванного.

Зелёное: `block-protocol` (новый), `preflight --profile docs` (16),
`--profile selftest` (16), `--profile source` (13), `--profile provenance` (3),
gate parity (113).

### 2026-09-24 — утечка answer-endpoint в блочном сервисе закрыта; ADR-0100 предложен

**Две вещи: исправление гигиены ресурсов в уже принятой работе ADR-0098, и
черновик решения, без которого топология ADR-0099 не влезает в свои же границы.**

**Утечка.** Канонический блочный сервис получал делегированный answer-endpoint для
READ, отправлял через него регион и **не освобождал** его. Для фикстуры с одним
READ это проходило случайно. Теперь освобождается всё, что запрос передал и чего
протокол не использует: канал READ — после отправки и **независимо от её успеха**;
endpoint, приложенный к WRITE, CAPACITY или любому отказу, — как полномочие,
которого никто не просил. Нового кода отказа для этого не вводится: контракт не
делает вложение лишнего endpoint'а ошибкой, это гигиена ресурсов, а не правило
провода.

**Свидетельство — тринадцать READ вместо одного.** Двенадцать повторов сверх
конформансной последовательности; это заведомо больше, чем исчерпывает таблицу
capability сервиса. Проверено: **до** исправления прогон краснеет на
`region_allocate` с `E_LIMIT` (`i64:-7806`), пройдя лишь 16 из 21 сообщения.
И добавлено **положительное** утверждение, а не только исчерпание: точный
выведенный счёт освобождений — 4 супервизора, 3 региона и 13 endpoint'ов сервиса,
13 регионов клиента, итого 33. Утечка в один дескриптор, слишком малая, чтобы
исчерпать таблицу, краснеет именно на нём («released 32 capabilities»).

**ADR-0100 предложен, не принят.** `IPC_V1` §6 делегирует права, которые держит
отправитель, а §2 допускает одного держателя `receive` — поэтому клиент обязан
делегировать **send-only** имя своего inbox. `block-protocol` делает это двумя
startup-именами; ADR-0099 так не может: его принятые счёты — 4 и 3, а второе имя
сделало бы 5 и 4 при `MAX_ENDOWMENT = 4`. Механизм в нуклеусе уже есть и
проверен против дерева: операция 5 универсальна, `capability::attenuate`
отказывает только affine-объектам, а `Object::Endpoint` к ним не относится, и
правило одного получателя на аттенюации сработать не может — держатель тот же.
Чего нет, так это строки схемы, которой канонический текст мог бы это назвать.

**Почему ADR, а не правка контракта.** `docs/38` относит изменения Tier-2
контрактов к Level 2 или 3, `docs/21` требует на Level 2 ADR, и ни одна принятая
клауза не разрешает объявить операцию 5 на ещё одном интерфейсе без решения:
§4 `SYSTEM_INTERFACE_V1` — необходимое условие на то, что *можно* объявить, а не
разрешение объявлять. ADR-0078 §1 упоминает аттенюацию runtime-полученного
полномочия как случай, который должно было унести **представление IR**, и решил
именно представление. Практика та же дважды: ADR-0097 и ADR-0098 добавляли строки
и каждая взяла решение.

**Остановлено до одобрения:** строка схемы, поверхность runtime-образа,
свидетельство аттенюации (§5 ADR-0100) и уточнение ADR-0099 / `STATE_STORE_V1`.
Ничего из `STATE_STORE_V1` не реализовано.

Зелёное: `block-protocol` (21 обмен, 13 READ), `block-lifecycle`,
`block-data-path`, `region-transfer-text`; `preflight --profile docs` (16),
`--profile selftest` (16), `--profile source` (13), `--profile provenance` (3),
gate parity (113).

### 2026-09-24 — ADR-0100 принят и реализован: send-only имя для endpoint'а

**Зелёный гейт `endpoint-attenuation.sh`.** Три модуля, три процесса, ни одной
части машины.

**Одна строка схемы поверх операции 5.** `capability_attenuate` на
`system.ipc.Endpoint` в `SYSTEM_INTERFACE_V1`, таблице фронтенда и мосте
runtime-образа. Новой операции ABI, изменений нуклеуса, видов capability и
объектов, границ, членов семейства представления, движения версии языка — нет:
операция 5 универсальна, нуклеус всегда принимал endpoint, и не хватало только
способа это назвать. Требование — `none`, как на четырёх интерфейсах, которые эту
операцию уже объявляли.

**Что доказано.** Аттенюация не потребляет вход и не переносит роль получателя:
держатель отправляет себе на `link` тем самым именем, которое сузил, и забирает
сообщение. Алиас **переходит** сообщением — а это и есть отсутствие взаимоблокировки:
делегирование несёт права отправителя, поэтому неаттенюированное имя было бы
отвергнуто целиком по правилу одного получателя `IPC_V1` §2. Пир отправляет через
алиас, и сообщение приходит в очередь держателя. Приём **через алиас** отвергается с
`E_NO_CAPABILITY` — это отказ проверки прав у дескриптора, а не пустая очередь,
у которой другой номер. И аттенюация — **пересечение, а не валидация**: send-only
имя, спрошенное на `send | receive`, даёт `send`; оно отправляет и никогда не
получает.

**Обязательная мутация.** Делегирование неаттенюированного `send | receive`
краснеет на существующем правиле §2, и гейт называет его: отправка ставится в
очередь (отправителю не говорят ничего, `has_room` — всё, что проверяет send), а
принимающий receive отказывается от **всего** сообщения, так что ожидающий висит,
пока его не отменит правило живости. Отменённый receive — это и есть вид
недоставляемого сообщения со стороны ожидающего, и гейт проверяет, что ни одного
такого в загрузке нет.

**Уточнение учёта ADR-0099 применено** (§2a там и §2a в `STATE_STORE_V1`): канал
для ответа — транзитный send-only алиас, создаваемый на каждый запрос и
освобождаемый **независимо от успеха вызова**, до чтения результата. Startup-наделения
остаются 4 и 3, `MAX_ENDOWMENT` остаётся 4, один запрос в полёте занимает ровно одну
дополнительную запись в таблице, и накапливаться это не может, потому что v1
допускает один неотвеченный READ/GET на endpoint ответа. Персистентная раскладка и
кодировка провода не тронуты.

**Ничего из `STATE_STORE_V1` не реализовано**: ни инициализатора, ни заголовка, ни
PUT/GET, ни битовой карты занятости, ни фикстуры персистентности.

Зелёное: `endpoint-attenuation` (новый), `block-protocol` (21 обмен, 13 READ) и его
три мутации по-прежнему краснеют, `block-lifecycle`, `block-data-path`,
`region-transfer-text`; `preflight --profile docs` (16), `--profile selftest` (16),
`--profile source` (13), `--profile provenance` (3), gate parity (114).

### 2026-09-24 — реализация ADR-0099 остановлена: ADR-0101 предложен, вызов с capability не отдаёт свой ответ

**Найдено при реализации, и это дефект поверхности ADR-0098, а не ADR-0099.**

`BLOCK_DEVICE_V1` §5 нормативен: каждый ответ отправляется через
`endpoint_reply_word`, клиент читает `system.ipc.Answer { length, word }`, проверяет
`length == 8` и различает успех от отказа по `word >= REFUSED`. Но §6a делает READ
единственной строкой, которая несёт endpoint для ответа —
`endpoint_call_word_carrying`, — а её принятая декларация возвращает `i64`, и в
runtime-образе стоит `Produced::Status`. **Поэтому соответствующий контракту клиент
READ не может наблюдать свой собственный ответ.** Ответ доставляется: `ipc::hand`
копирует полезную нагрузку в регион аргументов разбуженного вызывающего, а длина
возвращается в `rdx` — ровно то, что читает `Produced::Answer` у
`endpoint_call_word` и `endpoint_call_word_region` над той же операцией 3. Строка
его выбрасывает.

**ADR-0099 лишь сделал это невозможным игнорировать**, потому что
`STATE_STORE_V1` обязан переводить отказ нижнего слоя в `ST_BLOCK`, а переводить
нечего.

**Обходной путь отклонён.** Схема «вызов вернул OK, региона не пришло, значит
ST_BLOCK» смешивает как минимум три состояния, которые принятые контракты
различают намеренно: отказ `BLK_DEVICE`; отказ `BLK_RANGE`/`BLK_OPCODE`/
`BLK_MALFORMED`/`BLK_NO_ANSWER`, при котором устройство не тронуто; и успешный
ответ с неполной доставкой — §6a ставит ответ **перед** отправкой региона именно
для того, чтобы «чтение удалось и ровно один регион должен» было состоянием, в
котором клиент может находиться. Кроме того это нереализуемо без блокировки:
ожидание региона, который после отказа не придёт, заканчивается правилом живости.

**ADR-0101 предложен, не принят**: результат уже принятой строки становится
`Result<system.ipc.Answer, i64>`, а строка runtime-образа — `Produced::Answer`.
Новой операции ABI, изменений нуклеуса, видов capability и объектов, границ,
членов семейства представления, версии языка, семантики IPC и персистентного
формата — нет. Второй почти-дубликат строки не добавляется: §6a называет **эту**
строку, и её результат и есть дефект. Четыре существующих места вызова
(`block-data-path`, `block-lifecycle` ×2, `block-protocol` ×2) читают результат как
`i64` и станут `match`; их неизменные отчёты — и есть регрессия этого изменения.

**Там же зафиксирована поправка учёта границ ADR-0099**, найденная реализацией:
инициализатор законно нуждается в собственном inbox для блочного READ — он не может
аттенюировать endpoint *сервиса*, это чужое имя, а не входящий канал. Поэтому
endpoint'ов **5 из 6** (`block-serve`, `state-serve`, `state-inbox`, `client-inbox`,
`init-inbox`) и наделений у инициализатора **3**, а не 2. Планов по-прежнему 4,
`MAX_ENDOWMENT` = 4 и `MAX_ENDPOINTS` = 6 не двигаются. Применяется к ADR-0099 и
`STATE_STORE_V1` **после** рассмотрения ADR-0101.

**Реализация ADR-0099 остановлена** до одобрения. В дереве незакоммиченными лежат
черновые модули `source/tests/vectors/state-store/`; гейта нет, и ничего из ADR-0099
не заявляется.

### 2026-09-25 — ADR-0101 принят и реализован: вызов с capability читает свой ответ

**Принято Project Architect 2026-09-25 как корректирующее решение уровня 2**, с
двумя фактическими поправками к черновику: мест вызова **пять** в трёх модулях
(`block-data-path` 1, `block-lifecycle` 2, `block-protocol` 2), а обязательство §4.4
требует именно `BLK_RANGE`. `BLK_NO_ANSWER` там не альтернатива: доставленный
`endpoint_call_word_carrying` по построению несёт endpoint для ответа, так что эта
форма запроса такого отказа произвести не может, а вызвать его можно только через
`endpoint_call_word` — другую строку, которая исправленный результат не проверяет.

**Изменено ровно две вещи.** Результат строки `endpoint_call_word_carrying` в
`SYSTEM_INTERFACE_V1` и таблице фронтенда — `i64` → `Result<system.ipc.Answer, i64>`;
строка runtime-образа — `Produced::Status` → `Produced::Answer`. Ни операции ABI, ни
нуклеуса, ни видов capability и объектов, ни границ, ни версии языка, ни семантики
IPC, ни персистентного формата. Ничего ниже строки: та же операция 3, тот же слот
передачи, то же размещение слова в `MESSAGE_PAYLOAD`.

**Пять мест вызова читают результат как `match`**, и ни один отчёт не изменился —
именно поэтому их существующие гейты и есть регрессия. Там, где раньше стояло
`status == OK`, теперь требуется ответ ожидаемой формы, а не любой `Ok`:
`block-data-path` знает, что сервис подтверждает числом байт (`length == 8`,
`word == 512`), `block-lifecycle` — тем же; канонический READ `BLOCK_DEVICE_V1`
требует `length == 8` и `word == 0` **до** ожидания региона.

**Новый гейт `carried-call-answer`** — общее доказательство, независимое от
`block.device.v1`: два процесса, один endpoint между ними, никакого устройства,
региона и драйвера. Четыре обмена, и это не четыре копии одного факта: два разных
успешных слова (значит число не от строки); ответ всегда «запрос + сдвиг», а не эхо
(иначе строка, ничего не скопировавшая, вернула бы вызывающему его собственное слово
из его же региона аргументов и прошла бы проверку); одно слово выше старшего бита —
класс отказа, который `i64` нести не мог; и один ответ вовсе без полезной нагрузки,
`length == 0`, значит длина принадлежит отвечающему, а не строке. Мутация: вернуть
`Produced::Status` при исправленной схеме — загрузка падает в `TOS.RUN.TRAP`.

**`block-protocol` усилен**: 22 обмена вместо 21. Добавлен READ за пределами
ёмкости, сделанный именно строкой с переносом, и его отказ читается из ответа —
`length == 8`, `word == REFUSED + BLK_RANGE`. `BLK_DEVICE` не выдумывается.
«Регион за отказом не следует» утверждается дважды и никогда ожиданием отсутствия:
окно этого обмена не содержит `endpoint_send_region` вовсе, а общий счёт отправок
региона остаётся один на **обслуженный** READ плюс приманка. Отказ поставлен
**после** приманки: окно идёт от одного receive до следующего и содержит всё, что
сделали оба процесса, поэтому окно, за которым клиент отправляет приманку, держало
бы чужую отправку региона. Счёт освобождений 33 → 34 (сервис отпускает канал и
отказанного READ). Мутация «отправить регион раньше ответа» по-прежнему краснеет на
упорядочивании; мутация «подождать регион после отказа» краснеет тем, что клиент
остаётся заблокированным и до атомарной записи не доходит — то есть обходной путь из
§1 ADR-0101 не просто нечестен, он неисполним.

**Поправка учёта границ ADR-0099 применена** к ADR-0099 §13a и `STATE_STORE_V1`
§2a: endpoint'ов 5 из 6, наделений у инициализатора 3. `MAX_ENDPOINTS` = 6 и
`MAX_ENDOWMENT` = 4 не двигаются; это поправка счёта, а не новое решение о
топологии. `STATE_STORE_V1` §2a и §9 теперь несут честное отображение `GET`: ошибка
нижнего IPC — незавершённая операция, а не отказ; `length != 8` и `word >= REFUSED`
— `ST_BLOCK`; `word == 0` — должен ровно один регион. После отказа нижнего слоя
регион не ожидается.

Зелёное: `carried-call-answer` (новый), `block-protocol` (22 обмена, 13 READ, 14
доставленных вызовов с переносом), `endpoint-attenuation`, `block-lifecycle`,
`block-data-path`, `region-transfer-text`; `cargo test --release` весь workspace;
`preflight --profile docs` (16), `--profile selftest` (16), `--profile source` (13),
`--profile provenance` (3), gate parity (115).

**Реализация ADR-0099 продолжается** после этого commit. `state-store/` пока
незакоммичен, гейта нет, и ничего из ADR-0099 не заявляется.

### 2026-09-25 — ADR-0099 / STATE_STORE_V1 реализован: персистентное хранилище объектов

**Шесть канонических модулей, восемь процессов, реальное устройство.** Гейт
`source/host-tools/qemu-test/state-store.sh`:

```text
block service         создан первым, завершается последним; единственный держатель шины
initializer, прогон 1 capacity >= 65; сектор 0 весь нулевой; записан один заголовок
initializer, прогон 2 тот же текст против записанного заголовка: ОТКАЗ форматировать
state store A         открывает и валидирует заголовок с устройства
writer                put(1, pattern A); put(2, pattern B); завершается
A завершается, retire, и ОБА ending собраны до того, как создан B
state store B          тот же модуль из того же sealed plan; перечитывает заголовок
reader                get(3) отказан как absent; get(2) проверен байт за байтом
```

**Почему это утверждение о персистентности, а не о передаче сообщений.** Writer
завершился и собран. Генерация A завершилась, прошла retire — `process::retire`
очищает её capabilities — и её ending собран. Генерация B — другой процесс, которому
не передали ничего, кроме sealed plan. Значит 512 байт, которые проверяет reader,
пришли с устройства, и заголовок, который валидировала B, — тот, что на устройстве.

**Учёт, выведенный а не выбранный.** 12 запросов к блочному сервису (3 + 2 + 5 + 2), из
них 2 `CAPACITY` не доходят до кольца, 10 кольцевых по 3 адреса плюс 3 адреса самого
кольца = 33; 16 полученных сообщений и 16 ответов; 7 вызовов исправленной строки
ADR-0101 на двух уровнях; 6 отправок региона и 6 приёмов; 34 освобождения. `PUT` не
освобождает ничего — это правило одной копии, видимое как счёт: регион клиента
передаётся устройству, а не копируется.

**Топология читается из журнала, а не из исходника.** Шина запрашивается ровно дважды
— супервизором и блочным сервисом; endpoint `block.device.v1` ровно четыре раза — двумя
прогонами инициализатора и двумя генерациями хранилища; endpoint хранилища ровно дважды
— клиентами. Наделения: супервизор 8, блочный сервис 3, каждый прогон инициализатора
**3**, каждая генерация хранилища 4, каждый клиент 3 — это исправленный учёт ADR-0101
§6, посчитанный лаунчером, а не заявленный модулем.

**Control before data на двух уровнях, одной последовательностью.** Хвост журнала после
последнего ожидания завершения устройства — ровно шесть шагов:
reply, send, receive, reply, send, receive. Отправка раньше ответа на **любом** из двух
уровней краснит это.

**`get(3)` — не запасное утверждение.** Наличие определяется только `occupancy` (§5), а
харнесс засевает сектор 3 512 копиями `0xC3` — так что хранилище, выводящее наличие из
байтов, ответило бы этим заполнением. Требуется ровно `REFUSED + ST_ABSENT`, и
сектор не читается: это подтверждают и 12 посчитанных запросов к устройству.

**Все семь обязательных мутаций краснят гейт, каждая на своём утверждении:**

```text
1 убрать запись заголовка инициализатором   §0: форматировали оба прогона
2 убрать запись payload-сектора             reader: i64:-8, побайтовый свидетель
3 отвечать на get(2) сектором объекта 1      reader: i64:-8
4 игнорировать битовую карту занятости       reader: i64:-3, get(3) был обслужен
5 убрать обновление occupancy у PUT(2)       reader: i64:-6, объект 2 absent у B
6 отправить регион GET до ответа             §5: reply/send/receive/send/reply/receive
7 убрать проверку нулевого заголовка         §0 снова — 1 и 7 об одном и том же
```

Мутации 2 и 3 делят утверждение намеренно: ADR-0099 §13 называет обе побайтовым
свидетелем, потому что ломают они одно и то же утверждение.

**Что не заявляется:** устойчивость к потере питания, `VIRTIO_BLK_F_FLUSH`, crash
consistency, журналирование, транзакции, exactly-once `PUT`, delete, перечисление,
второй владелец или второе хранилище, `/state` из `docs/09`, любая семантика путей — и
**закрытие Stage 4**. Stage 4C, Stage 4D и Stage 4 остаются открытыми.

Зелёное: `state-store` (новый), `carried-call-answer`, `block-protocol`,
`endpoint-attenuation`, `block-lifecycle`, `block-data-path`, `region-transfer-text`;
`preflight --profile docs` (16), `--profile selftest` (16), `--profile source` (13),
`--profile provenance` (3), gate parity (116).

### 2026-09-25 — корректирующий раунд по ADR-0099: четыре исправления

**Архитектура `STATE_STORE_V1` принята, реализация была неполной.** Четыре узких
исправления, без нового ADR и без переделки решения.

**1. Незавершённая нижняя операция — не `ST_BLOCK`.** `read_sector()` сваливал в один
`Err` четыре разных исхода, а `one_object()` превращал любой из них в
`REFUSED + ST_BLOCK`; у `PUT` было то же. Это противоречит §2a/§9. Введены три класса —
`OK`, `LOWER_REFUSED`, `LOWER_INCOMPLETE` — и **только средний** становится `ST_BLOCK`:

```text
Err от вызова                       -> incomplete
length != 8                         -> ST_BLOCK
word >= BLK_REFUSED                 -> ST_BLOCK
word != 0 и word < BLK_REFUSED      -> incomplete (не успех READ и не отказ;
                                       код BLK не выдумывается)
word == 0, регион не пришёл         -> incomplete
```

При `incomplete` ответ **не отправляется** вовсе: отпускается то, что можно отпустить, и
вызывающий узнаёт об этом через правило живости, как и говорит §8b. Две новые мутации
это проверяют: нижний вызов, провалившийся на уровне IPC, и нижний READ, ответивший
успехом и не отправивший регион, — обе дают хранилищу `-5340`, а читателю **`-305`
(отменён)**, и никогда `-6`, каким был бы сфабрикованный `ST_BLOCK`.

**2. Негатив переформатирования усилен.** Второй прогон инициализатора теперь идёт
**после** writer'а и после сбора обеих концовок — то есть против заголовка, у которого
в `occupancy` два бита, и ни одного живого state-сервиса. Прежний порядок доказывал
только «валидное пустое хранилище не переформатируется». Мутация 7 теперь действительно
сбрасывает `occupancy`, и `get(2)` читателя падает (`-6`): именно этот поздний `get` и
есть свидетель того, что заголовок и занятость выжили. Второй прогон документирован как
adversarial-проба переосвоения, а не как политика обычной работы.

**3. Вся поверхность отказов `STATE_STORE_V1` §9 проверена.** Writer: `ST_OPCODE`,
`ST_ID` (id 0), `ST_NO_REGION`. Reader: `ST_ID` (id 65 — другой конец диапазона),
`ST_NO_ANSWER`, `ST_MALFORMED`, `ST_ABSENT`. Все — по точному слову ответа, кроме
`ST_MALFORMED`: единственная строка, позволяющая клиенту выбрать длину
(`endpoint_call`), возвращает статус, а не ответ, поэтому доказывается «вызов отвечен»
плюс бит в отчёте хранилища — та же форма, что у malformed-доказательства блочного
слоя. Ни один из ранних отказов не доходит до устройства, и это говорит счёт запросов.
`ST_BLOCK` остаётся **реализованным и не проверенным**: эталонная VirtIO-точка отвечает
`VIRTIO_BLK_S_OK` на всякий корректный запрос в диапазоне, и отказ устройства не
выдумывается ради зелёной строки.

**4. Открытие честно учитывает «устройство слишком мало».** §9 даёт `ST_STORE` две
причины, и обычный сервис проверял только заголовок. Теперь открытие сначала спрашивает
нижний `CAPACITY` и при `capacity < 65` объявляет хранилище закрытым — **не читая
сектор 0 вовсе**. Отказ или сбой нижнего слоя при открытии не превращается в `ST_STORE`:
это незавершённая операция, и сервис сообщает о ней, а не изображает закрытое хранилище.

**Второй boot** против образа на 64 сектора: инициализатор ничего не пишет (`32`),
хранилище закрыто (`2048`), probe получает пять `ST_STORE` на пять по-разному
сформированных запросов (`520093696`), и во всём boot'е **три** разрешённых адреса
устройства — только собственная настройка кольца, ни одного запроса. Блочный сервис
заканчивается сам: когда клиентов не осталось, правило живости завершает его receive, он
читает это как «спрашивать больше некому» и выходит с положительным отчётом.

Счёт обычного boot'а пересчитан: 14 запросов к блочному сервису (3 + 6 + 2 + 3), из них
4 `CAPACITY`; 24 полученных сообщения и 24 ответа; 8 вызовов исправленной строки
ADR-0101; 8 word-only вызовов; 7 атомарных с регионом; 1 вызов с длиной по выбору
клиента; 6 отправок региона и 6 приёмов; 33 адреса устройства; 38 освобождений.
Наделения по журналу: супервизор 8, каждая генерация хранилища 4, остальные пять по 3.

Зелёное: `state-store` (два boot'а) и все девять мутаций краснеют каждая на своём
утверждении; `carried-call-answer`, `block-protocol`, `endpoint-attenuation`,
`block-lifecycle`, `block-data-path`, `region-transfer-text`; весь workspace под
`cargo test --release`; `preflight --profile docs` (16), `--profile selftest` (16),
`--profile source` (13), `--profile provenance` (3), gate parity (116).

Stage 4C, Stage 4D и Stage 4 остаются открытыми.

### 2026-09-26 — ADR-0102 предложен: Stage-4 связывание капсулы с репозиторием

**Исследование закрыто, решение предложено, ничего не реализовано.** ADR-0102
покрывает D1–D5 одним решением уровня 3: у расширения репозитория, объектного
формата, читателя и его полномочий нет независимого существования, и пять ADR
ссылались бы друг на друга в каждом поле.

**Три поправки Project Architect к исследовательской записке внесены** (§0b записки
перечисляет все пять отменённых выводов):

1. **Точка приземления — не идентичность вызывающего модуля.** `CAPSULE_FORMAT_V1`
   §5 уже фиксирует ровно один boot-canonical файл — `/system/boot/init.tos`, — а
   `docs/04` называет именно этот модуль тем, кто обнаруживает и проверяет хранилище
   репозитория. Ни каталог капсулы, ни поиск произвольного файла, ни перечисление
   source set не открываются.
2. **Запись идентичности меньше, чем говорил §19.** `launch::Template` уже хранит
   `pub bi: &'static BootInfo` со всеми четырьмя проверенными полями; дублировать
   `oid_algorithm`/`oid_length` не нужно. Добавляется **одно** поле — уже проверенный
   32-байтовый SHA-256 boot-canonical файла, **сохраняемый, а не пересчитываемый**:
   нуклеус не делает криптографической работы.
3. **Capability не «не даёт полномочий».** `system.boot.Identity` разрешает ровно READ
   неизменной проверенной идентичности источника загрузки и ничего изменяемого — и это
   полномочие, выпускаемое на доверенной границе загрузки.

**Что решает ADR-0102.** Stage-4 половина handoff — связывание и проверка:
канонический текст убеждается, что расширение репозитория содержит коммит, который
называет капсула, и что этот коммит разрешает `source/system/boot/init.tos` в байты,
совпадающие с boot-canonical `/system/boot/init.tos` капсулы. Имя в `docs/16` не
меняется; половина перехода остаётся за Stage 5, и примирение с `docs/04`/`docs/11`
записано явно. `system.boot.Identity` как `AsInterface` с `RIGHT_READ`; одна операция
`SYSTEM_ABI_V1` 32, запись из 11 `u64` по смещению `MMIO_MAP_RECORD + 16 = 592`,
88 байт из кадра в 4096. Специализированный профиль `docs/36`: только commit/tree/blob,
**без refs**, SHA-1 для Git-идентичности и SHA-256 для сравнения с капсулой, честно
записанное ограничение по коллизиям, и loose-объекты только из stored-блоков DEFLATE —
ограниченная последовательность, а не один блок.

**Расширение и учёт.** Секторы `[65, 2113)`, 2048 секторов = 1 MiB, без таблицы
разделов, без файловой системы, без второго устройства. Заголовок 1 сектор, таблица
3 сектора (32 записи × 48 байт = 1536), данные 2044 сектора. Реальная цепочка на
`4600a8f` — 26 секторов данных, то есть 30 из 2048: запас 68×. Размещение объекта
**выводится** из суммы предыдущих длин, а не хранится, поэтому перекрытие не
представимо как состояние. В заголовке расширения **нет корневого коммита**: корень
приходит только из `system.boot.Identity`, иначе расширение решало бы, чему система
должна верить о себе.

**Обеспечение — хостовое, проверка — в TOS**, и порядок является решением: капсула,
затем её OID, затем разрешение этого коммита в репозитории хоста. Хостовый `HEAD`
после сборки капсулы не используется: фикстура, взявшая корень из `HEAD`, совпала бы
с капсулой по совпадению времени.

**Девятнадцать обязательных негативов**, включая «расширение содержит другой
корректный коммит, чем тот, что называет капсула» и «правильный коммит, но не тот
blob». Где отказ означает «нижнего блочного запроса не было», это доказывается
журналом блочного сервиса, а не заявлением читателя. Межперезагрузочная
персистентность **не заявляется**: харнесс пересоздаёт образ диска.

`preflight --profile docs` 16/16. Не реализовано ничего: ни `system.boot.Identity`, ни
операции ABI, ни вида объекта, ни хеша, ни zlib, ни строителя расширения, ни читателя,
ни резервирования секторов в production-коде. Stage 4C, Stage 4D и Stage 4 остаются
открытыми; Stage 4E остаётся обязательным после формального закрытия Stage 4 и до
Stage 5.

### 2026-09-26 — ADR-0102 исправлен по ревью: семь узких контрактных поправок

**Направление принято, статус прежний: Proposed, уровень 3, одобрения нет.** Дизайн не
менялся; уточнены семь мест, где черновик оставлял решение реализации.

**Проверено по коду, а не принято на слово.** `OBJECT_DMA_REGION = 13` — действительно
старший публичный вид объекта, поэтому `OBJECT_BOOT_IDENTITY = 14` верен; `scope` в
`LaunchCapability` документирован как *"zero where it does not [have one]"*, поэтому
`scope = 0` — существующее соглашение, а не новое правило.

1. **Кодирование вида capability зафиксировано в ADR**, а не выбрано реализацией:
   `OBJECT_BOOT_IDENTITY = 14`, `ObjectKind::BootIdentity`, `Object::BootIdentity`, без
   индекса и поколения (нечего различать и нечего инвалидировать), `scope = 0`, а
   описание наделения с видом 14 и ненулевым scope **отказывается**. Номер — часть
   версионированного контракта запуска (I-09).
2. **«Delegable normally» было ложным утверждением и снято.** Несущие строки
   типизированы под `system.ipc.Endpoint`; канонический текст не может положить Identity
   в сообщение. Объявлены ровно четыре строки (32, 5, 6, 22), ниже последних трёх нового
   механизма нет, а поддерживаемый путь делегирования — **bootstrap endowment или явное
   endowment через launch-plan**. Общая передача произвольных capability по IPC не
   добавляется ради задачи, которой у этого среза нет. Это тот же урок, что и ADR-0100.
3. **CAPACITY до любого чтения расширения**: требуется `capacity >= 2113`, иначе не
   выдаётся **ни одного** READ сектора репозитория. Иначе «расширение существует»
   выводилось бы из того, что первое чтение случайно не упало.
4. **Побайтово точное хранение объекта**: байты от `stored_length` до границы сектора
   **обязаны быть нулевыми**, ненулевой байт — отказ формата. Никаких непроверяемых
   скрытых байтов в персистентном формате, который пишет хост.
5. **Конец zlib-потока стал нормативным**: `BFINAL=0` на всех неконечных блоках, ровно
   один конечный, ровно один трейлер Adler-32, заканчивающийся точно на `stored_length`,
   и ничего после него внутри `stored_length`. Обязательная мутация одна — хвостовые
   байты после корректного потока.
6. **Минимальный Git-парсер определён**, а не только назван: конверт
   `<kind> SP <size> NUL <payload>` без байтов после объявленного размера; у коммита
   потребляется **только** первая строка `tree <40 hex>\n`, родители не обходятся,
   остальное непрозрачно; деревья — обычные двоичные записи с границами по имени,
   количеству и полноте OID; режим `040000` для промежуточных компонент и
   `100644`/`100755` для листа; компонент пути должен встретиться **ровно один раз** —
   «первый выигрывает» не принимается.
7. **Словарь отказов определён**: девять классов (`REPO_OK`, `REPO_FORMAT`,
   `REPO_BOUNDS`, `REPO_MISSING`, `REPO_KIND`, `REPO_OID`, `REPO_LINKAGE`,
   `REPO_UNSUPPORTED`, `REPO_BLOCK`) с нормативным отображением, а не девятнадцать
   одноразовых кодов. Это классы результата канонического читателя, а не новое
   пространство ошибок `SYSTEM_ABI_V1`. Прежний черновик обещал «by exact refusal code»,
   не определив ни одного кода, — обещание, которое документ не мог сдержать.
   Незавершённая нижняя операция остаётся незавершённой операцией процесса и **не**
   превращается в семантический отказ репозитория — правило `STATE_STORE_V1` §2a и
   ADR-0101, унаследованное, а не переоткрытое.

**Доказательства дополнены девятью пунктами**: ненулевой scope, процесс без Identity,
endowment через план, аттенюация не расширяет, capacity < 2113 без чтения сектора,
ненулевой padding, хвост после потока, испорченная запись дерева, дважды встреченный
компонент пути.

`preflight --profile docs` 16/16. По-прежнему не реализовано ничего. Stage 4C, Stage 4D
и Stage 4 остаются открытыми; Stage 4E остаётся обязательным после формального закрытия
Stage 4 и до Stage 5.

### Открытые решения — машинно-проверяемый список

**Единственное место в этом файле, которое говорит о статусах ADR в настоящем
времени.** Всё остальное в журнале — датированные записи, верные на свою дату и
не являющиеся утверждением о сегодняшнем дне. Гейт
`scripts/check-open-decisions.sh` сверяет список ниже с перебором строк
`- Status:` по `docs/adr/` и краснеет, если они разошлись, — чтобы правило «не
читать историю как текущий статус» не приходилось помнить.
...
Помимо ADR: формальное рассмотрение закрытия Stage 4C, Stage 4D и Stage 4; решения
§6 `docs/evidence/STAGE4_PERFORMANCE_REPORT.md`; запись решения по патентному обзору
Stage 4 (`docs/research/PATENT_LANDSCAPE.md`, `docs/24` шаг 8).

### Требуют решения Project Architect

**На 2026-09-20 ни один пункт этого раздела не открыт.** H и G были последними
и закрыты ещё 2026-08-25 — раздел просто не был обновлён, ровно та же ошибка
журнала, что уже случилась с пунктом C. Правило остаётся прежним и теперь
проверено трижды: **статус берётся из строки `- Status:` самого файла ADR**, а
этот раздел — историческая запись.

**Поправка от 2026-09-21: первая редакция этой шапки утверждала, что закрытие
Stage 4C/4D — единственное, что ждёт Project Architect. Это неверно.**
`ADR-0044` (versioned module digest) стоит `Proposed (awaiting Project
Architect decision)` с 2026-08-12 и в этот раздел никогда не попадал — то есть
раздел не только держал закрытое открытым, но и не держал открытое вовсе. Он
описывает решения, которые через него проходили, а не множество открытых
решений системы. Полный список открытого берётся перебором строк `- Status:`
по `docs/adr/`, и на 2026-09-21 это `ADR-0044`, `ADR-0092` и `ADR-0093`,
плюс закрытие Stage 4C/4D, записанное выше.

**H. Судьба относительного бюджета `8x` — ЗАКРЫТО 2026-08-25: ADR-0068
принят Project Architect и реализован одним изменением** (`docs/35`, `IPC_V1`
§8, ADR-0066 §3/§5 и гейт в одном коммите). Статус берётся из строки
`- Status:` самого ADR — там **Accepted**. Формулировка ниже сохранена как
запись того, чем это было до принятия, и её последняя строка («ADR ждёт
утверждения») с тех пор неверна.
...
### 2026-09-26 — ADR-0102 реализован: канонический текст читает репозиторий и связывает его с капсулой

**Принят на уровне 3 и построен целиком.** Каждое достижимое обязательное обязательство
по свидетельству ADR-0102 выполнено; `REPO_BLOCK` реализован и честно недостижим на
соответствующем эталонном устройстве, а межперезагрузочная сохранность не заявляется.
Stage 4 не закрывается, 4C/4D не закрываются, следующее обязательство Stage 4 не начато.

**Что доказано на реальной машине.** Загрузка, в которой каждый слой — канонический
текст: блочный сервис `block.device.v1` и над ним читатель, который держит SHA-1,
SHA-256, ограниченный inflater stored-блоков, минимальный парсер Git и формат экстента —
и **не держит ни одной части машины**. Он берёт идентификатор коммита и SHA-256
загрузочного файла из `system.boot.Identity` (операция 32), читает экстент с сектора 65,
проверяет каждый объект против идентификатора, по которому он был найден, и сравнивает
`SHA-256(blob) == boot_content_sha256`. Учёт `8176` = класс `REPO_OK` плюс все девять
бит доказательств.

**Лаунчер фикстуры — это и есть blob, на который указывает путь.** Коммит, который
называет капсула, — настоящий коммит Git с настоящей историей репозитория позади и
настоящими соседними деревьями вокруг, построенный в alternates-виде, так что в
проверяемый репозиторий ничего не пишется. Это и есть путь владельца к загрузке
изменённого исходника, пройденный целиком: машина грузит изменённый `init.tos`, и от
репозитория требуется содержать ровно это изменение в названном коммите.

**Двадцать три загрузки-отказа**, каждая отличается от обычной ровно одним фактом и
каждая отказана точным классом §10a. Три из них — о форме утверждения, а не о байтах:
`capacity` (устройство на сектор короче разметки — и **ни одного** чтения сектора
репозитория), `other-commit` (корректный экстент **другого** коммита → `REPO_MISSING`:
свидетельство того, что читатель стартует от капсулы, а не от константы) и `linkage`
(тот коммит, но загрузочный модуль взят с другого пути репозитория → обход проходит,
все идентификаторы сходятся, а сравнение SHA-256 — нет).

**§11d засвидетельствован после починки корневой причины.** `drain_report` читает
область отчёта процесса как один физически непрерывный прогон через собственную
identity-карту нуклеуса, и `install` заявляет, что ему её такой и дают, — а `map_fresh`
выделял её по кадру. Первые процессы загрузки получали смежный набор с фронтира пула
по везению; процесс, созданный после того, как другой был разобран, получал то, что
отдавал список освобождённых. Нуклеус выливал в последовательный порт кадры, принадлежащие
чему-то другому, — двадцать восемь килобайт вперемешку с текстом второго поколения, из-за
чего `grep '^TOS\.'` выбрасывал каждую строку, к которой приклеился мусор. Область отчёта
теперь вырезается одним прогоном и возвращается через `release_run`; `release_mapped_run`
**проверяет** непрерывность, а не предполагает её. Гейт утверждает порядок: A достигает
свидетеля, завершается, собран; B создан после, читает устройство сам, достигает того же
свидетеля, завершается, собран.

**Два дефекта найдены и починены по дороге.** `tos-engine` отказывал `to_u64(size)`,
хотя `docs/40` §3 говорит «any fixed-width integer or `size`» и все статические стадии
это принимали — отказ приходил из рантайма после того, как чекер, лоурер и независимый
верификатор согласились. `write_argument_record` не проверял собственную границу.

**Три правила самой capability и живая граница глубины.** Процесс, которому
`system.boot.Identity` не выдан, отказан до первой инструкции с названным binding'ом;
аттенюация — это пересечение (`read | send` даёт `read` и читает, `send` в одиночку
отказан); описание наделения с видом объекта 14 и ненулевым `scope` не запускает
загрузку. `MAX_TREE_DEPTH` проверяется понижением границы в конформансной копии
читателя, отличающейся ровно одной строкой, — путь продукта и интерфейс не меняются.

**Экстенты не пересекаются, и это говорит настоящий объект.** В той же загрузке
работает принятая реализация `state.store.v1` целиком: инициализатор форматирует
нулевой сектор, генерация A обслуживает писателя, который кладёт 512-байтный объект
и завершается, — затем идут все репозиторные чтения двух поколений читателя, — и
**новая** генерация хранилища перечитывает заголовок с устройства, а клиент
запрашивает объект по идентификатору и проверяет каждый байт (ADR-0102 §11c).
Учёты — собственные учёты ADR-0099, потому что работают собственные модули ADR-0099.

**`system.boot.Identity` не получает общей передачи по IPC.** Единственный путь
общей передачи в нуклеусе — `resolve_transfers`, он вызывается из одного места и
`Object::travels_in_a_message` отвечает для этого вида «нет». Канонический текст не
может даже выразить попытку: несущая позиция типизирована под `system.ipc.Endpoint`,
и загрузка с такой попыткой отказана чекером —
`E1215_ARGUMENT_TYPE_MISMATCH … expected=system.ipc.Endpoint actual=system.boot.Identity`.

- `bash source/host-tools/qemu-test/repository-linkage.sh` → `PASS (the linkage proved
  from a real device by two generations, 23 refusal(s) each with the ADR-0102 §10a class
  §11c names for it, and the identity's denial, attenuation and scope rules)`;
  29 загрузок QEMU, ~9 мин.
- `bash scripts/tests/check-repository-extent.sh` → `PASS (20 refusal case(s),
  deterministic bytes, ordinary git reads every stream, capsule-is-the-authority
  checked=1)`.
- Топливо канонического читателя: `1 489 739` из объявленных `67 108 864` на цепочку из
  шести объектов; SHA-1 ≈ 7 700 топлива на 64-байтный блок, SHA-256 ≈ 11 400.

### 2026-09-26 — Подготовка к закрытию Stage 4: аудит, недостающие свидетельства, отчёт о производительности

**Цель среза — не новая возможность, а состояние, в котором Project Architect может
принять одно явное решение о закрытии Stage 4C, 4D и 4.** Stage 4E не начат, Stage 5
не тронут, статусы стадий не менялись.

**Аудит сначала, код потом.** Каждое обязательство Stage 4 — deliverables и exits
`docs/16`, evidence `docs/37`, `docs/35` §Stage 4, правило `docs/34` для новой границы,
разделы свидетельств каждого принятого ADR Stage 4 — сопоставлено с гейтом:
`docs/evidence/STAGE4_CLOSURE_AUDIT.md`, 70 строк, 60 PASS, 1 PARTIAL, 3 MISSING,
6 NONCLAIM, проверяется гейтом `closure_audit_stage4`.

**Чего не хватало, и что добавлено.**

- **ADR-0084 §8** — пункты 4, 9a, 9b, 11, 12 (churn) и 13a (fail-closed) не имели
  свидетельства в рантайме. `dma-quarantine.sh`: одна каноническая программа, две
  загрузки; во второй у функции Transactions Pending не очищается никогда.
  Мутации refund-on-release и reclaim-по-`BME=0` проверены — обе краснят гейт.
  Операция 30 теперь отвечает на функцию без PCI Express capability
  `E_BAD_ARGUMENT`, как всегда говорила строка 30 `SYSTEM_ABI_V1`.
- **Случай D ADR-0093 и незавершённый READ** — `block-fault.sh`: принятый сервис
  плюс три точки выхода, выбираемые модулем политики в капсуле. Наблюдение клиента
  одинаково (`E_CANCELLED`), состояние устройства различно (0 и 1 доставок; нули и
  паттерн в образе после выхода QEMU).
- **Отказывающее устройство** — `run.sh --stage4-block-fault`: QEMU `blkdebug`
  проваливает настоящий запрос. Первый прогон зависал: после отказа устройства
  сервис не считал запрос прошедшим через кольцо, следующий публиковался в тот же
  слот. Исправлено во всех четырёх копиях сервиса; `block-protocol`, `state-store`,
  `repository-linkage` перепрогнаны зелёными.
- **Лгущее устройство** — `test-hostile-device`: DMA-противник пишет в память
  драйвера до его пробуждения; чужой id (-53), длина больше (-59) и меньше (-54),
  «успешное» чтение без данных (-56) отвергнуты каждое своим шагом.
- **Стоимость запроса** — `test-request-cost` и `stage4-request-cost.sh`.

**Отчёт о производительности** — `docs/evidence/STAGE4_PERFORMANCE_REPORT.md`:
H2, H4, H5 выполнены; H1 (1 выделение на READ) и H3 (8 передач процессора до
таймерных) превышены по причинам, зафиксированным принятыми решениями; R1–R3 — P0,
принятого метода измерения нет. Наблюдательно: ~40 мс на READ под TCG.

**Модель угроз** сведена (X4.1–X4.12, коллизии идентификаторов устранены, раздел
«What Stage 4 does not claim»); **патентный обзор** `docs/24` шаги 1–6 записан в
`docs/research/PATENT_LANDSCAPE.md`, три пункта отмечены для решения.

**Вывод свидетельства:** Stage 4C — READY TO CLOSE; Stage 4D — READY TO CLOSE;
Stage 4 — BLOCKED отчётом о performance contract (решения §6) и записью решения по
патентному обзору.

Проверено:

- `./scripts/preflight.sh --full` на `9dadd27` → **120 из 122**: два непрошедших —
  `qemu_stage3_observer_conformance` и `qemu_stage3_ipc_conformance`, оба выходят с
  `the selected QEMU has no ADR-0066 observer-build.json` до загрузки; это известный
  локальный пробел окружения, CI строит наблюдатель сам.
- После правок самопроверки (устаревшие формулировки о P1/P5, «пять мутаций» → девять
  в X4.12, строка 11.3 аудита) перепрогнаны `dma-region.sh`, `virtio-queue.sh`,
  `dma-quarantine.sh` → PASS; `--profile docs` → PASS.
- Гейты среза по отдельности: `dma-quarantine.sh` (~15 с), `block-fault.sh` (~100 с,
  12 загрузок), `stage4-request-cost.sh` (~12 с), `repository-linkage.sh` после
  исправления кольца (~9 мин) — все PASS.
