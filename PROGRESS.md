...
- Полномочие публикации в смысле `CAPABILITY_V1` §6 не реализовано: что
  предъявляет публикатор — обычный endpoint, а не capability, чей номинальный
  тип есть публикуемый интерфейс. Имя `block.device.v1` в протоколе не едет.
  Негативное свидетельство ADR-0093 §10.1 — **BLOCKED**, см. ADR-0093-Q1.
...
**Следующий честный фронтир** — один из двух, и оба узкие:
1. **Регион через IPC из канонического текста** — то, что превращает «клиент
   получил число» в «клиент получил байты»: клиент составляет полезную нагрузку
   в `Region<mut T>`, замораживает её, передаёт как capability в вызове, сервис
   копирует её в `DmaRegion<mut T>`. Это граница §1 упомянутой research-заметки
   и, вероятно, требует решения: перенос региона из текста ADR-0094 §10 явно не
   решал.
2. **Полномочие публикации** — решение по ADR-0093-Q1, после которого
   негативный гейт §10.1 становится выразимым.

Ни то, ни другое не начато, и порядок между ними — решение Архитектора.
...
- `ADR-0093-Q1` — полномочие публикации не реализовано в том виде, в котором
  принято: публикатор предъявляет обычный endpoint, а не capability, чей
  номинальный тип — публикуемый интерфейс. Негативное свидетельство ADR-0093
  §10.1 записано BLOCKED. Варианты — в
  `docs/research/PUBLICATION_AUTHORITY_CONFLICT.md` §5, ни один не выбран.
