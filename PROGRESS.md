...
Порядок категорий — тот, который аудит попросил различать.

**Формально закрыто** (approval в `source/legal/publication-records/`): Stage 1,
Stage 1.5, Stage 2, Stage 3, Stage 4A, Stage 4B.

**Построено и зелено, не закрыто.** Stage 4C-1…4C-3 и Stage 4D-1…4D-5. На
реальном железе, которое предоставляет эмулятор: канонический текстовый драйвер
претендует на PCI-функцию, обходит её capability list, отображает окно BAR,
согласует `VIRTIO_F_VERSION_1`, строит split-virtqueue, выполняет настоящее
чтение сектора (4D-2), переиспользует дескрипторы одной очереди (4D-3), держит
два запроса в полёте одновременно (4D-4) и выполняет настоящую запись,
доказанную независимым обратным чтением (4D-5). Плюс срез сервис/клиент
(`block-service.sh`, 2026-09-21, исправлен 2026-09-23) и передача capability
сообщением между текстовыми процессами (`capability-transfer.sh`,
`name-service.sh`). Плюс жизненный цикл сервиса — смерть, отзыв публикации,
успешник, T1-сброс устройства и повторная публикация (`block-lifecycle.sh`,
2026-09-23).

**Что доказано гейтами на этом коммите.** Что канонический текстовый процесс,
не держащий **ничего** от машины — ни PCI-шины, ни функции, ни окна, ни
источника прерываний, ни DMA-региона, ни полномочия памяти, и не имеющий имени
сервиса до того, как реестр ADR-0093 P3 ему его пришлёт, — достигает текстового
сервиса через IPC, и что число, которое он получает в ответ, происходит от
настоящего чтения устройства: буфер сервиса был заполнен 0xA5, ответ равен 512,
и прогон без DMA ответил бы 0. Границы capability читаются из журнала загрузки,
а не предполагаются.

**Что НЕ доказано и не заявляется.**
- Запись через путь клиент/сервис есть с 2026-09-23, а с 2026-09-24 под ней есть
  **нормативный протокол**: ADR-0098 отвечает на §9 ADR-0093, и
  `source/interfaces/device/BLOCK_DEVICE_V1.md` его фиксирует —
  `word = sector * 4 + opcode`, все три операции, каждый отказ — ответ, а запись —
  **один атомарный вызов, несущий свой регион**. Его говорят канонический
  текстовый сервис и канонический текстовый клиент в `block-protocol.sh`. Старые
  `block-data-path` и `block-lifecycle` продолжают нести собственную кодировку до
  отдельного среза миграции. Один сектор на запрос;
  несколько секторов в полёте, планировщик запросов и zero-copy не построены;
  zero-copy недостижим по решению ADR-0037, а не по упущению. И ничто из этого не
  является persistent object/state storage или capsule-to-repository handoff — это
  отдельные deliverables `docs/16` для Stage 4, и ни одного из них нет.
- Полномочие публикации **реализовано** в том виде, в котором его теперь
  требует `CAPABILITY_V1` §6 (поправка ADR-0095 от 2026-09-23): выделенный
  publication endpoint, чья идентичность фиксирует, что через него можно
  опубликовать; реестр держит `receive`, авторизованный сервис — `call`. Имя
  интерфейса в протоколе не едет и не должно. Негативное свидетельство —
  `publication-authority.sh` (ADR-0095 §5).
- Случай C ADR-0093 (рестарт и повторная публикация) доказан 2026-09-23 в той
  узкой области, которую описывает запись ниже, — и только в ней. **Случай D
  остаётся намеренно неоднозначным** (§5a), а свидетельства падения в полёте и
  поведения против враждебного устройства ещё нет. Durability, персистентное
  хранение объектов/состояния, файловая система, разделы, обобщённый driver
  framework, обнаружение нескольких устройств и производительность пути данных
  Stage 4 — не построены и не заявлены.
- Stage 4C, Stage 4D и Stage 4 **не закрыты**, и формальное рассмотрение их
  закрытия открыто.

**Граница данных §1 достигнута 2026-09-23** (ADR-0097): все 512 байт сектора
переходят границу клиент/сервис обычным неизменяемым `Region<u8>` через
`MESSAGE_REGIONS`, и клиент проверяет каждый байт в каноническом тексте.

**Случай C ADR-0093 и T1-восстановление ADR-0092 R1a доказаны 2026-09-23**
(`block-lifecycle.sh`): сервис обслуживает запись и заканчивается, всё ещё держа
функцию, окно, источник прерываний и DMA-регион; супервизор собирает его
окончание, **отзывает публикацию** у реестра и только потом создаёт успешника;
успешник претендует на ту же функцию с **новой generation**, гонит
`DEVICE_STATUS` в 0 и читает обратно 0, инициализирует устройство заново в
обычном порядке и публикуется снова; свежий lookup даёт клиенту endpoint, через
который возвращаются все 512 байт, записанных первым экземпляром. Устаревшее имя
не отремонтировано: вызов по нему не обслуживается, пока успешник ждёт запроса.
Отзыв доказан именно для публикации A: в промежутке между отзывом и публикацией B
реестр отвечает на lookup **ничем**, и попытка выдать уже освобождённую запись
получает отказ нуклеуса — реестр, освободивший какую-то другую capability, сохранив
A, эту попытку прошёл бы.

**Что Stage 4 ещё должен — по списку deliverables самого `docs/16`, по пунктам, а
не одним словом:**

- **persistent object/state storage** и **capsule-to-repository handoff** — два
  разных deliverable, ни один из них не durability и ни одного нет. Engineering
  exit этого этапа в `docs/16` — «persistent storage works through a textual
  user-space driver», и драйвер, читающий и пишущий сектора, этим ещё не является;
- **падение в полёте**: первый экземпляр lifecycle-бота заканчивается уже после
  подтверждения записи, поэтому о запросе, принятом и не отвеченном, он ничего не
  спрашивает — это случай D ADR-0093, намеренно неоднозначный. Adversarial-device
  набор — отдельное обязательство;
- **отчёт по performance contract** Stage 4;
- актуализация документации и closure review.
