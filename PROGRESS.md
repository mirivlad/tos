...
- Запись через путь клиент/сервис есть с 2026-09-23 (`block-lifecycle.sh`:
  клиент пишет сектор и читает его обратно), но **кодировка запроса в нём —
  фикстурная**. Нормативный общий wire-протокол `block.device.v1` не принят из
  того, что у одной фикстуры есть кодировка write/read: ADR-0093 §9 по-прежнему
  оставляет форму `read`, `write` и `capacity` открытой. Один сектор на запрос;
