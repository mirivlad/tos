...
- Писать через путь клиент/сервис нельзя: этот срез только читает. Durability
  сверх обратного чтения 4D-5, несколько секторов в полёте, framing запросов и
  zero-copy не построены; zero-copy недостижим по решению ADR-0037, а не по
  упущению.
...
`MESSAGE_REGIONS`, и клиент проверяет каждый байт в каноническом тексте. Осталось
до рассмотрения закрытия Stage 4: случай C (рестарт и повторная публикация),
свидетельство crash/reset lifecycle, отчёт о производительности Stage 4 и сам
closure review.
...
продолжал бы читать память, которую отдал. **Ядро не менялось, ABI не менялся,
нового object kind нет.**
