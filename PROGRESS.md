...
**E. ADR-0058 — блокировка.** Форма согласована с владельцем: состояние
...
решением. Отдельно: формулировать правило живости надо как «нет Runnable **и
ничто маршрутизированное не может это изменить**» — на Stage 4 появится
устройственное прерывание, и первая формулировка тихо станет неверной.

**F. Как вызов называет больше, чем помещается в регистрах.** До четырёх
capability в сообщении (`IPC_V1` §3, ADR-0057) и наделение ребёнка при
`process_create` упираются в одно и то же: свободных аргументных регистров нет.
Слот сообщения уже существует и уже обоснован — вероятный ответ в том, чтобы
сделать его областью объёмных аргументов вызова, а не только полезной нагрузкой
сообщения. Пока не решено, ребёнок наделяется пустым набором, а сообщение не
может **назвать** ни одной capability — что сильнее отказа, но это другое
утверждение.
