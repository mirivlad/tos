...
**A. Семантика `defer` для ownership.** docs/40 §5 говорит, что `defer`
регистрируется в момент выполнения, а тело исполняется при выходе из блока.
Какое ownership-состояние тело наблюдает в этот момент, контракт не задаёт.
Реализация теперь **ничего не анализирует внутри тела `defer`** вместо выбора
семантики. Неопределённые вопросы с минимальными примерами (проверено: сейчас
не сообщается ничего):

```tos
// A1. Захватывает ли тело значения в момент регистрации или читает их при
// выходе? Если захват — это перемещение при регистрации, и take ниже уже
// ошибка; если чтение при выходе — ошибкой является defer.
pub fn main(message: Message) -> unit {
    take(message);
    defer { take(message); }
}

// A2. Может ли cleanup потреблять значение, которое scope ещё использует?
pub fn main(message: Message) -> unit {
    defer { take(message); }
    take(message);
}

// A3. Допустимо ли использование ресурса между регистрацией defer и выходом,
// если defer его потребляет? Это следствие выбора между A1 и A2.
```

Пока это не решено, `E1225_INVALID_DEFER` продолжает действовать — он о форме
тела, а не о владении.

**B. Отсутствующие диагностики при живом borrow.** docs/40 §5 запрещает
aliasing mutable borrow, но docs/44 распределяет `E1302` только на
borrow-vs-borrow, а `E1303` только на запись при живом *immutable* borrow. Три
условия остаются без назначенного кода (проверено: сейчас не сообщается
ничего):

```tos
// B1. Чтение владельцем при живом перекрывающем mutable borrow.
pub fn main() -> i32 {
...
// B2. Перемещение при любом живом перекрывающем borrow.
pub fn main(message: Message) -> unit {
    let v = borrow message;
    take(message);
}

// B3. Мутация владельцем при mutable borrow, удерживаемом другим биндингом.
pub fn main() -> unit {
    let mut c = Counter(value: 0i32);
    let m = borrow mut c;
    c.value = 1i32;
...
`E1302`/`E1303` молча не расширяются: их условия зафиксированы в реестре
docs/44 §7, и изменение — versioned language decision.
