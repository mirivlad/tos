...
- Клиент не читает сектор: 512 байт блочных данных через IPC не проходят.
  Проходит одно число, вычисленное сервисом. Граница
  `docs/research/STAGE4_DATA_PATH_BOUNDARY.md` §1 — память клиента через IPC в
  DMA-память сервиса — не достигнута, и переноса региона из канонического текста
  не существует (ADR-0094 §10).
...
**Следующий честный фронтир — регион через IPC из канонического текста**, то,
что превращает «клиент получил число» в «клиент получил байты»: 512 байт сектора
переходят границу клиент/сервис через принятый region-механизм `IPC_V1` §5 и
`MESSAGE_REGIONS`. Это граница §1 упомянутой research-заметки, и Архитектор
назначил её следующей целью 2026-09-23.
