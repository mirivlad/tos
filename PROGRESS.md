...
**Принят на уровне 3 и построен целиком, кроме одного пункта свидетельства.** Stage 4
не закрывается, 4C/4D не закрываются, следующее обязательство Stage 4 не начато.
...
**Двадцать две загрузки-отказа**, каждая отличается от обычной ровно одним фактом и
...
- `bash source/host-tools/qemu-test/repository-linkage.sh` → `PASS (the linkage proved
  from a real device, and 22 refusal(s), each with the ADR-0102 §10a class §11c names
  for it)`; 23 загрузки QEMU, 5 мин 12 с.
