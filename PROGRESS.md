...
**D. ADR-0064 (Proposed) — какой диагностикой отвечает нонконструируемый тип в
позиции значения.** Расхождение между двумя принятыми ADR (0036 §1 ↔ 0039 rev 3
§1), реестром `docs/44` и вектором R070; реализация не соответствует ни одному
из принятых текстов полностью. Три варианта, рекомендация — A. До подписи ничего
не меняю: `docs/38` называет молчаливое противоречие недопустимым и запрещает
разрешать его выбором реализации. Подробности — в записи журнала от 2026-08-21.
...
revision 3, вынесено как ADR-0064 (Proposed).** См. запись журнала от
2026-08-21. Реализация, реестр `docs/44` и вектор R070 отвечают по-разному на
один и тот же вопрос; решать это в коде `docs/38` прямо запрещает.
