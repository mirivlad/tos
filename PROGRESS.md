...
**§11d NOT MEASURED, и гейт это печатает.** Два поколения читателя запускаются и оба
собираются нуклеусом как чистые выходы, но собственный учёт второго поколения не
доходит до журнала: с середины его прогона область отчёта перестаёт сливаться, а в
логе внутри одной из его строк лежат тридцать два байта, похожие на указатели кучи.
Повторное чтение **выполнено и не засвидетельствовано** — одно поколение не считается
за два.
