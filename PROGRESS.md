...
**E. Четыре принятых ADR отсутствуют в манифесте источников.** Найдено при
принятии ADR-0064. `docs/38` §Release check: документационный релиз недействителен,
если «an accepted ADR is absent from the source manifest», а
`docs/SPECIFICATION_SOURCES.txt` не содержит ADR-0061, 0062, 0063 и 0064 — все
четыре Accepted. Не добавляю в одиночку: это четыре решения сразу и заметное
изменение сводной спецификации, а не правка при своей работе. Форма дефекта та
же, что у только что закрытого: правило записано прозой, и **ни один гейт не
проверяет, что каждый принятый ADR перечислен**. Механически проверяемо в одну
строку, если решение — добавить.
