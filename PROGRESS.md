...
**G. Как супервизор узнаёт, что сервис закончился — ADR-0067 переработан
2026-08-25, вариант D, ждёт утверждения.** Прежние A/B/C оставлены в ADR как
разобранные и отвергнутые; реализовывать их не нужно.

D: блокирующая операция `process_wait_child` (номер 14) принимает capability на
конкретный Process object с новым правом `RIGHT_WAIT_CHILD` и наблюдает только
прямых детей того объекта, который capability именует. Обычный супервизор ждёт
на authority над собой. Это не wait-for-anything: множество событий ограничено
capability и уже существующим отношением parent/child.

Уведомление — не IPC-сообщение и очередь endpoint'а не трогает. При завершении
ребёнка ядро сразу освобождает его execution resources и оставляет в слоте
(`State::Over`) tombstone: instance id ребёнка, identity родителя, вид
завершения, self-reported status (только если был `process_exit`), `ended_by` и
детерминированный порядок завершения с тиком. Слот с неполученной записью не
переиспользуется — поэтому число pending записей ограничено размером таблицы
процессов (сейчас 4), потерять уведомление нельзя, аллокаций и роста очереди
нет, а супервизор, который не собирает endings, получает `E_LIMIT` на следующем
`process_create`. Это осознанное следствие, а не побочный эффект.

`process_wait_child` отдаёт самую раннюю pending-запись прямого ребёнка; без
записей блокируется, с NON_BLOCKING даёт `E_WOULD_BLOCK`, отмена —
`E_CANCELLED`. После получения tombstone освобождается. Generation слота
поднимается в момент завершения, а не сбора, поэтому устаревший child handle
после переиспользования слота даёт `E_NO_CAPABILITY`, а не живую ссылку на
чужой процесс.

Смерть самого супервизора: audit-записи остаются на логе (они были выпущены при
завершении детей), programmatic notices без живого получателя освобождаются, и
слоты не становятся вечными. Reparenting и restart policy этим ADR не вводятся.

Отдельно закрыт restart-identity gap: `process_create` принимает
supervisor-asserted `restart_generation` в `r8`; ядро его записывает, но не
вычисляет и не инкрементирует. Первый запуск — 0, при restart супервизор
передаёт +1. И корреляция: `process_create` возвращает instance id ребёнка в
`rdx`, а lifecycle record несёт тот же id; capability handle идентичностью не
является, как и индекс слота.

Названо прямо, что в реализации сегодня нет ни boot-уникального instance id, ни
поля родителя — и то и другое входит в работу по этому ADR, а не
предполагается существующим. В ADR отдельными разделами: threat model,
boundedness, семантика slot-reuse/generation и десять conformance-тестов, среди
них — два ребёнка, завершившихся до единственного `wait`.

Реализация не начата: ADR ждёт утверждения.
