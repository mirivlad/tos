...
**F. Что обязан гарантировать локальный preflight и что — CI.** Вынесено, а не
решено: `docs/30` формулирует гейты как исходы («suites pass»), не распределяя их
между локальным прогоном и CI, поэтому «CI обязан запускать всё, что запускает
preflight» из принятых документов **не следует**. Факты для решения: CI не
запускает ни одной проверки контрактов и схем (список в записи от 2026-08-21);
ADR-0026 performance conformance, наоборот, есть только в CI — то есть зелёный
`preflight --full` не означает зелёный CI на том же коммите; шесть регресс-тестов
самих гейтов не запускает никто. Варианты, как мне видится: (а) CI обязан
запускать ровно `preflight --full`, а workflow становятся его оболочками с
разбиением на джобы; (б) фиксируется два уровня — CI доказывает подмножество,
названное в docs/30, а preflight остаётся полным, и различие документируется;
(в) статус-кво с явным списком исключений. Ни один не выбираю: это решение о
том, что означает «зелено», а не правка.
