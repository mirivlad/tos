...
**G. ADR-0060 (Proposed) — как модуль TOS Core вызывает операцию по capability.**
Уровень 3. Всё, что ниже по Stage 3, ждёт его: супервизор, читающий
`/system/policy/` и запускающий по нему процессы, — текстовый модуль, обязанный
уметь позвать `process_create`, а на вопрос идентичности Stage 3 нельзя ответить
«да», пока contract'ы capability и IPC исполняет только двоичный образ.
