...
- Писать через путь клиент/сервис нельзя: этот срез только читает. Несколько
  секторов в полёте, framing запросов и zero-copy не построены; zero-copy
  недостижим по решению ADR-0037, а не по упущению. И ничто из этого не является
  persistent object storage или capsule-to-repository handoff — это отдельные
  deliverables `docs/16` для Stage 4.
...
- Случай C ADR-0093 (рестарт и повторная публикация), случай D (намеренно
  неоднозначен, §5a), durability, планировщик запросов, файловая система,
  разделы, обобщённый driver framework, обнаружение нескольких устройств,
  производительность пути данных Stage 4 — ничего из этого не построено и не
  заявлено.
