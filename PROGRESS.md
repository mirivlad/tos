...
**H. Под каким профилем измеряется каждый из двух бюджетов латентности —
ОТКРЫТО, вынесено ADR-0068 (Proposed).** Пороги не трогаются. Вопрос в том, что
именно меряет каждая половина. Предложение: относительный `8x` меряет intrinsic
overhead и берётся при одинаковом no-preemption профиле с обеих сторон;
абсолютные `200 µs` продолжают мерить настоящий IPC с активным production
preemption и сохраняют хвосты.

Вариант «включить preemption и у denominator» разобран и отвергнут арифметикой:
вероятность попадания тика равна `интервал / период`, а интервалы различаются
вшестеро, поэтому из красного прогона получается четырёхисходная лотерея — 5,89x
без попаданий, 8,05x при попадании в numerator, **1,74x при попадании в
denominator** и 2,37x при обоих. Третья строка дисквалифицирует симметрию:
редкое прерывание в компараторе спасало бы бюджет, то есть измерение
вознаграждало бы шум прибора.

**Исправлено в Proposed ADR до утверждения:** ранговый интервал был назван
неверно. `X₍₂₉₄₎…X₍₃₀₀₎` покрывает не 95 %, а `91,82 %`; 95 % даёт
`X₍₂₉₀₎…X₍₃₀₀₎` — `95,07 %`, и 290 — самый узкий такой нижний ранг. Ошибка была
в методе: нормальная аппроксимация вокруг ранга 297 при `p=0,99` не годится,
биномиальное распределение скошено, а верхний конец интервала упирается в
максимум. В ADR теперь стоит точная формула
`Σ_{k=r}^{s-1} C(n,k) p^k (1-p)^{n-k}`, по которой любое из этих чисел
пересчитывается. Размер серии 300 не менялся.

Отдельно в ADR разобрана недостаточность 21 сэмпла: nearest-rank p99 при n=21 —
это максимум серии, он лежит на уровне `21/22 = 95,45 %`, в 81 % случаев ниже
настоящего p99, а при измеренной доле тикнутых 3 % настоящий p99 — тикнутое
значение, которое серия из 21 содержит лишь в 47,8 % прогонов. Предложено 300
удержанных измерений (p99 — 297-й порядковый статистик, 95 % ранговый интервал
294…300). Длиннее серия текущую структуру не спасает, а делает её отказ
надёжным: при n=300 хотя бы один тик попадает с вероятностью 0,9999.

Quantum и APIC divider предложено сделать обязательной идентичностью
reference profile для active-preemption измерений.

Предусловие названо прямо и не спрятано: no-preemption numerator измерен
медленнее active на `+7,23 µs` (9/12), и почему — ничем измеренным не
объяснено. Пока не объяснено, пункт 2 ADR опирается на допущение, которого
данные не подтверждают.

Ничего в `docs/35`, `IPC_V1` и гейте не изменено — по условию, до утверждения.

**G. Как супервизор узнаёт, что сервис закончился — ОТКРЫТО, вынесено
ADR-0067 (Proposed).** Пятый пункт Stage 3 evidence в `docs/37` — «service
restart preserves identity/audit records», а `PROCESS_IDENTITY_V1` §4 требует
новый instance id, инкремент restart generation и сохранение lineage. Заявить
это некому: из четырёх способов закончиться три известны только ядру, а среди
тринадцати операций `SYSTEM_ABI_V1` §5 нет ни одной, которой другой процесс мог
бы об этом спросить. Супервизор, перезапускающий по таймеру, утверждал бы
generation для события, которого не наблюдал.

Три формы ответа разобраны в ADR: (A) блокирующий `process_wait` на той же
process-authority capability — буквально по §6, но одна задача ждёт одного
ребёнка, и `retire` сегодня гасит capability в момент смерти, так что поздний
`process_wait` получает ошибку вместо исхода; (B) lifecycle-сообщение на
endpoint, названный при `process_create` — единственная форма, где одно
ожидание покрывает N детей, но она должна дописать в `IPC_V1` §7 правило
резервирования места (очередь — 4 и расти не может) и ответить на подделку,
поскольку сообщение не несёт отправителя; (C) неблокирующий `process_status` —
это тот самый polling, который ADR-0059 отверг измерением 2077 тиков против 81.

Рекомендация — B с оговорками A о границах; решение не принято и в код не
внесено. Уровень 2: любая форма добавляет операцию в закрытую таблицу.
