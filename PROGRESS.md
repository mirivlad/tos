...
**Экстенты не пересекаются, и это говорит устройство.** Инициализатор хранилища
состояния запускается до всех репозиторных чтений и после каждого из них: сначала
форматирует нулевой сектор, затем находит собственный заголовок и отказывается
форматировать.
